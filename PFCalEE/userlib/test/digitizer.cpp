...
void processHist(const unsigned iL, TH2Poly* histE,
		HGCSSDigitisation & myDigitiser, TH1F* & p_noise, const TH2Poly* histZ,
		const double & meanZpos, const bool isTBsetup,
		const HGCSSSubDetector & subdet,
		const std::vector<unsigned> & pThreshInADC, const bool pSaveDigis,
		HGCSSRecoHitVec & lDigiHits, HGCSSRecoHitVec & lRecoHits,
		const bool pMakeJets, std::vector<PseudoJet> & lParticles) {
	...
	while ((obj = next())) {
		...
		if (simEcor > 0)
			noiseFrac = (digiE - simEcor) / simEcor;
		...
		bool aboveThresh = //digiE > 0.5;
				(isSi && adc > pThreshInADC[iL])
						|| (isScint
								&& digiE
										> pThreshInADC[iL]
												* myDigitiser.adcToMIP(1, adet,
														false));
		//histE->SetBinContent(iX,iY,digiE);
		if ((!pSaveDigis && aboveThresh) || pSaveDigis) { //save hits
			...
			if (pSaveDigis)
				lDigiHits.push_back(lRecHit);

			lRecoHits.push_back(lRecHit);

			if (pMakeJets) {
				if (posz > 0)
					lParticles.push_back(
							PseudoJet(lRecHit.px(), lRecHit.py(), lRecHit.pz(),
									lRecHit.E()));
			}
...
int main(int argc, char** argv) {	//main  
	...
	if (nPar < nReqA - 1) {
		std::cout << " Usage: " << argv[0] << " <nEvts to process (0=all)>"
				<< std::endl << "<full path to input file>" << std::endl
				<< "<full path to output file>" << std::endl
				<< "<granularities \"layer_i-layer_j:factor,layer:factor,...\">"
				<< std::endl
				<< "<noise (in Mips) \"layer_i-layer_j:factor,layer:factor,...\">"
				<< std::endl
				<< "<threshold (in ADC counts) \"layer_i-layer_j:factor,layer:factor,...\">"
				<< std::endl << "<intercalib factor in %>" << std::endl
				<< "<Number of si layers for TB setups>" << std::endl
				<< "<number of PU>" << std::endl
				<< "<full path to MinBias file if nPU!=0>" << std::endl
				<< std::endl << "<optional: randomSeed (default=0)> "
				<< std::endl << "<optional: debug (default=0)>" << std::endl
	...
	if (nPU > 0 && puPath.size() == 0) {
		std::cout << " -- Error! Missing full path to minbias file. Exiting."
				<< std::endl;
		return 1;
	}
	...
	std::cout << " events." << std::endl << " -- Granularities: " << granulStr
			...
			<< " -- number of PU: " << nPU << std::endl << " -- pu file path: "
			<< puPath << std::endl;
	...
	if (nPar > nReqA)
		std::istringstream(argv[nReqA]) >> pSeed;
	if (nPar > nReqA + 1) {
		debug = atoi(argv[nReqA + 1]);
		std::cout << " -- DEBUG output is set to " << debug << std::endl;
	}
	if (nPar > nReqA + 2)
		std::istringstream(argv[nReqA + 2]) >> pSaveDigis;
	if (nPar > nReqA + 3)
		std::istringstream(argv[nReqA + 3]) >> pSaveSims;
	if (nPar > nReqA + 4)
		std::istringstream(argv[nReqA + 4]) >> pMakeJets;
	...
	if (pSaveDigis)
		std::cout << " -- DigiHits are saved." << std::endl;
	if (pSaveSims)
		std::cout << " -- SimHits are saved." << std::endl;
	if (pMakeJets)
	...
	if (nPU != 0) {
		...
		while (fgets(eoslsName, 100, script)) {
			...
			std::string temp = std::string(eoslsName).substr(0,
					strlen(eoslsName) - 1);
			if (temp.find("Digi") != temp.npos)
				continue;
			puInput << puPath << temp;
			if (puInput.str() == inFilePath) {
				std::cout
						<< " -- duplicate: file already used as signal. Removing."
						<< std::endl;
				signalIsPu = true;
				continue;
			}
			puTree->AddFile(puInput.str().c_str());
			std::cout << "Adding MinBias file:" << puInput.str().c_str()
					<< std::endl;
		}
		pclose(script);
		system("rm ./eosls.sh");
		...
		std::cout << "- Number of PU events available: " << nPuEvts
				<< std::endl;
	}
	/////////////////////////////////////////////////////////////
	//input tree
	...
	if (isTBsetup)
		std::cout << " -- Number of Si layers: " << nSiLayers << std::endl;
	else
		std::cout
				<< " -- Number of Si layers ignored: hardcoded as a function of radius in HGCSSGeometryConversion class."
				<< std::endl;
	...
	if (pSaveDigis)
		outputStr << "_withDigiHits";
	if (pSaveSims)
		outputStr << "_withSimHits";
	outputStr << ".root";
	...
	TTree *outputTree = new TTree("RecoTree",
			"HGC Standalone simulation reco tree");
	...
	if (pSaveSims)
		outputTree->Branch("HGCSSSimHitVec", "std::vector<HGCSSSimHit>",
				&lSimHits);
	if (pSaveDigis)
		outputTree->Branch("HGCSSDigiHitVec", "std::vector<HGCSSRecoHit>",
				&lDigiHits);
	outputTree->Branch("HGCSSRecoHitVec", "std::vector<HGCSSRecoHit>",
			&lRecoHits);
	outputTree->Branch("HGCSSGenParticleVec", "std::vector<HHGCSSGenParticle>",
			genvec);

	if (pMakeJets)
		outputTree->Branch("HGCSSRecoJetVec", "std::vector<HGCSSRecoJet>",
				&lCaloJets);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iH(0); iH < (*hitvec).size(); ++iH) {  //loop on hits
			...
			double realtime = mycalib.correctTime(lHit.time(), posx, posy,
					posz);
			...
			if (energy > 0
					&& lHit.silayer()
							< geomConv.getNumberOfSiLayers(type, radius)) {
				if (debug > 1)
					std::cout << " hit " << iH << " lay " << layer << " x "
							<< posx << " y " << posy << " z " << posz << " t "
							<< lHit.time() << " " << realtime << std::endl;
				geomConv.fill(type, subdetLayer, energy, realtime, posx, posy,
						posz);
			}
		...
		if (nPU != 0) {
			...
			if (signalIsPu)
				nPuVtx -= 1;
			std::cout << " -- Adding " << nPuVtx << " events to signal event: "
					<< ievt << std::endl;
			...
			for (unsigned iV(0); iV < nPuVtx; ++iV) {     //loop on interactions
				...
				while (1) {
					...
					if (lidxSet.find(ipuevt) == lidxSet.end()) {
						...
						std::cout
								<< " -- Found duplicate ! Taking another shot."
								<< std::endl;
					}
				}
				...
				for (unsigned iH(0); iH < (*puhitvec).size(); ++iH) { //loop on hits
					...
					if (lHit.energy() > 0) {
						...
						std::pair<double, double> xy = lHit.get_xy(isScint,
								geomConv);
						double posx = xy.first;      //lHit.get_x(cellSize);
						double posy = xy.second;      //lHit.get_y(cellSize);
						double posz = lHit.get_z();
						...
						if (lHit.silayer()
							...
							double energy = lHit.energy()
									* mycalib.MeVToMip(layer, radius);
							double realtime = mycalib.correctTime(lHit.time(),
									posx, posy, posz);
							bool passTime = myDigitiser.passTimeCut(type,
									realtime);
							if (!passTime)
								continue;

							if (debug > 1)
								std::cout << " hit " << iH << " lay " << layer
										<< " x " << posx << " y " << posy
										<< " z " << posz << " t " << lHit.time()
										<< " " << realtime << std::endl;
							geomConv.fill(type, subdetLayer, energy, realtime,
									posx, posy, posz);
						}
					}
		...
		if (debug > 1) {
			std::cout << " **DEBUG** simhits = " << (*hitvec).size() << " "
					<< lSimHits.size() << std::endl;
		}
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {    //loop on layers
			...
			if (debug > 1) {
				std::cout << " -- Layer " << iL << " " << subdet.name << " z="
						<< meanZpos << " totbins = " << nTotBins
						<< " histE entries = " << histE->GetEntries()
						<< std::endl;
			}
			...
			processHist(iL, histE, myDigitiser, p_noise, histZ, meanZpos,
					isTBsetup, subdet, pThreshInADC, pSaveDigis, lDigiHits,
					lRecoHits, pMakeJets, lParticles);

		}    //loop on layers

		if (debug) {
			std::cout << " **DEBUG** sim-digi-reco hits = " << (*hitvec).size()
					<< "-" << lDigiHits.size() << "-" << lRecoHits.size()
					<< std::endl;
		}
		...
		if (pMakeJets) {    //pMakeJets
			...
			// print the jets
			std::cout << "-- evt " << ievt << ": found " << jets.size()
					<< " Jets." << std::endl;
			for (unsigned i = 0; i < jets.size(); i++) {
				const PseudoJet & lFastJet = jets[i];
				//TOFIX // inverted y and z...
				HGCSSRecoJet ljet(lFastJet.px(), lFastJet.py(), lFastJet.pz(),
						lFastJet.E());
				if (lFastJet.has_constituents())
					ljet.nConstituents(lFastJet.constituents().size());
				...
				std::cout << " -------- jet " << i << ": " << lFastJet.E()
						<< " " << lFastJet.perp() << " " << lFastJet.rap()
						<< " " << lFastJet.phi() << " "
		...
		if (lSimHits.size() > maxSimHits) {
			...
			std::cout << " -- INFO: event " << ievt << " maxSimHits updated to "
					<< maxSimHits << std::endl;
		}
		...
		if (lCaloJets.size() > maxRecJets) {
			...
			std::cout << " -- INFO: event " << ievt << " maxRecJets updated to "
					<< maxRecJets << std::endl;
		}
		lSimHits.clear();
		lDigiHits.clear();
