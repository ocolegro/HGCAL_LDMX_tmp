...
HGCSSCalibration::HGCSSCalibration(std::string filePath, const bool bypassR,
		const unsigned nSi) {
...
double HGCSSCalibration::addTimeOfFlight(const double & aTime,
		const double & posx, const double & posy, const double & posz,
		const double & vtxx, const double & vtxy, const double & vtxz) {
	double distance = sqrt(
			pow(posx - vtxx - vtx_x_, 2) + pow(posy - vtxy - vtx_y_, 2)
...
double HGCSSCalibration::correctTime(const double & aTime, const double & posx,
		const double & posy, const double & posz) {
	return correctTime(aTime, posx, posy, posz, vtx_x_, vtx_y_, vtx_z_);
}

double HGCSSCalibration::correctTime(const double & aTime, const double & posx,
		const double & posy, const double & posz, const double & vtxx,
		const double & vtxy, const double & vtxz) {
	double distance = sqrt(
			pow(posx - vtxx, 2) + pow(posy - vtxy, 2) + pow(posz - vtxz, 2));
	double c = 299.792458;  //3.e8*1000./1.e9;//in mm / ns...
	double cor = distance / c;
	// if (aTime>0 && cor > aTime) std::cout << " -- Problem ! Time correction is too large ";
	// if (aTime>0) std::cout << " -- hit time,x,y,z,cor = " 
...
double HGCSSCalibration::MeVToMip(const unsigned layer, const double aRadius,
		const bool absWeight) const {
	double res = 1;
	if (layer < theDetector().nLayers())
		res =
				theDetector().subDetectorByLayer(layer).mipWeight
						* (absWeight ?
								theDetector().subDetectorByLayer(layer).absWeight :
								1.0);

	if (theDetector().subDetectorByLayer(layer).isSi == false)
		return res;

	if (bypassRadius_)
		return res * 2. / nSiLayers_;
