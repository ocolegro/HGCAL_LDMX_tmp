...
PCAShowerAnalysis::PCAShowerAnalysis(bool segmented, bool logweighting,
		bool debug) :
		principal_(0), logweighting_(logweighting), segmented_(segmented), alreadyfilled_(
				false), debug_(debug) {
...
void PCAShowerAnalysis::showerParameters(const HGCSSCluster & clus) {
	...
	if (!alreadyfilled_) {
		...
		if (debug_)
			std::cout << " -- Number of rechits in cluster: " << clus.nRecHits()
					<< " " << lmap.size() << std::endl;
		unsigned counter = 0;
		...
		if (counter != lmap.size())
			std::cout
					<< " -- Warning, not all hits found for making principals ! Found "
					<< counter << " out of " << lmap.size() << std::endl;
	}
	...
	if (debug_)
		std::cout << "*** Principal component analysis (standalone) ****"
				<< std::endl;
	if (debug_)
		std::cout << "shower average (x,y,z) = " << "("
				<< (*principal_->GetMeanValues())[0] << ", "
				<< (*principal_->GetMeanValues())[1] << ", "
				<< (*principal_->GetMeanValues())[2] << ")" << std::endl;

	TMatrixD matrix = *principal_->GetEigenVectors();
	if (debug_)
		std::cout << "shower main axis (x,y,z) = " << "(" << matrix(0, 0)
				<< ", " << matrix(1, 0) << ", " << matrix(2, 0) << ")"
				<< std::endl;

	TVectorD eigenvalues = *principal_->GetEigenValues();
	if (debug_)
		std::cout << "shower eigen values = " << "(" << eigenvalues(0) << ", "
				<< eigenvalues(1) << ", " << eigenvalues(2) << ")" << std::endl;

	TVectorD sigmas = *principal_->GetSigmas();
	if (debug_)
		std::cout << "shower sigmas = " << "(" << sigmas(0) << ", " << sigmas(1)
				<< ", " << sigmas(2) << ")" << std::endl;

	showerBarycenter = ROOT::Math::XYZPoint((*principal_->GetMeanValues())[0],
			(*principal_->GetMeanValues())[1],
			(*principal_->GetMeanValues())[2]);
	showerAxis = ROOT::Math::XYZVector(matrix(0, 0), matrix(1, 0),
			matrix(2, 0));

	showerEigenValues = ROOT::Math::XYZVector(eigenvalues(0), eigenvalues(1),
			eigenvalues(2));
	...
	if (showerAxis.z() * showerBarycenter.z() < 0) {
		showerAxis = ROOT::Math::XYZVector(-matrix(0, 0), -matrix(1, 0),
				-matrix(2, 0));
		if (debug_)
			std::cout << "PCA shower dir reverted " << showerAxis << "eta "
					<< showerAxis.eta() << " phi " << showerAxis.phi()
					<< std::endl;
	}
