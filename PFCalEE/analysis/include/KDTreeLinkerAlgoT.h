...
private:
	...
	KDTreeNodeT<DATA, DIM> *recBuild(int low, int hight, int depth,
			const KDTreeBoxT<DIM> &region);
...
void KDTreeLinkerAlgo<DATA, DIM>::build(
...
//Fast median search with Wirth algorithm in eltList between low and high indexes.
template<typename DATA, unsigned DIM>
int KDTreeLinkerAlgo<DATA, DIM>::medianSearch(int low, int high,
		int treeDepth) {
...
template<typename DATA, unsigned DIM>
void KDTreeLinkerAlgo<DATA, DIM>::recSearch(
	...
	if ((current->left == 0) && (current->right == 0)) {      //leaf case
		...
		for (unsigned i = 0; i < DIM; ++i) {
			...
			isInside *= thedim >= trackBox.dimmin[i]
					&& thedim <= trackBox.dimmax[i];
		}
		if (isInside)
		...
		for (unsigned i = 0; i < DIM; ++i) {
			...
			hasIntersection *= (regionmin < trackBox.dimmax[i]
					&& regionmax > trackBox.dimmin[i]);
		}
		if (isFullyContained) {
...
template<typename DATA, unsigned DIM>
void KDTreeLinkerAlgo<DATA, DIM>::addSubtree(
		const KDTreeNodeT<DATA, DIM> *current) {
	// By construction, current can't be null
	// assert(current != 0);
...
template<typename DATA, unsigned DIM>
KDTreeNodeT<DATA, DIM>*
KDTreeLinkerAlgo<DATA, DIM>::recBuild(int low, int high, int depth,
		const KDTreeBoxT<DIM>& region) {
