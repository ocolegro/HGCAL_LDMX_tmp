...
struct FitResult {
	...
	bool found;
	FitResult() :
			pos_x(0), pos_y(0), pos_z(0), tanangle_x(0), tanangle_y(0), found(
					false) {
	}
	;
};
...
struct Direction {
	...
	double eta() const {
		return asinh(
				1.0 / sqrt(tanangle_x * tanangle_x + tanangle_y * tanangle_y));
	}
	;
	double phi() const {
	...
	ROOT::Math::XYZPoint GetPosFF(const ROOT::Math::XYZPoint & vtx) const {
		...
		return ROOT::Math::XYZPoint(GetX(zposFF, vtx.x(), vtx.z()),
				GetY(zposFF, vtx.y(), vtx.z()), zposFF);
	}
	;
	void Print() const {
		std::cout << "tanangles = " << tanangle_x << " " << tanangle_y << " pt="
				<< dir().Pt() << " eta=" << eta() << " " << dir().Eta()
				<< " phi=" << phi() << " " << dir().Phi() << " p="
				<< dir().Mag() << std::endl;
	}
	;
...
public:
	...
	PositionFit(const unsigned nSR, const double & residualMax,
			const unsigned nLayers, const unsigned nSiLayers,
			const bool applyPuMixFix, const unsigned debug = 0,
			const bool doMatrix = true, const double& vtxx = 0,
			const double& vtxy = 0);
	...
	std::pair<unsigned, std::pair<double, double> > findMajorityValue(
			std::vector<std::pair<double, double> > & values) const;

	void initialise(TFile* outputFile, const std::string outputDir,
	...
	void getZpositions(const unsigned versionNumber, TTree *aSimTree,
			const unsigned nEvts);

	void getInitialPositions(TTree *simTree, TTree *recoTree,
			const unsigned nEvts, const unsigned G4TrackID = 1);

	bool getInitialPosition(const unsigned ievt, const unsigned nVtx,
			std::vector<HGCSSRecoHit> *rechitvec, unsigned & nTooFar,
			unsigned & nNoCluster);

	bool getGlobalMaximum(const unsigned ievt, const unsigned nVtx,
			std::vector<HGCSSRecoHit> *rechitvec,
			const ROOT::Math::XYZVector & truthPos0, double & phimax,
			double & etamax);
	...
	bool setTruthInfo(std::vector<HGCSSGenParticle> *genvec,
			const int G4TrackID);
	...
	void getEnergyWeightedPosition(std::vector<HGCSSRecoHit> *rechitvec,
			const unsigned nPU, const std::vector<double> & xmax,
			const std::vector<double> & ymax,
			std::vector<ROOT::Math::XYPoint> & recoPos,
			std::vector<double> & recoE, std::vector<unsigned> & nHits,
			std::vector<double> & puE, const bool puSubtracted = true);

	void getPuContribution(std::vector<HGCSSRecoHit> *rechitvec,
			const std::vector<double> & xmax, const std::vector<double> & ymax,
			std::vector<double> & puE);

	void fillErrorMatrix(const std::vector<ROOT::Math::XYPoint> & recoPos,
	...
	bool getPositionFromFile(const unsigned ievt,
			...
			bool cutOutliers = false, bool print = true);

	bool initialiseLeastSquareFit();
	//return 1 if no input file or <3 layers
	//return 2 if chi2/ndf>chi2ndfmax_
	//return 0 if success
	unsigned performLeastSquareFit(const unsigned ievt, FitResult & fit,
	...
	//return 2 if chi2/ndf>chi2ndfmax_
	//return 0 if success
	unsigned fitEvent(const unsigned ievt, FitResult & fit,
	...
	inline ROOT::Math::XYPoint truthPos(const unsigned iL) const {
		return ROOT::Math::XYPoint(
				truthDir_.GetX(avgZ_[iL], truthVtx_.x(), truthVtx_.z()),
