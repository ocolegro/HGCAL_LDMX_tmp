...
std::pair<float, float> minmax(const float a, const float b) {
	return (b < a ?
			std::pair<float, float>(b, a) : std::pair<float, float>(a, b));
}
...
KDTreeCube fill_and_bound_kd_tree(const std::vector<T>& points,
	...
	std::array<float, 3> minpos { { 0.0f, 0.0f, 0.0f } }, maxpos { { 0.0f, 0.0f,
			0.0f } };
	for (unsigned i = 0; i < points.size(); ++i) {
		if (!usable[i])
			continue;
		const auto& pos = points[i].position();
		nodes.emplace_back(i, (float) pos.X(), (float) pos.Y(),
				(float) pos.Z());
	...
	return KDTreeCube(minpos[0], maxpos[0], minpos[1], maxpos[1], minpos[2],
			maxpos[2]);
}
}    //namespace
