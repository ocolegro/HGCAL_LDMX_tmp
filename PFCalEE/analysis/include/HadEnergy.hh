...
public:
	HadEnergy(const std::vector<unsigned> & dropLay, HGCSSDetector & myDetector,
			TChain* lSimTree, TChain* lRecTree, TFile *outputFile,
			const unsigned pNevts);
	~HadEnergy();

	inline void addLimMIP(const double lim) {
		if (lim > 0)
			LimMIP_.push_back(lim);
	}
	;
	...
	inline void setEEtoHPar(const double EEtoHslopePar0,
			const double EEtoHslopePar1) {
	...
	double calcGlobalC(const double LimMIP, const double EmipMean,
			TH1F* spectrum);

	inline double getGlobalC(const unsigned iLim) {
		if (iLim < Cglobal_.size())
			return Cglobal_[iLim];
		else
			return 0;
	}
	;
