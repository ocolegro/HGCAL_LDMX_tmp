...
public:
	SignalRegion(const std::string inputFolder, const unsigned nLayers,
			const unsigned nevt, const HGCSSGeometryConversion & geomConv,
			const HGCSSPUenergy & puDensity, const bool applyPuMixFix,
			const unsigned versionNumber = 12, const bool doHexa = true);
	...
	ROOT::Math::XYZPoint getAccuratePos(const unsigned ievt,
			const unsigned iL) const;

	ROOT::Math::XYZPoint getAccuratePos(const FitResult & fit,
			const unsigned iL) const;
	...
	bool fillEnergies(const unsigned ievt, const HGCSSEvent & event,
	...
	inline double getEtotalSR(const unsigned iSR, const bool subtractPU) const {
		...
		for (unsigned iL(0); iL < nLayers_; iL++) {
			if (subtractPU)
				Etotal += subtractedenergySR_[iL][iSR];
			else
				Etotal += energySR_[iL][iSR];
		}
		return Etotal;
	}
	;

	inline double getSR(const unsigned iSR, const unsigned layer,
			const bool subtractPU) const {
		if (layer >= nLayers_)
			return 0;
		if (iSR >= nSR_)
			return 0;
		if (subtractPU) {
			return subtractedenergySR_[layer][iSR];
		} else {
			return energySR_[layer][iSR];
		}
	}
	;
