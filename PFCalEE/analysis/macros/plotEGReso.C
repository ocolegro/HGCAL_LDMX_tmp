...
TPad* plotCalibration(TGraphErrors *gr, TPad *pad, bool doRatio,
	...
	if (doRatio) {
		...
		for (int ip(0); ip < gr->GetN(); ++ip) {
			...
			std::cout << "Calib " << ip << " Egen=" << x << " Erec=" << y
					<< " delta=" << ((y - loffset) / lslope - x) / x
					<< std::endl;
		}
		grDelta->SetTitle("");
...
bool plotResolution(TGraphErrors *gr, TPad *pad, const unsigned ipu,
	...
	if (fitFunc->GetChisquare() / fitFunc->GetNDF() > 20) {
		...
		if (!dovsE)
			status = gr->Fit(fitFunc, "BIME0", "", 5, 100);
		else
			status = gr->Fit(fitFunc, "BIME0", "", 10, 500);
	}
...
bool retrievePuSigma(TTree *atree, TTree *atreePu, TH1F **p_sigma,
		TH2F **p_subtrEvspuE, double * calib, double etaval) {

	unsigned nLayers = 30;
	unsigned nSR = 5;
	if (!atree || !atreePu) {
		std::cout << " -- Info, both trees were not found:" << atree << " "
				<< atreePu << std::endl;
		return true;
	} else
		std::cout << " Trees found." << std::endl;
	...
	if (atree->GetBranchStatus("eventIndex") != 1
		...
		std::cout << " -- Error! Branch eventIndex not found: "
				<< atree->GetBranchStatus("eventIndex") << " "
				<< atreePu->GetBranchStatus("eventIndex") << std::endl;
		return false;
	}
	double totalE = 0;
	double totalEpu = 0;
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		for (unsigned iSR(0); iSR < nSR; ++iSR) {
			...
			atreePu->SetBranchAddress(label.str().c_str(),
					&energySR[1][iL][iSR]);
			label.str("");
			label << "subtractedenergy_" << iL << "_SR" << iSR;
			atreePu->SetBranchAddress(label.str().c_str(),
					&subtractedenergySR[iL][iSR]);
		}
	}
	int nEvtsPu = atreePu->GetEntries();
	int nEvts = atree->GetEntries();
	if (nEvts != nEvtsPu) {
		std::cout << " -- Warning, not all events found! nEvts = " << nEvts
				<< " nEvtsPu=" << nEvtsPu << std::endl;
		//return;
	}
	...
	for (int ievt(0); ievt < nEvts && ievtpu < nEvtsPu; ++ievt, ++ievtpu) { //loop on entries
		...
		if (ievt % 100 == 0) {
			//if (nSkipped < 10) 
			std::cout << "... Processing entry: " << ievt << " idx=" << evtIndex
					<< " pu " << ievtpu << " idx=" << evtIndexPu << std::endl;
		}

		if (evtIndex != evtIndexPu) {
			if (nSkipped < 10 || nSkipped % 500 == 0)
				std::cout << " -- Different events found: " << evtIndex << " "
						<< evtIndexPu;
		...
		if (energySR[1][14][2] < energySR[0][14][2] && evtIndex < 10) {
			std::cout << "nopu " << ievt << " idx=" << evtIndex << " pu "
					<< ievtpu << " idx=" << evtIndexPu << std::endl
					<< "SR2 E[14] = " << energySR[0][14][2] << " "
					<< energySR[1][14][2] << std::endl;
		}
		...
		for (unsigned iSR(0); iSR < nSR; ++iSR) {  //loop on signal region
			...
			p_subtrEvspuE[iSR]->Fill((puE[iSR] - E[iSR]) / calib[iSR],
					(puE[iSR] - subtrE[iSR]) / calib[iSR]);
		}  //loop on SR

		   //p_sigma[nSR+2]->Fill((totalEpu-totalE)/tanh(etaval)*1./calib[nSR+2]);
		if (p_sigma[nSR])
			p_sigma[nSR]->Fill((totalEpu - totalE) / calib[nSR]);

	}  //loop on events

	for (unsigned iSR(0); iSR < nSR + 1; ++iSR) {
		if (p_sigma[iSR])
			std::cout << " -- SR " << iSR << " - Found "
					<< p_sigma[iSR]->GetEntries()
					<< " events for pu subtraction sigma! nEvts = " << nEvts
					<< " nEvtsPu=" << nEvtsPu << std::endl;
	}
...
int plotEGReso() {  //main
	...
	const unsigned nS = 1;
	std::string scenario[nS] = { "model3/gamma/" };
	...
	//mycSig->Divide(2,2);
	TCanvas *mycCalibEta = new TCanvas("mycCalibEta", "mycCalibEta", 1500,
			1000);
	TCanvas *mycOffsetEta = new TCanvas("mycOffsetEta", "mycOffsetEta", 1500,
			1000);
	...
	for (unsigned ic(0); ic < nIC; ++ic) {  //loop on intercalib
		...
		for (unsigned iV(0); iV < nV; ++iV) {  //loop on versions
			for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
				...
				TString plotDir =
						"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalHexa/gittestHexa/version"
				...
				for (unsigned ieta(0); ieta < neta; ++ieta) {  //loop on eta
					...
					for (unsigned ipu(0); ipu < nPu; ++ipu) {  //loop on pu
						...
						for (unsigned iE(0); iE < nGenEnAll; ++iE) {
							...
							linputStr << plotDir;
							linputStr << "eta" << eta[ieta] << "_et"
									<< genEnAll[iE] << "_pu" << pu[ipu] << "_IC"
									<< ICval[ic] << "_Si2";
							if (!processNoFitFiles)
								linputStr << ".root";
							else
							...
							if (!inputFile) {
								...
								ltree[ieta][ipu][iE] = (TTree*) gDirectory->Get(
										"Ereso");
						...
						if (pu[ipu] > 0) {
							for (unsigned iSR(0); iSR < nSR; ++iSR) {
								label.str("");
								label << "p_sigma_" << iSR << "_"
										<< lsave.str();
								if (iSR < (nSR - 1))
									p_sigma[iSR] = new TH1F(label.str().c_str(),
											";PuE-E (GeV)", 500, -20, 20);
								else
									p_sigma[iSR] = new TH1F(label.str().c_str(),
											";PuE-E (GeV)", 5000, 0, 10000);
								label.str("");
								label << "p_subtrEvspuE_" << iSR << "_"
										<< lsave.str();
								p_subtrEvspuE[iSR] = new TH2F(
										label.str().c_str(),
										";PuE-E (GeV);PuE-PuEsubtr (GeV)", 500,
										-20, 20, 500, -20, 20);
						...
						for (unsigned iE(0); iE < nGenEn; ++iE) {
							...
							std::cout << " -- Tree entries for eta="
									<< eta[ieta] << " pu=" << pu[ipu] << " : "
									<< ltree[ieta][ipu][oldIdx[iE]]->GetEntries()
									<< std::endl;

							for (unsigned iSR(0); iSR < nSR; ++iSR) { //loop on signal region
								std::cout << " --Processing signal region: "
										<< iSR << std::endl;
								...
								if (ipu > 0)
									lName << " - " << offset[0][ieta][iSR]
											<< ")/" << calib[0][ieta][iSR];
								ltree[ieta][ipu][oldIdx[iE]]->Draw(
										lName.str().c_str(), "", "");
								lName.str("");
								...
								std::cout << " --- Reco E = entries "
										<< p_Ereco[iE][iSR]->GetEntries()
										<< " mean "
										<< p_Ereco[iE][iSR]->GetMean()
										<< " rms " << p_Ereco[iE][iSR]->GetRMS()
										<< " overflows "
										<< p_Ereco[iE][iSR]->GetBinContent(
												p_Ereco[iE][iSR]->GetNbinsX()
														+ 1) << std::endl;
								...
								lpad->cd();
								char buf[500];
								sprintf(buf, "#gamma E_{T}=%d GeV + PU %d",
										genEn[iE], pu[ipu]);
								TLatex lat;
								lat.SetTextSize(0.05);
								lat.DrawLatexNDC(0.25, 0.965, buf);
								sprintf(buf, "#eta=%3.1f, SR %d", etaval[ieta],
										iSR);
								...
								if (!dovsE)
									calibRecoFit[iSR]->SetPoint(np, genEn[iE],
											lres.mean);
								else
									calibRecoFit[iSR]->SetPoint(np,
											E(genEn[iE], eta[ieta]), lres.mean);
								calibRecoFit[iSR]->SetPointError(np, 0.0,
										lres.meanerr);

								double reso = fabs(lres.sigma / lres.mean);
								if (!dovsE)
									resoRecoFit[ipu][ieta][iSR]->SetPoint(np,
											genEn[iE], reso);
								else
									resoRecoFit[ipu][ieta][iSR]->SetPoint(np,
											E(genEn[iE], eta[ieta]), reso);
								double errFit =
										reso
												* sqrt(
														pow(
																...
																+ pow(
																		lres.meanerr
																				/ lres.mean,
																		2));
								resoRecoFit[ipu][ieta][iSR]->SetPointError(np,
										0, errFit);
							...
							if (ipu > 1 && genEn[iE] > 5 && genEn[iE] < 40) {
								bool success = retrievePuSigma(
										ltree[ieta][1][oldIdx[iE]],
										ltree[ieta][ipu][oldIdx[iE]], p_sigma,
										p_subtrEvspuE, calib[0][ieta],
										etaval[ieta]);
								if (!success)
									return 1;
								mycSig->cd();
						...
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							TPad *upper = plotCalibration(calibRecoFit[iSR],
									...
									offset[ipu][ieta][iSR],
									offsetErr[ipu][ieta][iSR], eta[ieta],
									dovsE);
							upper->cd();
							char buf[500];
							sprintf(buf, "#gamma #eta=%3.1f + PU %d",
									etaval[ieta], pu[ipu]);
							TLatex lat;
							lat.SetTextSize(0.1);
						...
						if (ipu == 0)
							...
							lsave << "Calib";
						lsave << "_eta" << eta[ieta] << "_pu" << puOption
								<< pSuffix;
						if (dovsE)
							lsave << "_vsE";
						...
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							double stoch0 =
									pu[ipu] == 0 ?
											(dovsE ? 0.25 : 0.14) :
											sigmaStoch[1][ieta][iSR];
							double const0 =
									pu[ipu] == 0 ?
											0.01 : sigmaConst[1][ieta][iSR];

							//limit range to get more realistic RMS ?
							//if (pu[ipu]!=0 && iSR<(nSR-1)) p_sigma[iSR]->GetXaxis()->SetRangeUser(0,5);
							double noise0 =
									pu[ipu] == 0 ? 0 : p_sigma[iSR]->GetRMS();

							bool success = plotResolution(
									resoRecoFit[ipu][ieta][iSR], lpad, ipu,
									eta[ieta], stoch0, const0, noise0,
									...
									sigmaNoiseErr[ipu][ieta][iSR], dovsE);
							lpad->cd();
							char buf[500];
							sprintf(buf, "#gamma #eta=%3.1f + PU %d",
									etaval[ieta], pu[ipu]);
				...
				for (unsigned ipu(0); ipu < nPu; ++ipu) {	    //loop on pu
					...
					for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
						...
						for (unsigned ieta(0); ieta < neta; ++ieta) {//loop on eta
							grSlope[ipu][iSR]->SetPoint(ieta, etaval[ieta],
									calib[ipu][ieta][iSR]);
							grSlope[ipu][iSR]->SetPointError(ieta, 0.0,
									calibErr[ipu][ieta][iSR]);

							grOffset[ipu][iSR]->SetPoint(ieta, etaval[ieta],
									offset[ipu][ieta][iSR]);
							grOffset[ipu][iSR]->SetPointError(ieta, 0.0,
				...
				if (dovsE) {
					...
					grDummy->SetPoint(1,
							E(genEnAll[nGenEnAll - 1], eta[neta - 1]), 0.1);
				} else {
				...
				for (unsigned ipu(1); ipu < nPu; ++ipu) {	    //loop on pu
					...
					for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
						...
						for (unsigned ieta(0); ieta < neta; ++ieta) {//loop on eta
							...
							resoRecoFit[ipu][ieta][iSR]->SetMarkerColor(
									ieta + 1);
							resoRecoFit[ipu][ieta][iSR]->SetMarkerStyle(
									ieta + 20);
							resoRecoFit[ipu][ieta][iSR]->Draw("PLsame");
							label.str("");
							label << "#eta = " << etaval[ieta];
							legeta->AddEntry(resoRecoFit[ipu][ieta][iSR],
									label.str().c_str(), "P");
						}	    //loop on eta
								//add ref pu=0 curve
						if (pu[ipu] != 0) {
							...
							legeta->AddEntry(resoRecoFit[1][0][iSR], "Ref pu=0",
									"L");
						}
						legeta->Draw("same");
				...
				if (neta > 1) {
					...
					for (unsigned iC(0); iC < 3; ++iC) {
						...
						for (unsigned ieta = 0; ieta < neta; ++ieta) {//loop on pt values
							if (ieta < 4)
								mypad[iC][ieta] = (TPad*) left[iC]->GetPad(
										ieta + 1);
							else
								mypad[iC][ieta] = (TPad*) right[iC]->GetPad(
										(ieta - 4) + 1);
						}
					}
				} else {
				...
				for (unsigned ieta(0); ieta < neta; ++ieta) {
					...
					for (unsigned ipu(0); ipu < ((nPu > 1) ? (nPu - 1) : nPu);
						...
						grStoch[ipu][ieta] = new TGraphErrors(nSR, srval,
								sigmaStoch[ipu + 1][ieta], srerr,
								sigmaStochErr[ipu + 1][ieta]);
						grConst[ipu][ieta] = new TGraphErrors(nSR, srval,
								sigmaConst[ipu + 1][ieta], srerr,
								sigmaConstErr[ipu + 1][ieta]);
						grNoise[ipu][ieta] = new TGraphErrors(nSR, srval,
								sigmaNoise[ipu + 1][ieta], srerr,
								sigmaNoiseErr[ipu + 1][ieta]);

						TGraphErrors *gr = 0;
						for (unsigned iP(0); iP < 3; ++iP) {
							gr = (iP == 0) ? grStoch[ipu][ieta] :
									(iP == 1) ?
											grConst[ipu][ieta] :
											grNoise[ipu][ieta];
							if (!gr)
							...
							gr->SetMinimum(0);
							if (iP < 2)
								gr->SetMaximum(iP == 0 ? 0.5 : 0.06);
							else
								gr->SetMaximum(5);
							if (iP == 0)
								gr->SetTitle(
										";SR;sampling term (GeV^{#frac{1}{2}})");
							else if (iP == 1)
								gr->SetTitle(";SR;constant term");
							else
								gr->SetTitle(";SR;noise term (GeV)");
