			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				for (unsigned iH(0); iH < nHists; ++iH) {
					...
					maxNhit[iH]->GetXaxis()->SetRange(
							maxNhit[iH]->FindFirstBinAbove(0),
							maxNhit[iH]->FindLastBinAbove(0));
					...
					maxNhit[iH]->GetXaxis()->SetTitle(
							"#frac{E_{missed}}{E_{tot}}");
					//maxNhit[iH]->DrawNormalized(iH==0?"PE":"PEsame");
					maxNhit[iH]->Sumw2();
					if (!integrate && maxNhit[iH]->GetEntries() > 0)
						maxNhit[iH]->Scale(1. / maxNhit[iH]->GetEntries());
