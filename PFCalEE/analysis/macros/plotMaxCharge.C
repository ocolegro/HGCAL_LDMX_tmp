...
int plotMaxCharge() { //main
	...
	for (unsigned ic(0); ic < nC; ++ic) {
		label.str("");
		label << "myc" << ic;
		myc[ic] = new TCanvas(label.str().c_str(), label.str().c_str(), 1500,
				750);
		myc[ic]->Divide(2, 1);
	}
	...
	std::string histname[nC] = { "hxy", "hphieta", "hxyAbove", "hphietaAbove",
			"nAbove", "nTot", "prof_nAbovevsLayer", "EmaxNeighbourSC3D",
			"hLayerProfile_above50mips", "EmaxNeighbourSC3DInt" };
	...
	EmaxNeighbourSC3DInt[0] =
			new TH1F("EmaxNeighbourSC3DInt0",
					";E_{max}^{SC3D} (Mips);(hits E<E_{max}^{SC3D})/(hits in 1.9 < #eta < 2.1)",
					400, 0, 100);
	EmaxNeighbourSC3DInt[1] =
			new TH1F("EmaxNeighbourSC3DInt1",
					";E_{max}^{SC3D} (Mips);(hits E<E_{max}^{SC3D})/(hits in 1.9 < #eta < 2.1)",
					400, 0, 100);
	...
	for (unsigned ih(0); ih < nC; ++ih) {
		for (unsigned iF(0); iF < npu; ++iF) {
			...
			if (ih < 4) {
				...
				gStyle->SetOptStat("eMRuo");
				if (ih < (nC - 1))
					hist[ih] = (TH1F*) gDirectory->Get(histname[ih].c_str());
				else
					hist[ih] = EmaxNeighbourSC3DInt[iF];
				if (!hist[ih])
					return 1;
				...
				unsigned total = 0;
				std::cout << " -- n in bin 0 = " << hist[ih]->GetBinContent(1)
						<< std::endl;
				for (int b(1); b < hist[ih]->GetNbinsX() + 1; ++b) {
					total += hist[ih]->GetBinContent(b)
							* hist[ih]->GetXaxis()->GetBinLowEdge(b);
				}
				std::cout << " File " << iF << " hist " << histname[ih]
						<< " nTotalCells = " << total << std::endl;
				if (ih == 4) {
					...
					hist[ih]->GetXaxis()->SetRangeUser(0, 130);
				else if (ih == 6) {
					gPad->SetLogy(1);
					gStyle->SetOptStat(0);
					...
					for (int b(1); b < hist[ih]->GetNbinsX() + 1; ++b) {
						double error = 0;
						double integral = hist[ih]->IntegralAndError(0, b,
								error);
						EmaxNeighbourSC3DInt[iF]->SetBinContent(b,
								integral
										/ hist[ih]->Integral(0,
												hist[ih]->GetNbinsX() + 2));
						EmaxNeighbourSC3DInt[iF]->SetBinError(b,
								error
										/ hist[ih]->Integral(0,
												hist[ih]->GetNbinsX() + 2));
					}
					std::cout << " -- Check integral : "
							<< hist[ih]->Integral(0, hist[ih]->GetNbinsX() + 2)
							<< std::endl;
				} else if (ih == 8)
					gStyle->SetOptStat("e");
				else if (ih == 9) {
