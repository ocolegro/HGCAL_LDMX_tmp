			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				if (integrate) {
					for (unsigned iH(0); iH < nHists; ++iH) {
						double inttot = maxNhit[iH]->Integral(0,
								maxNhit[iH]->GetNbinsX() + 1);
						for (int bin(0); bin < maxNhit[0]->GetNbinsX() + 2;
								++bin) {
							double err = 0;
							double integral = maxNhit[iH]->IntegralAndError(bin,
									maxNhit[iH]->GetNbinsX() + 1, err);
				...
				for (unsigned iH(0); iH < nHists; ++iH) {
					...
					maxNhit[iH]->GetXaxis()->SetRange(
							maxNhit[iH]->FindFirstBinAbove(0), 25);	//maxNhit[iH]->FindLastBinAbove(0));
					...
					maxNhit[iH]->Sumw2();
					if (!integrate
							&& maxNhit[iH]->Integral(0,
									maxNhit[iH]->GetNbinsX() + 1) > 0)
						maxNhit[iH]->Scale(
								1.
										/ maxNhit[iH]->Integral(0,
												maxNhit[iH]->GetNbinsX() + 1));
					...
					if (iH == 0) {
						...
						//if (nGenEn==1) 
						sprintf(buf,
								"Geant4 Standalone, e- %d GeV, %s #mu m Si",
								genEn[iE], pSuffix.c_str());
						lat.DrawLatexNDC(0.1, 0.92, buf);
					}
				}	  //loop on hists
				...
				mycL->cd(iE + 1);
				if (genEn[iE] != 2000)
					lat.DrawLatexNDC(0.5, 0.6, "Saturation: 3000 MIPs");
				else
					lat.DrawLatexNDC(0.11, 0.6, "Saturation: 3000 MIPs");
