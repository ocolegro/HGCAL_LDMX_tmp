...
int plotSubtractedE() {
	...
	if (!f) {
		std::cout << " -- Error, cannot open input root file " << filename
				<< ". Exiting." << std::endl;
		return 1;
	}
	...
	for (unsigned ipu(0); ipu < nPu; ++ipu) { //loop on pu
		...
		for (unsigned ieta(0); ieta < neta; ++ieta) { //loop on eta
			...
			grsigma[ieta] = (TGraphErrors *) grmean[ieta]->Clone(
					"grsigma" + srStr);
			...
			for (unsigned iSR(0); iSR < nSR; ++iSR) { //loop on signal region
				...
				if (!hist) {
					std::cout << " -- Error, could not find hist "
							<< label.str() << ". Continue..." << std::endl;
					//return 1;
					continue;
				}
				label.str("");
				...
				sprintf(buf, "Mean = %3.3f +/- %3.3f GeV", hist->GetMean(),
						hist->GetMeanError());
				lat.DrawLatexNDC(0.2, 0.8, buf);
				sprintf(buf, "RMS = %3.3f +/- %3.3f GeV", hist->GetRMS(),
						hist->GetRMSError());
				...
				if (iSR == 2) {
					...
					sprintf(buf, "SR %d", iSR);
					lat.DrawLatexNDC(0.2, 0.87, buf);
					sprintf(buf, "Mean x = %3.2f +/- %3.2f GeV",
							hist2d->GetMean(1), hist2d->GetMeanError(1));
					lat.DrawLatexNDC(0.2, 0.8, buf);
					sprintf(buf, "Mean y = %3.2f +/- %3.2f GeV",
							hist2d->GetMean(2), hist2d->GetMeanError(2));
					lat.DrawLatexNDC(0.2, 0.73, buf);
					sprintf(buf, "RMS x = %3.2f +/- %3.2f GeV",
							hist2d->GetRMS(1), hist2d->GetRMSError(1));
					lat.DrawLatexNDC(0.2, 0.66, buf);
					sprintf(buf, "RMS y = %3.2f +/- %3.2f GeV",
							hist2d->GetRMS(2), hist2d->GetRMSError(2));
					lat.DrawLatexNDC(0.2, 0.59, buf);
				}
				...
				if (iSR < nSR - 1) {
					...
					if (hist->GetMean() > maxmean)
						maxmean = hist->GetMean();
					if (hist->GetRMS() > maxsig)
						maxsig = hist->GetRMS();
				}
		...
		for (unsigned ieta(1); ieta < neta + 1; ++ieta) {	//loop on eta
			...
			lsave << "PLOTS/SubtractedE_pu" << pu[ipu] << "_eta"
					<< eta[ieta - 1] << ".pdf";
		...
		for (unsigned ieta(0); ieta < neta; ++ieta) {	//loop on eta
			...
			std::cout << " -- eta=" << eta[ieta] << " npoints = " << np
					<< " min=" << grmean[ieta]->GetMinimum() << " max="
					<< grmean[ieta]->GetMaximum() << std::endl;
