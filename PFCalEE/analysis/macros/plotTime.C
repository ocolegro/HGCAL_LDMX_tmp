...
int plotTime() { //main
	...
	for (unsigned iE(0); iE < nGenEn; ++iE) {
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			...
			if (!p_time[iE][iL]) {
				std::cout
						<< " -- ERROR, pointer for histogram Etot is null for layer: "
						<< iL << ". Exiting..." << std::endl;
				return 1;
			}
			if (p_time[iE][iL]->GetMaximum() > maxY)
				maxY = p_time[iE][iL]->GetMaximum();
			//if (iL==0) p_timeTot[iE] = (TH1F*)p_time[iE][iL]->Clone();
			//else p_timeTot[iE]->Add(p_time[iE][iL]);
			std::cout << " Layer " << iL << " mean = "
					<< p_time[iE][iL]->GetMean() << " RMS = "
					<< p_time[iE][iL]->GetRMS() << std::endl;
		}
		for (unsigned iL(0); iL < nLayers; ++iL) {
