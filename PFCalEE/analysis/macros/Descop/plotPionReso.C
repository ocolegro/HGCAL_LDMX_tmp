...
double absWeight(const unsigned layer, const bool dedx = true) {
	if (dedx == false) {
		if (layer == 0)
			return 0.0378011; //6.285/95.4=0.06588;
		if (layer == 1)
			return 1; //95.4/95.4=1
		if (layer == 2)
			return 0.646989; //88.16/95.4=0.92
		if (layer == 3)
			return 0.617619; //51.245/95.4=0.537
		if (layer == 4)
			return 0.646989;
		if (layer == 5)
			return 0.617619;
		...
		if (layer == 10)
			return 0.646989;
		if (layer == 11)
			return 0.942829; //74.45/95.4=0.78
		if (layer == 12)
			return 0.859702; //102.174/95.4=1.071
		if (layer == 13)
			return 0.942829;
		if (layer == 14)
			return 0.859702;
		...
		if (layer == 20)
			return 0.859702;
		if (layer == 21)
			return 1.37644; //105.39/95.4=1.1047
		if (layer == 22)
			return 1.30447; //131.476/95.4=1.378
		if (layer == 23)
			return 1.37644;
		if (layer == 24)
			return 1.30447;
		...
		if (layer == 1)
			return 1; //95.4/95.4=1
		if (layer == 2)
			return 0.92; //88.16/95.4=0.92
		if (layer == 3)
			return 0.537; //51.245/95.4=0.537
		if (layer == 4)
			return 0.92;
		if (layer == 5)
			return 0.537;
		...
		if (layer == 10)
			return 0.92;
		if (layer == 11)
			return 0.78; //74.45/95.4=0.78
		if (layer == 12)
			return 1.071; //102.174/95.4=1.071
		if (layer == 13)
			return 0.78;
		if (layer == 14)
			return 1.071;
		...
		if (layer == 20)
			return 1.071;
		if (layer == 21)
			return 1.1047; //105.39/95.4=1.1047
		if (layer == 22)
			return 1.378; //131.476/95.4=1.378
		if (layer == 23)
			return 1.1047;
		if (layer == 24)
			return 1.378;
...
unsigned fitEnergy(TH1F *hist, TPad *pad, std::string unitStr, FitResult & lres,
		unsigned isr) {
	...
	TF1 *fitResult = new TF1("fitResult", "[0]*TMath::Gaus(x,[1],[2],0)",
	...
	if (status != 0 && status != 4000) {
		std::cout << " Warning! Fit failed with status " << status
				<< "! Please have a look at the verbose output below...."
				<< std::endl;
		fitResult->SetParameters(hist->Integral(),
				hist->GetXaxis()->GetBinCenter(hist->GetMaximumBin()),
				hist->GetRMS());
		...
		hist->Fit("fitResult", "L0VEMI", "",
				...
				fitResult->GetParameter(1)
						+ nRMSp * fitResult->GetParameter(2));
	...
	char buf[500];
	TLatex lat;
	double latx = hist->GetXaxis()->GetXmin()
			+ (hist->GetXaxis()->GetXmax() - hist->GetXaxis()->GetXmin()) / 20.;
	double laty = hist->GetMaximum();
	sprintf(buf, "<Efit> = %3.3f +/- %3.3f %s", fitResult->GetParameter(1),
			fitResult->GetParError(1), unitStr.c_str());
	lat.DrawLatex(latx, laty * 0.9, buf);
	sprintf(buf, "RMSfit = %3.3f +/- %3.3f %s", fitResult->GetParameter(2),
			fitResult->GetParError(2), unitStr.c_str());
	lat.DrawLatex(latx, laty * 0.8, buf);
	sprintf(buf, "RMS/meanfit = %3.3f",
			fitResult->GetParameter(2) / fitResult->GetParameter(1));
	lat.DrawLatex(latx, laty * 0.7, buf);

	sprintf(buf, "#chi^{2}/N = %3.3f/%d = %3.3f", fitResult->GetChisquare(),
			fitResult->GetNDF(),
...
TPad* plotCalibration(TGraphErrors *gr, TPad *pad, bool doRatio,
		TGraphErrors *grDelta, std::string unit, double & calib,
		double & calibErr, double & offset, double & offsetErr,
	...
	gr->GetYaxis()->SetTitle(
			("Average energy deposited (" + unit + ")").c_str());
	char buf[500];
	TF1 *fitFunc = new TF1("calib", "[0]+[1]*x", gr->GetXaxis()->GetXmin(),
			gr->GetXaxis()->GetXmax());
	...
	if (dovsE)
		gr->Fit(fitFunc, "QIME", "same", 0, 400);
	else
		gr->Fit(fitFunc, "QIME", "same");
	TLatex lat;
	lat.SetTextColor(6);
	lat.SetTextSize(0.1);
	if (!dovsE)
	...
	sprintf(buf, "a = %3.3f #pm %3.3f %s", fitFunc->GetParameter(0),
			fitFunc->GetParError(0), unit.c_str());
	lat.DrawLatexNDC(0.2, 0.7, buf);
	sprintf(buf, "b = %3.3f #pm %3.3f %s/GeV", fitFunc->GetParameter(1),
			fitFunc->GetParError(1), unit.c_str());
	...
	sprintf(buf, "#chi^{2}/N = %3.3f/%d = %3.3f", fitFunc->GetChisquare(),
			fitFunc->GetNDF(), fitFunc->GetChisquare() / fitFunc->GetNDF());
	...
	if (doRatio) {
		...
		if (!dovsE)
			...
			grDelta->GetXaxis()->SetTitle("E (GeV)");
		grDelta->GetYaxis()->SetTitle("(#Delta E)/E");

		TLine *line = new TLine(grDelta->GetXaxis()->GetXmin(), 0,
				grDelta->GetXaxis()->GetXmax(), 0);
...
TPad* plotCalibrationRev(TGraphErrors *gr, TPad *pad, bool doRatio,
		TGraphErrors *grDelta, std::string unit, double & calib2,
		double & calib2Err, double & calib, double & calibErr, double & offset,
		double & offsetErr, const unsigned eta, const bool dovsE) {
	...
	TF1 *fitFunc = new TF1("calib", "[0]+[1]*x+[2]*x*x",
	...
	//else sprintf(buf,"<E> #propto a + b #times E ");
		...
		sprintf(buf, "<E> #propto a + b #times E + c #times E^{2} ");
	lat.DrawLatexNDC(0.2, 0.85, buf);
	sprintf(buf, "a = %3.3f #pm %3.3f %s", fitFunc->GetParameter(0),
			fitFunc->GetParError(0), unit.c_str());
	lat.DrawLatexNDC(0.2, 0.7, buf);
	sprintf(buf, "b = %3.3f #pm %3.3f %s/GeV", fitFunc->GetParameter(1),
			fitFunc->GetParError(1), unit.c_str());
	lat.DrawLatexNDC(0.2, 0.55, buf);
	sprintf(buf, "c = %3.2fe-5 #pm %3.2fe-5 %s/GeV",
			fitFunc->GetParameter(2) * 100000, fitFunc->GetParError(2) * 100000,
			unit.c_str());
	lat.DrawLatexNDC(0.2, 0.4, buf);
	sprintf(buf, "#chi^{2}/N = %3.3f/%d = %3.3f", fitFunc->GetChisquare(),
			fitFunc->GetNDF(), fitFunc->GetChisquare() / fitFunc->GetNDF());
	...
	if (doRatio) {
		...
		for (int ip(0); ip < gr->GetN(); ++ip) {
			...
			double y = 0;
			gr->GetPoint(ip, x, y);
			grDelta->SetPoint(ip, x,
					(loffset + lslope * x + lslope2 * x * x - y) / y);
...
bool plotResolution(TGraphErrors *gr, TPad *pad, const unsigned ipu,
		const unsigned eta, const double & stoch0, const double & const0,
		const double & noise0, double & stoch, double & stochErr,
		double & constant, double & constErr, double & noise, double & noiseErr,
		const bool dovsE) {
	...
	TF1 *fitFunc = new TF1("reso", "sqrt([0]*[0]/x+[1]*[1]+[2]*[2]/(x*x))",
			gr->GetXaxis()->GetXmin(), gr->GetXaxis()->GetXmax());
	...
	std::cout << " Initial params: " << fitFunc->GetParameter(0) << " "
			<< fitFunc->GetParameter(1) << " " << fitFunc->GetParameter(2)
			<< std::endl;
	...
	if (fitFunc->GetChisquare() / fitFunc->GetNDF() > 20) {
		...
		if (!dovsE)
			status = gr->Fit(fitFunc, "BIME0Q", "", 5, 100);
		else
			status = gr->Fit(fitFunc, "BIME0Q", "", 10, 500);
	}
	...
	if (!dovsE)
		sprintf(buf,
				"#frac{#sigma}{E} #propto #frac{s}{#sqrt{E_{T}}} #oplus c #oplus #frac{n}{E_{T}}");
	else
		sprintf(buf,
				"#frac{#sigma}{E} #propto #frac{s}{#sqrt{E}} #oplus c #oplus #frac{n}{E}");
	...
	sprintf(buf, "#chi^{2}/N = %3.1f/%d = %3.1f", fitFunc->GetChisquare(),
			fitFunc->GetNDF(), fitFunc->GetChisquare() / fitFunc->GetNDF());
...
double HoverEcor(const double & ee, const double & fh, const double & bh,
		const double & Cglobal, const unsigned model) {
...
int plotPionReso() {      //main
	...
	for (unsigned iE(0); iE < nGenEnAll; ++iE) {
		...
		mycE[iE] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		//mycE[iE]->Divide(2,1);
	}
	...
	mycEtot->Divide(4, 2);
	TCanvas *mycCalibLayer = new TCanvas("mycCalibLayer", "mycCalibLayer", 1500,
			1000);
	TCanvas *mycCalib2Layer = new TCanvas("mycCalib2Layer", "mycCalib2Layer",
			1500, 1000);
	TCanvas *mycOffsetLayer = new TCanvas("mycOffsetLayer", "mycOffsetLayer",
			1500, 1000);
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		std::ostringstream lName;
		lName << "myc" << iC;
		myc[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		//myc[iC]->Divide(1,2);
	}
	...
	for (unsigned iSR(0); iSR < nSR; ++iSR) {  //loop on signal region
		...
		p_chi2ndf[iSR] = new TH1F(label.str().c_str(), ";#chi^{2}/N;entries",
				500, 0, 50);
	...
	for (unsigned ic(0); ic < nRemove; ++ic) {  //loop on intercalib
		...
		for (unsigned iV(0); iV < nV; ++iV) {  //loop on versions
			for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
				...
				TString plotDir =
						"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalDescop/gitV04-02-02/version"
								+ version[iV] + "/" + scenario[iS] + "/";
				...
				for (unsigned ipu(0); ipu < nPu; ++ipu) {  //loop on pu
					...
					for (unsigned iE(0); iE < nGenEnAll; ++iE) {
						...
						if (!inputFile[iE]) {
							...
							if (!ltree[ipu][iE]) {
								std::cout << " -- File "
										<< inputFile[iE]->GetName()
										<< " sucessfully opened but tree Ereso not found! Skipping."
										<< std::endl;
								skip[iE] = true;
							} else {
								std::cout << " -- File "
										<< inputFile[iE]->GetName()
										<< " sucessfully opened and tree found."
										<< std::endl;
								nValid++;
							}
						}
					...
					for (unsigned iL(0); iL < nGF; ++iL) {	    //loop on layers
						...
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							calibRecoDelta[iSR] =
									(TGraphErrors *) calibRecoFit[iSR]->Clone(
											"calibRecoDelta" + srStr);
							resoRecoFit[ipu][iL][iSR] =
									(TGraphErrors *) calibRecoFit[iSR]->Clone(
											"resoRecoFit" + srStr);
						}
						...
						for (unsigned iE(0); iE < nGenEn; ++iE) {
							...
							std::cout << " -- Tree entries for eta=" << eta
									<< " pu=" << pu[ipu] << " : "
									<< ltree[ipu][oldIdx[iE]]->GetEntries()
									<< std::endl;
							...
							for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
								...
								lName.str("");
								if (ipu > 1)
									lName << offset[1][iL][iSR] << "+"
											<< calib[1][iL][iSR] << "*";
								//if (ipu>0) lName << "(";
								if (iL == 0) {
									lName << "HoverEcor(EECAL,EFHCAL,EBHCAL,1,"
											<< tpmod << ")";
								} else if (iL == 1) {
									if (genEn[iE] < 800)
										lName
												<< "HoverEcor(EECAL,EFHCAL,EBHCAL,globalC_15,"
												<< tpmod << ")";
									else
										lName
												<< "HoverEcor(EECAL,EFHCAL,EBHCAL,globalC_25,"
												<< tpmod << ")";
								} else if (iL > 1) {
									lName
											<< "HoverEcor(EECAL,EFHCAL,EBHCAL,globalC_"
											<< list[iL] << "," << tpmod << ")";
								}
								//if (ipu>0) lName << " - " << offset[0][iL][iSR] << ")/" << calib[0][iL][iSR];

								if (ipu > 1) {
									lName << "+" << calib2[1][iL][iSR]
											<< "*pow(";
									if (iL == 0) {
										lName
												<< "HoverEcor(EECAL,EFHCAL,EBHCAL,1,"
												<< tpmod << ")";
									} else if (iL == 1) {
										if (genEn[iE] < 800)
											lName
								...
								ltree[ipu][oldIdx[iE]]->Draw(
										lName.str().c_str(), "", "");
								lName.str("");
								lName << "energy" << genEn[iE] << "_SR" << iSR;
								p_Ereco[iE][iSR] = (TH1F*) (gPad->GetPrimitive(
										"htemp"))->Clone(lName.str().c_str()); // 1D
								//if (iSR==7) p_Ereco[iE][iSR] = (TH1F*)gDirectory->Get("p_wgtEtotal");
								if (!p_Ereco[iE][iSR]) {
									std::cout
											<< " -- ERROR, pointer for histogram "
											<< lName.str() << " is null."
											<< std::endl;
									return 1;
								}
								...
								FitResult lres;
								if (fitEnergy(p_Ereco[iE][iSR], lpad, unit,
										lres, iSR) != 0)
									return 1;
								lpad->cd();
								char buf[500];
								sprintf(buf, "#pi^{-} E_{T}=%d GeV + PU %d",
										genEn[iE], pu[ipu]);
								TLatex lat;
								...
								if (ipu < 1) {
									if (!dovsE)
										calibRecoFit[iSR]->SetPoint(np,
												genEn[iE], lres.mean);
									else
										calibRecoFit[iSR]->SetPoint(np,
												E(genEn[iE], eta), lres.mean);
									calibRecoFit[iSR]->SetPointError(np, 0.0,
											lres.meanerr);
								} else {
									if (!dovsE)
										calibRecoFit[iSR]->SetPoint(np,
										...
										calibRecoFit[iSR]->SetPoint(np,
												lres.mean, E(genEn[iE], eta));
									calibRecoFit[iSR]->SetPointError(np,
											lres.meanerr, 0.0);
								}
								double reso = fabs(lres.sigma / lres.mean);
								if (!dovsE)
									resoRecoFit[ipu][iL][iSR]->SetPoint(np,
											genEn[iE], reso);
								else
									resoRecoFit[ipu][iL][iSR]->SetPoint(np,
											E(genEn[iE], eta), reso);
														...
														pow(
																...
																+ pow(
																		lres.meanerr
																				/ lres.mean,
																		2));
								resoRecoFit[ipu][iL][iSR]->SetPointError(np, 0,
										errFit);

							}	      //loop on SR

							saveName.str("");
							saveName << plotDir << "/Ereco_lay" << iL << "_pu"
									<< puOption;
							if (ipu == 0)
								saveName << "raw";
						...
						if (ipu == 10) {
							for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
								if (pu[ipu] != 0 && iSR == nSR - 1)
									continue;
								TPad *lpad = (TPad*) (myc[0]->cd());//iSR+1));
								TPad *upper = plotCalibration(calibRecoFit[iSR],
										lpad, true, calibRecoDelta[iSR], unit,
										calib[ipu][iL][iSR],
										calibErr[ipu][iL][iSR],
										offset[ipu][iL][iSR],
										offsetErr[ipu][iL][iSR], eta, dovsE);
								upper->cd();
								char buf[500];
								sprintf(buf, "#pi^{-} #eta=%3.1f + PU %d",
										etaval, pu[ipu]);
								TLatex lat;
								lat.SetTextSize(0.1);
							...
							for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
								...
								TPad *upper = plotCalibrationRev(
										calibRecoFit[iSR], lpad, true,
										calibRecoDelta[iSR], unit,
										calib2[ipu][iL][iSR],
							...
							for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
								...
								bool success = plotResolution(
										resoRecoFit[ipu][iL][iSR], lpad, ipu,
										eta, stoch0, const0, noise0,
										sigmaStoch[ipu][iSR][iL],
										sigmaStochErr[ipu][iSR][iL],
										sigmaConst[ipu][iSR][iL],
										sigmaConstErr[ipu][iSR][iL],
										sigmaNoise[ipu][iSR][iL],
										sigmaNoiseErr[ipu][iSR][iL], dovsE);
								lpad->cd();
								char buf[500];
								sprintf(buf, "#pi^{-} #eta=%3.1f + PU %d",
										etaval, pu[ipu]);
							...
							if (ipu == 1 && iL > 0 && iL < nLayers - 2
									&& sigmaConst[ipu][4][iL] < minval) {
								bool ignore = false;
								for (unsigned r(0); r < lToRemove.size(); r++) {
									if (abs(
											static_cast<int>(iL)
													- static_cast<int>(lToRemove[r]))
											< 2)
										ignore = true;
								}
								if (!ignore) {
				...
				for (unsigned ipu(0); ipu < 1; ++ipu) {      //loop on pu
					...
					for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
						...
						grSlope[ipu][iSR]->SetMarkerColor(
								iSR > 3 ? 2 + iSR : 1 + iSR);
						grSlope[ipu][iSR]->SetLineColor(
								iSR > 3 ? 2 + iSR : 1 + iSR);
						llabel.str("");
						...
						grSlope2[ipu][iSR]->SetMarkerColor(
								iSR > 3 ? 2 + iSR : 1 + iSR);
						grSlope2[ipu][iSR]->SetLineColor(
								iSR > 3 ? 2 + iSR : 1 + iSR);
						llabel.str("");
						llabel << "grOffset_" << ipu << "_SR" << iSR;
						grOffset[ipu][iSR] =
								(TGraphErrors *) grSlope[ipu][iSR]->Clone(
										llabel.str().c_str());
						llabel.str("");
						...
						for (unsigned iL(0); iL < nGF; ++iL) {	//loop on layers
							grSlope2[ipu][iSR]->SetPoint(iL, iL,
									calib2[ipu][iL][iSR]);
							grSlope2[ipu][iSR]->SetPointError(iL, 0.0,
									calib2Err[ipu][iL][iSR]);

							grSlope[ipu][iSR]->SetPoint(iL, iL,
									calib[ipu][iL][iSR]);
							grSlope[ipu][iSR]->SetPointError(iL, 0.0,
									calibErr[ipu][iL][iSR]);

							grOffset[ipu][iSR]->SetPoint(iL, iL,
									offset[ipu][iL][iSR]);
							grOffset[ipu][iSR]->SetPointError(iL, 0.0,
				...
				if (!dovsE)
					grDummy->GetXaxis()->SetTitle("E_{T} (GeV)");
				else
					grDummy->GetXaxis()->SetTitle("E (GeV)");
				...
				for (unsigned ipu(1); ipu < nPu; ++ipu) {	//loop on pu
					if (pu[ipu] == 0)
						grDummy->SetMaximum(0.4);
					else
						grDummy->SetMaximum(0.16);
					for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
						if (pu[ipu] != 0 && iSR == nSR - 1)
							continue;
						...
						for (unsigned iL(0); iL < nGF; ++iL) {	//loop on lay
							...
							resoRecoFit[ipu][iL][iSR]->SetMarkerColor(
									iL % 9 + 1);
							resoRecoFit[ipu][iL][iSR]->SetMarkerStyle(
									iL / 9 + 20);
							...
							if (iL > 1)
								...
								label << "C_{global} = E<800?15:25";
							leglay->AddEntry(resoRecoFit[ipu][iL][iSR],
									label.str().c_str(), "P");
						}	//loop on lay
							//add ref pu=0 curve
						if (pu[ipu] != 0) {
							...
							leglay->AddEntry(resoRecoFit[1][0][iSR], "Ref pu=0",
									"L");
						}
						leglay->Draw("same");
						...
						mycReso->Update();
						std::ostringstream lsave;
						lsave << plotDir << "/Resolution_pu" << pu[ipu] << "_SR"
								<< iSR;
						if (dovsE)
							lsave << "_vsE";
						lsave << pSuffix;
				...
				for (unsigned iSR(0); iSR < nSR; ++iSR) {
					...
					for (unsigned ipu(0); ipu < ((nPu > 1) ? (nPu - 1) : nPu);
							++ipu) {	  //loop on pu
						double layval[nGF];
						...
						//layval[10-1] = -1;
						grStoch[ipu][iSR] = new TGraphErrors(nGF, layval,
								sigmaStoch[ipu + 1][iSR], layvalerr,
								sigmaStochErr[ipu + 1][iSR]);
						grConst[ipu][iSR] = new TGraphErrors(nGF, layval,
								sigmaConst[ipu + 1][iSR], layvalerr,
								sigmaConstErr[ipu + 1][iSR]);
						grNoise[ipu][iSR] = new TGraphErrors(nGF, layval,
								sigmaNoise[ipu + 1][iSR], layvalerr,
								sigmaNoiseErr[ipu + 1][iSR]);

						TGraphErrors *gr = 0;
						for (unsigned iP(0); iP < 3; ++iP) {
							gr = (iP == 0) ? grStoch[ipu][iSR] :
									(iP == 1) ?
											grConst[ipu][iSR] :
											grNoise[ipu][iSR];
							if (!gr)
							...
							gr->SetMinimum(0);
							if (iP < 2)
								gr->SetMaximum(iP == 0 ? 1.1 : 0.13);
							else
								gr->SetMaximum(5);
							if (iP == 0)
								gr->SetTitle(
										";Layer removed;sampling term (GeV^{#frac{1}{2}})");
							else if (iP == 1)
								gr->SetTitle(";Layer removed;constant term");
							else
								gr->SetTitle(";Layer removed;noise term (GeV)");
		...
		std::cout << " --min value " << minval << "found for layer " << minlayer
				<< ", using " << list[ic] << std::endl;
