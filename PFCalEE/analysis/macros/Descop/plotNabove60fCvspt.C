...
int plotNabove60fCvspt() { //main
	...
	const std::string path =
			"/afs/cern.ch/work/a/amagnan/PFCalEEAna//HGCalTime/gitV05-02-04/"
					+ particle + "/";
	...
	for (unsigned ie(0); ie < neta; ++ie) { //loop on version
		...
		lname << "myc_" << eta[ie];
		myc[ie] = new TCanvas(lname.str().c_str(), lname.str().c_str(), 1500,
				1000);
		myc[ie]->Divide(3, 2);
		lname.str("");
		lname << "myc1_" << eta[ie];
		myc1[ie] = new TCanvas(lname.str().c_str(), lname.str().c_str(), 1500,
				1000);
	}
	...
	for (unsigned iT(0); iT < nT; ++iT) {
		...
		for (unsigned il(0); il < nLim; ++il) { //loop on limits
			...
			label.str("");
			label << ";p_{T} (GeV);proba(n_{E>" << thresh[iT] << "fC}#geq"
					<< limit[il] << ")";
		...
		label.str("");
		if (!doresol)
			label << ";p_{T} (GeV);<n_{E>" << thresh[iT] << "fC}>";
		else if (!docm)
			label << ";p_{T} (GeV);<50/#sqrt{n_{E>" << thresh[iT]
					<< "fC}}> (ps)";
		else
			label << ";p_{T} (GeV);<50#times c/#sqrt{n_{E>" << thresh[iT]
					<< "fC}}> (cm)";
		...
		TLegend *leg;
		if (!doresol)
			leg = new TLegend(0.68, 0.15, 0.92, 0.4);
		else
			leg = new TLegend(0.68, 0.7, 0.92, 0.94);
		leg->SetFillColor(10);
		for (unsigned ieta(0); ieta < neta; ++ieta) { //loop on eta
			...
			for (unsigned iv(0); iv < nV; ++iv) { //loop on version
				for (unsigned il(0); il < nLim; ++il) { //loop on limits
					std::ostringstream lname;
					lname << "grv" << v[iv] << "_eta" << eta[ieta] << "_lim"
							<< limit[il];
				...
				for (unsigned ipt(0); ipt < npt; ++ipt) { //loop on pt
					...
					fname << path << "v" << v[iv] << "_et" << pt[ipt] << "_eta"
							<< eta[ieta];
					if (eta[ieta] == 2)
						fname << ".0";
					if (thresh[iT] != 60)
						fname << "_thresh" << threshMip[iT];
					if (ieta == 4 && (pt[ipt] == 50 || pt[ipt] == 70))
						fname << "/NAbove" << threshMip[iT] << "_old.root";
					else
						fname << "/NAbove" << threshMip[iT] << ".root";
					...
					TH1F *hprob = new TH1F("hprob", fname.str().c_str(), nbins,
							0, nbins);
					...
					if (!doresol)
						...
						tree->Draw("50./sqrt(nover)>>hprob", "nover>=1");
					else
						tree->Draw("50.*0.03/sqrt(nover)>>hprob", "nover>=1");
					hprob->GetXaxis()->SetRangeUser(0, 1000);
					unsigned nentries = hprob->GetEntries();
					std::cout << "v " << v[iv] << " et " << pt[ipt] << " eta "
							<< eta[ieta];
					if (nentries > 0) {
						std::cout << " nentries " << nentries << " mean "
								<< hprob->GetMean() << " "
								<< hprob->GetMeanError();
						...
						double yval = 0;
						if (iv == 0) {
							if (grMean[0][ieta]->GetN() > 0)
								grMean[0][ieta]->GetPoint(
										grMean[iv][ieta]->GetN(), xval, yval);
							else
								yval = hprob->GetMean();
						}
						grMean[iv][ieta]->SetPoint(grMean[iv][ieta]->GetN(),
								pt[ipt], hprob->GetMean() / yval);
						grMean[iv][ieta]->SetPointError(
								grMean[iv][ieta]->GetN() - 1, 0,
								hprob->GetMeanError() / yval);
						//}
						//else if (!docm) {
					...
					for (unsigned il(0); il < nLim; ++il) {	    //loop on limits
						double error = 0;
						double integral = hprob->IntegralAndError(limit[il] + 1,
								nbins + 1, error);

						std::cout << " entries=" << hprob->GetEntries()
								<< " integral(" << limit[il] + 1 << ",N) "
								<< integral << "+/-" << error << std::endl;
						gr[iv][ieta][il]->SetPoint(gr[iv][ieta][il]->GetN(),
								pt[ipt], integral);
						gr[iv][ieta][il]->SetPointError(
								gr[iv][ieta][il]->GetN() - 1, 0, error);
					}
					hprob->Delete();
					ftmp->Close();
				}	    //loop on pt
				for (unsigned il(0); il < nLim; ++il) {	    //loop on limits
					myc[ieta]->cd(il + 1);
					std::cout << "v " << v[iv] << " eta " << eta[ieta]
							<< " limit " << limit[il] << " npoints = "
							<< gr[iv][ieta][il]->GetN() << " nmean = "
							<< grMean[iv][ieta]->GetN() << std::endl;
			...
			for (unsigned il(0); il < nLim; ++il) {	    //loop on limits
				...
				if (isPi)
					...
					sprintf(buf, "#gamma, #eta = %3.2f", eta[ieta]);
				...
				if (!doresol && !docm) {
					...
					lsave << "SummaryNabove" << thresh[iT] << "fC_" << particle
							<< "_vspt_eta" << eta[ieta] << "_limit"
							<< limit[il];
					mycl->Update();
			...
			lsave.str("");
			if (!doresol)
				lsave << "SummaryNMeanabove" << thresh[iT] << "fC_" << particle
						<< "_vspt_eta" << eta[ieta];
			else if (!docm)
				lsave << "SummaryResolabove" << thresh[iT] << "fC_" << particle
						<< "_vspt_eta" << eta[ieta];
			else
				lsave << "SummaryResolAtVertexabove" << thresh[iT] << "fC_"
						<< particle << "_vspt_eta" << eta[ieta];
