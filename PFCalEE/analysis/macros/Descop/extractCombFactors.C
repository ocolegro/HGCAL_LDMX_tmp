...
int extractCombFactors() {
	...
	TString plotDir =
			"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalDescop/gitV04-02-02/version25/pi-/";
	...
	for (unsigned iE(0); iE < nGenEnAll; ++iE) { //loop on E
		...
		linputStr << plotDir;
		linputStr << "pion_gc3-50_reso" << model.str() << "_et" << genEnAll[iE]
				<< ".root";
		...
		if (!inputFile[iE]) {
			...
			TH2F *hbhfh = (TH2F*) (gPad->GetPrimitive("htemp"))->Clone(
					lName.str().c_str());
			hbhfh->ProfileX();
			lName << "_pfx";
	...
	for (unsigned iE(0); iE < nGenEnAll; ++iE) {      //loop on E
		...
		TH2F *hhvse = (TH2F*) (gPad->GetPrimitive("htemp"))->Clone(
				lName.str().c_str());
		hhvse->Draw("colz");
		hhvse->ProfileX();
		lName << "_pfx";
	...
	for (unsigned iE(0); iE < nGenEnAll; ++iE) {      //loop on E
		...
		TH1F *hetot = (TH1F*) (gPad->GetPrimitive("htemp"))->Clone(
				lName.str().c_str());
		hetot->Fit("gaus", "", "same");
	...
	for (unsigned iE(0); iE < nGenEnAll; ++iE) {  //loop on E
		...
		for (unsigned ic(0); ic < nGF; ++ic) {
			...
			lName << "((EECAL-" << ECALoffset << ")/" << ECALslope
					<< "+((EFHCAL-" << FHtoEoffset << ")/" << FHtoEslope
					<< "+(EBHCAL-" << BHtoEoffset << ")/(" << BHtoEslope << "*"
					<< BHoverFH << "))/" << HoverE << "):globalC_" << list[ic];
			ltree[iE]->Draw(lName.str().c_str(), "", "colz");
			lName.str("");
			lName << "EvsC_" << genEnAll[iE];
			TH2F *hevsc = (TH2F*) (gPad->GetPrimitive("htemp"))->Clone(
					lName.str().c_str());
