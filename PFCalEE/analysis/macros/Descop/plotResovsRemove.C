...
int plotResovsRemove() {
	...
	int colorSel[nRemove] = { kBlack, kBlue + 2, kBlue, kBlue - 2, kCyan, kGreen
			+ 2, kGreen, kGreen - 2, kYellow + 2, kOrange + 2, kPink, kRed - 2,
			kRed };
	int color[nRemove] = { kGray, kBlue + 1, kBlue - 1, kBlue - 3, kCyan - 1,
			kGreen + 1, kGreen - 1, kGreen - 3, kYellow + 1, kOrange + 1, kPink
					- 1, kRed - 3, kRed - 1 };
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		...
		lName << "mycC" << iC;
		mycC[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		mycC[iC]->Divide(2, 2);
		lName.str("");
		lName << "mycR" << iC;
		mycR[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		lName.str("");
		lName << "mycF" << iC;
		mycF[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		lName.str("");
		lName << "mycS" << iC;
		mycS[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		mycS[iC]->Divide(2, 2);
	}
	...
	for (unsigned r(0); r < nRemove; ++r) { //loop on nremove
		...
		for (unsigned l(0); l < nL; ++l) { //loop on layers
			...
			if (selected)
				grCal[r][l]->SetLineWidth(3);
			if (selected)
				grCal[r][l]->Draw(first ? "APEL" : "PEL");
			lname.str("");
			lname << "calibRecoDelta" << SR << "lay" << l << "pu0";
			grDelta[r][l] = (TGraphErrors*) gDirectory->Get(
					lname.str().c_str());
			mycC[0]->cd(3);
			...
			if (selected)
				grDelta[r][l]->SetLineWidth(3);
			if (selected)
				grDelta[r][l]->Draw(first ? "APEL" : "PEL");
			lname.str("");
			...
			if (selected) {
				lname.str("");
				lname << "reso_" << r;
				fit[r] = new TF1(lname.str().c_str(),
						"sqrt([0]*[0]/x+[1]*[1]+[2]*[2]/(x*x))", 0, 1000);
				fit[r]->SetParameters(0.212, 0.009, 0.04);
				fit[r]->FixParameter(2, 0.04);
				grRes[r][l]->Fit(fit[r], "BIME0Q");
				std::cout << "Fit" << r << " " << fit[r]->GetParameter(0) << " "
						<< fit[r]->GetParameter(1) << std::endl;
		...
		lname.str("");
		lname << "resoRef_" << r;
		fitRef[r] = new TF1(lname.str().c_str(),
				"[3]*sqrt([0]*[0]/x+[1]*[1]+[2]*[2]/(x*x))", 0, 1000);
	...
	for (unsigned ir(0); ir < 3; ++ir) {
		...
		lname.str("");
		lname << "fitTP_" << ir;
		fittp[ir] = new TF1(lname.str().c_str(),
				"sqrt([0]*[0]/x+[1]*[1]+[2]*[2]/(x*x))", 0, 1000);
		...
		fittp[ir]->Draw("same");
		std::cout << ir << " " << fittp[ir]->GetParameter(0) << " "
				<< fittp[ir]->GetParameter(1) << " "
				<< fittp[ir]->GetParameter(2) << std::endl;
		std::cout << ir << " " << fittp[ir]->GetParError(0) << " "
				<< fittp[ir]->GetParError(1) << " " << fittp[ir]->GetParError(2)
				<< std::endl;
	}
