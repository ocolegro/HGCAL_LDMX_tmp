...
TPad* plotCalibration(TGraphErrors *gr, TPad *pad, bool doRatio,
	...
	if (dovsE)
		gr->Fit(fitFunc, "QIME", "same", 0, 200);
	else
			...
			for (unsigned iS(0); iS < nS; ++iS) {    //loop on scenarios
				...
				for (unsigned ipu(0); ipu < nPu; ++ipu) {    //loop on pu
					...
					for (unsigned iE(0); iE < nGenEnAll; ++iE) {
						...
						linputStr << plotDir;
						linputStr << "EMcalibration" << model << "_"
								<< genEnAll[iE] << ".root";
						...
						if (!inputFile[iE]) {
							...
							ltree[ipu][iE] = (TTree*) gDirectory->Get(
									linputStr.str().c_str());
					...
					if (ipu == 0) {
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							upper->cd();
							char buf[500];
							sprintf(buf, "#gamma #eta=%3.1f + PU %d", etaval,
									pu[ipu]);
							TLatex lat;
							lat.SetTextSize(0.1);
