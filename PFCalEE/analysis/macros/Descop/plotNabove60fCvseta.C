...
int plotNabove60fCvseta() { //main
	...
	for (unsigned iv(0); iv < nV; ++iv) { //loop on version
		...
		lname << "myc_" << v[iv];
		myc[iv] = new TCanvas(lname.str().c_str(), lname.str().c_str(), 1500,
				1000);
		lname.str("");
		lname << "myc1_" << v[iv];
		myc1[iv] = new TCanvas(lname.str().c_str(), lname.str().c_str(), 1500,
				1000);
	}
	...
	TLegend *leg;
	if (isPi)
		leg = new TLegend(0.72, 0.43, 0.87, 0.93);
	else
		leg = new TLegend(0.78, 0.11, 0.94, 0.54);
	leg->SetFillColor(10);
	for (unsigned iv(0); iv < nV; ++iv) {    //loop on version
		myc[iv]->cd();
		grDummy->Draw("AL");
		...
		for (unsigned ipt(0); ipt < npt; ++ipt) {    //loop on pt
			...
			for (unsigned ieta(0); ieta < neta; ++ieta) {    //loop on pt
				...
				TH1F *hprob = new TH1F("hprob", ";n(E>26 mips);proba", 30, 0,
						30);
				...
				if (nentries > 0) {
					std::cout << " mean " << hprob->GetMean() << " "
							<< hprob->GetMeanError();
					grMean[iv][ipt]->SetPoint(grMean[iv][ipt]->GetN(), neweta,
							hprob->GetMean());
					grMean[iv][ipt]->SetPointError(grMean[iv][ipt]->GetN() - 1,
							0, hprob->GetMeanError());
				...
				std::cout << " entries=" << hprob->GetEntries() << " integral("
						<< limit + 1 << ",N) " << integral << "+/-" << error
						<< std::endl;
			...
			myc[iv]->cd();
			std::cout << "v " << v[iv] << " pt " << pt[ipt] << " npoints = "
					<< gr[iv][ipt]->GetN() << " nmean = "
					<< grMean[iv][ipt]->GetN() << std::endl;
			...
			if (iv == 0)
				leg->AddEntry(gr[iv][ipt], label.str().c_str(), "P");
		...
		lsave << "SummaryNabove60fC_" << particle << "_limit" << limit
				<< "_vseta_version" << v[iv] << ".pdf";
		...
		myc1[iv]->Update();
		lsave.str("");
		lsave << "SummaryNMeanabove60fC_" << particle << "_vseta_version"
				<< v[iv] << ".pdf";
