			...
			for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
				...
				for (unsigned ipu(0); ipu < nPu; ++ipu) {  //loop on pu
					...
					for (unsigned iE(0); iE < nGenEn; ++iE) {
						...
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							FitResult lres;
							if (fitEnergy(p_Ereco[iE][iSR], lpad, unit, lres,
									iSR) != 0)
								return 1;
							lpad->cd();
							char buf[500];
							...
							if (ipu < 1) {
								...
								if (!dovsE)
									calibRecoFit[iSR]->SetPoint(np, lres.mean,
											genEn[iE]);
								else
									calibRecoFit[iSR]->SetPoint(np, lres.mean,
											E(genEn[iE], eta));
							...
							double errFit = reso
									* sqrt(
											pow(lres.sigmaerr / lres.sigma, 2)
													+ pow(
															lres.meanerr
																	/ lres.mean,
															2));
						...
						mycE[iE]->Update();
						mycE[iE]->Print(
								(saveName.str().c_str() + pSuffix) + ".pdf");
					...
					if (ipu == 2) {
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							TPad *upper = plotCalibration(calibRecoFit[iSR],
									...
									calib[ipu][iL][iSR], calibErr[ipu][iL][iSR],
									offset[ipu][iL][iSR],
									offsetErr[ipu][iL][iSR], eta, dovsE);
							upper->cd();
							char buf[500];
							sprintf(buf, "#pi^{-} #eta=%3.1f + PU %d", etaval,
									pu[ipu]);
							TLatex lat;
							lat.SetTextSize(0.1);
						...
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							TPad *upper = plotCalibrationRev(calibRecoFit[iSR],
									lpad, true, calibRecoDelta[iSR], unit,
									calib2[ipu][iL][iSR],
					...
					if (ipu == 2) {
						...
						for (unsigned iSR(0); iSR < nSR; ++iSR) {//loop on signal region
							...
							bool success = plotResolution(
									resoRecoFit[ipu][iL][iSR], lpad, ipu, eta,
				...
				for (unsigned iSR(0); iSR < nSR; ++iSR) {
					...
					for (unsigned ipu(0); ipu < ((nPu > 1) ? (nPu - 1) : nPu);
						...
						//layval[10-1] = -1;
						grStoch[ipu][iSR] = new TGraphErrors(nLayers, layval,
								sigmaStoch[ipu + 1][iSR], layvalerr,
								sigmaStochErr[ipu + 1][iSR]);
						grConst[ipu][iSR] = new TGraphErrors(nLayers, layval,
								sigmaConst[ipu + 1][iSR], layvalerr,
								sigmaConstErr[ipu + 1][iSR]);
						grNoise[ipu][iSR] = new TGraphErrors(nLayers, layval,
