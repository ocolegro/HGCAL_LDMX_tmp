...
int EvtDisplay() {    //main  
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		lName.str("");
		lName << "myc" << iC;
		myc[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1400,
				1000);
		//myc[iC]->Divide(2,1);
	}
	...
	inputDir
			<< "/afs/cern.ch/work/a/amagnan/public/HGCalGeant4/git_V00-03-06/version_33/model_2/";
	inputDir << scenario;
	...
	if (!inputFile) {
		std::cout << " -- Error, input file " << inputDir.str()
	...
	for (unsigned ievt(0); ievt < 2; ++ievt) {  //loop on events
		...
		unsigned nX = (maxX[ievt] - minX[ievt]) / 10., nY = (maxY[ievt]
				- minY[ievt]) / 10.;
		lName.str("");
		lName << "xyz_" << ievt;

		p_xyz[ievt] = new TH3F(lName.str().c_str(), ";z(mm);x(mm);y(mm)", nZ,
				minZ, maxZ, nX, minX[ievt], maxX[ievt], nY, minY[ievt],
				maxY[ievt]);
		...
		lvar
				<< "HGCSSRecoHitVec.ypos_:HGCSSRecoHitVec.xpos_:HGCSSRecoHitVec.zpos_>>"
				<< lName.str();
		...
		std::cout << " --bins, min and max of hist = "
				<< p_xyz[ievt]->GetNbinsY() << " " << p_xyz[ievt]->GetMinimum()
				<< " " << p_xyz[ievt]->GetMaximum() << std::endl;
		...
		for (unsigned iS(0); iS < nSlices; ++iS) {
			...
			p_slice[iS] = new TH3F(lName.str().c_str(), ";z(mm);x(mm);y(mm)",
					p_xyz[ievt]->GetNbinsX(),
					p_xyz[ievt]->GetXaxis()->GetBinLowEdge(1),
					p_xyz[ievt]->GetXaxis()->GetBinLowEdge(
							p_xyz[ievt]->GetNbinsX() + 1),
					p_xyz[ievt]->GetNbinsY(),
					p_xyz[ievt]->GetYaxis()->GetBinLowEdge(1),
					p_xyz[ievt]->GetYaxis()->GetBinLowEdge(
							p_xyz[ievt]->GetNbinsY() + 1),
					p_xyz[ievt]->GetNbinsZ(),
					p_xyz[ievt]->GetZaxis()->GetBinLowEdge(1),
					p_xyz[ievt]->GetZaxis()->GetBinLowEdge(
							p_xyz[ievt]->GetNbinsZ() + 1));
		...
		std::cout << " --min and max by hand = " << lmin << " " << lmax
				<< std::endl;
		...
		myc[ievt]->Update();
		saveName.str("");
		saveName << plotDir << "/evtDisplay_evt" << event[ievt] << "_mipThresh"
				<< mipThresh;
