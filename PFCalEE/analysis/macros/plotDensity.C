...
double getBinWidth(const double etaMin, const double etaMax, std::string etaStr,
		const double xLaymm) {
	double eta0 = 0;
	if (etaStr.find("eta20") != etaStr.npos)
		eta0 = 2.0;
	else if (etaStr.find("eta25") != etaStr.npos)
		eta0 = 2.5;
	else if (etaStr.find("eta30") != etaStr.npos)
		eta0 = 3.0;
	else if (etaStr.find("eta35") != etaStr.npos)
		eta0 = 3.5;
	else {
		std::cout
				<< " ERROR: new eta point ! Please implement value to convert y position... Exiting..."
				<< std::endl;
		exit(1);
	}
...
int plotDensity() {  //main
	...
	const unsigned nS = 4;
	std::string scenario[nS] = { "scenario_0/PedroPU/eta20/",
			"scenario_0/PedroPU/eta25/", "scenario_0/PedroPU/eta30/",
		...
		for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
			...
			if (!inputFile) {
				std::cout << " -- Error, input file " << plotDir
						<< "/CalibHistos.root cannot be opened. Exiting..."
						<< std::endl;
				return 1;
			}
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				if (scenario[iS].find("Pedro") != scenario[iS].npos)
					genEn[iE] = iE;
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {
					...
					p_Edensity[iV][iS][iE][iL] = (TH1F*) (gDirectory->Get(
							lName.str().c_str())->Clone());
					if (!p_Edensity[iV][iS][iE][iL]) {
						std::cout
								<< " -- ERROR, pointer for histogram Edensity is null for layer: "
								<< iL << ". Exiting..." << std::endl;
						return 1;
					}
	...
	for (unsigned iV(0); iV < nV; ++iV) {      //loop on versions
		...
		TH2F *p_allEtaDensityLayervsEta = new TH2F("p_allEtaDensityLayervsEta",
				";#eta;Layer;E density (MIP/cm^{2})", 61, 1.88, 4.32, 30, 0,
				30);
		TH2F *p_allEtaDensityEtavsLayer = new TH2F("p_allEtaDensityEtavsLayer",
				";Layer;#eta;E density (MIP/cm^{2})", 30, 0, 30, 61, 1.88,
				4.32);
		p_allEtaDensityLayervsEta->Sumw2();
		p_allEtaDensityEtavsLayer->Sumw2();

		TH1F *p_etavsLayer[4];
		p_etavsLayer[0] = new TH1F("p_eta20vsLayer",
				";layer;E density (MIP/cm^{2})", 30, 0, 30);
		p_etavsLayer[1] = new TH1F("p_eta25vsLayer",
				";layer;E density (MIP/cm^{2})", 30, 0, 30);
		p_etavsLayer[2] = new TH1F("p_eta30vsLayer",
				";layer;E density (MIP/cm^{2})", 30, 0, 30);
		p_etavsLayer[3] = new TH1F("p_eta35vsLayer",
				";layer;E density (MIP/cm^{2})", 30, 0, 30);
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {      //loop on layers
			...
			p_allEtaDensity[iL] = new TH1F(lName.str().c_str(), ";#eta", 130,
					1.8, 4.4);
			...
			for (unsigned iE(isPU ? 0 : eSignal);
					iE < (isPU ? nGenEn : eSignal + 1); ++iE) { //loop on energies
				//unsigned iE = 5; 
				for (unsigned iS(0); iS < nS; ++iS) {	//loop on scenarios
					...
					for (int iB(1);
							iB < p_Edensity[iV][iS][iE][iL]->GetNbinsX() + 1;
							++iB) {	  //loop on bins
						double etaMin =
								p_Edensity[iV][iS][iE][iL]->GetBinLowEdge(iB);
						double etaMax =
								p_Edensity[iV][iS][iE][iL]->GetBinLowEdge(
										iB + 1);
				...
				for (unsigned iS(0); iS < nS; ++iS) {	  //loop on scenarios
					...
					int binEta =
							p_Edensity[iV][iS][iE][iL]->GetXaxis()->FindBin(
									eta[iS]);
					...
					for (int iB(1);
						...
						double etaMax =
								p_Edensity[iV][iS][iE][iL]->GetBinLowEdge(
										iB + 1);
						double etaCenter =
								p_Edensity[iV][iS][iE][iL]->GetBinCenter(iB);

						if ((iS < nS - 1 && etaCenter > minEta[iS + 1])
								|| (iS > 0 && etaCenter < maxEta[iS - 1]))
							overlap = true;

						double norm = (isPU ? 1 / 1000. : 1 / 100.) * 1
								/ signalRegionInX * 1
								/ getBinWidth(etaMin, etaMax, scenario[iS],
										xLay[iL]);
						double density =
								p_Edensity[iV][iS][iE][iL]->GetBinContent(iB)
										* norm;
						if (overlap)
							density = density / 2.;
						//std::cout << density << std::endl;
						p_allEtaDensity[iL]->Fill(etaCenter, density / Emip);
						p_allEtaDensityLayervsEta->Fill(etaCenter, iL,
								density / Emip);
						p_allEtaDensityEtavsLayer->Fill(iL, etaCenter,
								density / Emip);

						if (iB == binEta)
							p_etavsLayer[iS]->Fill(iL, density / Emip);
		...
		p_allEtaDensityLayervsEta->GetZaxis()->SetRangeUser(isPU ? 0.1 : 0.01,
				isPU ? 250 : 300);
		...
		p_allEtaDensityEtavsLayer->GetZaxis()->SetRangeUser(isPU ? 0.1 : 0.01,
				isPU ? 250 : 300);
		...
		for (unsigned iS(0); iS < nS; ++iS) {    //loop on scenarios
			...
			for (int iB(1); iB < p_etavsLayer[iS]->GetNbinsX() + 1; ++iB) {
				std::cout << "-- Bin " << iB << " "
						<< p_etavsLayer[iS]->GetBinContent(iB) << std::endl;
			}

			saveName.str("");
			saveName << "../PLOTS/version_" << version[iV]
					<< "/DensityPerLayer_eta" << eta[iS];
			myc[iS]->Update();
