...
int plotMipHistosForMu() {
	...
	outFilePath
			<< "/afs/cern.ch/work/a/amagnan/PFCalEEAna/PLOTS/gitV00-02-12/version12/MinBias/MipStudy_eta"
			<< eta - deta << "_" << eta + deta << "_MinBias5_5.root";
	...
	TH1F *hitSpectrumAll = new TH1F("hitSpectrumAll",
			";E_{1#times1cm^{2}}(mips);N_{cells}", 100, 0, 5);
	...
	for (unsigned il(0); il < nLayers; ++il) {
		...
		hitSpectrum[il] = new TH1F(label.str().c_str(),
				";E_{1#times1cm^{2}}(mips);N_{cells}", 100, 0, 5);
		lvar.str("");
	...
	for (unsigned il(0); il < nLayers; ++il) {
		...
		if (hitSpectrum[il]->GetEntries() == 0)
			continue;
		TF1 *fit = 0;
		double mpc = 0;
		double minfit = 0.7;
