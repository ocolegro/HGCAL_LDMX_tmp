...
int plotMomentumShowerMax() {
	...
	if (!outputFile) {
		std::cout << " -- Error, output file " << lstr.str()
	...
	TH2F *p_volumes = new TH2F("p_volumes", ";entry volume;exit volume", 5, 0,
			5, 5, 0, 5);
	...
	TH1F *p_logmomentum_p = new TH1F("p_logmomentum_p",
			";log(p) (log(GeV));particles", 100, -2, 2);
	TH1F *p_momentum_p = new TH1F("p_momentum_p", ";p (GeV);particles", 1000, 0,
			100);
	TH1F *p_time_p = new TH1F("p_time_p", ";time (ns);particles", 1000, 0,
			1000);
	TH2F *p_timevslogp_p = new TH2F("p_timevslogp_p",
			";log(p) (log(GeV));time (ns);particles", 100, -2, 2, 1000, 0,
			1000);
	TH2F *p_timevsp_p = new TH2F("p_timevsp_p", ";p (GeV);time (ns);particles",
			1000, 0, 100, 1000, 0, 1000);

	outputFile->cd();
	outputFile->cd("neutrons");
	TH1F *p_logmomentum_n = new TH1F("p_logmomentum_n",
			";log(p) (log(GeV));particles", 100, -2, 2);
	TH1F *p_momentum_n = new TH1F("p_momentum_n", ";p (GeV);particles", 1000, 0,
			100);
	TH1F *p_time_n = new TH1F("p_time_n", ";time (ns);particles", 1000, 0,
			1000);
	TH2F *p_timevslogp_n = new TH2F("p_timevslogp_n",
			";log(p) (log(GeV));time (ns);particles", 100, -2, 2, 1000, 0,
			1000);
	TH2F *p_timevsp_n = new TH2F("p_timevsp_n", ";p (GeV);time (ns);particles",
			1000, 0, 100, 1000, 0, 1000);

	outputFile->cd();
	outputFile->cd("pipm");
	TH1F *p_logmomentum_pi = new TH1F("p_logmomentum_pi",
			";log(p) (log(GeV));particles", 100, -2, 2);
	TH1F *p_momentum_pi = new TH1F("p_momentum_pi", ";p (GeV);particles", 1000,
			0, 100);
	TH1F *p_time_pi = new TH1F("p_time_pi", ";time (ns);particles", 1000, 0,
			1000);
	TH2F *p_timevslogp_pi = new TH2F("p_timevslogp_pi",
			";log(p) (log(GeV));time (ns);particles", 100, -2, 2, 1000, 0,
			1000);
	TH2F *p_timevsp_pi = new TH2F("p_timevsp_pi",
			";p (GeV);time (ns);particles", 1000, 0, 100, 1000, 0, 1000);

	outputFile->cd();
	outputFile->cd("Si");
	TH1F *p_logmomentum_Si = new TH1F("p_logmomentum_Si",
			";log(p) (log(GeV));particles", 100, -5, 2);
	TH1F *p_momentum_Si = new TH1F("p_momentum_Si", ";p (GeV);particles", 1000,
			0, 1);
	TH1F *p_time_Si = new TH1F("p_time_Si", ";time (ns);particles", 1000, 0,
			1000);
	TH2F *p_timevslogp_Si = new TH2F("p_timevslogp_Si",
			";log(p) (log(GeV));time (ns);particles", 100, -5, 2, 1000, 0,
			1000);
	TH2F *p_timevsp_Si = new TH2F("p_timevsp_Si",
			";p (GeV);time (ns);particles", 1000, 0, 1, 1000, 0, 1000);
	TH1F *p_ionisingE = new TH1F("p_ionisingE", ";E ionising (MeV);ions", 2000,
			0, 2);
	TH2F *p_ionisingEvspid = new TH2F("p_ionisingEvspid",
			";Z+(A-2Z)*0.1;E ionising (MeV);ions", 300, 0, 30, 5000, 0, 5);
	TH2F *p_ionAvsZ = new TH2F("p_ionAvsZ", ";Z;A;ions", 100, 0, 100, 200, 0,
			200);
	TH2F *p_trkStatus = new TH2F("p_trkStatus", ";process name;trk status;ions",
			21, 0, 21, 6, 0, 6);
	...
	TH1F *p_stepLength = new TH1F("p_stepLength", ";step length (mm);ions", 101,
			0, 0.101);
	...
	for (unsigned iF(0); iF < nFiles; ++iF) {
		...
		label
				<< "/afs/cern.ch/work/a/amagnan/public/HGCalGeant4/git_V00-03-03/version_25/model_2/minbiasAllSi/BON/run_"
				<< iF << "/layer30_version25_model2_BON_run" << iF << ".dat";
		input.open(label.str().c_str());
		if (!input.is_open()) {
			std::cout << " Cannot open input file " << label.str()
					<< ". Continue..." << std::endl;
			continue;
		} else
			std::cout << " Successfully opened file " << label.str()
					<< std::endl;
		...
		while (!input.eof()) {
			...
			if (pdgid > 1000000000)
				iss >> trkS >> stepl >> totE >> nonIoE >> proc >> kinE;
			//return 1;
			if (evtid < 1000 && pdgid != 0) {
				...
				if (p == 0) {
					if (abs(pdgid) > 1000000000) {
						...
						unsigned A = (abs(pdgid)
								- static_cast<unsigned>(abs(pdgid) / 1000.)
										* 1000) / 10.;
						unsigned Z = (abs(pdgid) / 1000.
								- static_cast<unsigned>(abs(pdgid) / 1000000.)
										* 1000) / 10.;
						p_ionisingEvspid->Fill(Z + (A - 2. * Z) * 0.1,
								totE - nonIoE);
						p_ionAvsZ->Fill(Z, A);
						int binx = p_trkStatus->GetXaxis()->FindBin(
								proc.c_str());
						int biny = p_trkStatus->GetYaxis()->FindBin(trkS);
						p_trkStatus->SetBinContent(binx, biny,
								p_trkStatus->GetBinContent(binx, biny) + 1);
						p_stepLength->Fill(stepl);
						insertProc = procnameMap.insert(
								std::pair<std::string, unsigned>(proc, 1));
						if (!insertProc.second)
							insertProc.first->second += 1;
					}
					continue;
				}
				int binx = p_volumes->GetXaxis()->FindBin(volStart.c_str());
				int biny = p_volumes->GetYaxis()->FindBin(volEnd.c_str());
				p_volumes->SetBinContent(binx, biny,
						p_volumes->GetBinContent(binx, biny) + 1);

				if (siLayer == 0 && !(binx == 2 && biny == 2)
						&& !(binx == 2 && biny == 3)
						&& !(binx == 2 && biny == 1))
					continue;
				if (siLayer == 1 && !(binx == 3 && biny == 3)
						&& !(binx == 3 && biny == 4)
						&& !(binx == 3 && biny == 2))
					continue;
				if (siLayer == 2 && !(binx == 4 && biny == 4)
						&& !(binx == 4 && biny == 5)
						&& !(binx == 4 && biny == 3))
					continue;

				isInserted = pdgIdMap.insert(
						std::pair<int, unsigned>(pdgid, 1));
				...
				if (abs(pdgid) == 2212) {
					...
					p_ionisingE->Fill(totE - nonIoE);
					unsigned A = (abs(pdgid)
							- static_cast<unsigned>(abs(pdgid) / 1000.) * 1000)
							/ 10.;
					unsigned Z = (abs(pdgid) / 1000.
							- static_cast<unsigned>(abs(pdgid) / 1000000.)
									* 1000) / 10.;
	...
	part.push_back("#pi^{0}");
	if (siLayer == 1 || siLayer == 3)
		part.push_back("#rho^{0}");
	...
	for (; iter != pdgIdMap.end(); ++iter) {
		if (i < part.size())
			std::cout << i << " " << iter->first << " " << part[i] << " "
					<< iter->second * 1. / nEvts << std::endl;
		else
	...
	for (unsigned k = 0; k < gr->GetN(); k++) {
		if (k < part.size())
			...
			std::cout << " Warning, update particle list !" << std::endl;
	}
