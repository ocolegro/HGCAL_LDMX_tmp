...
int plotMomentumMuonsBHCAL() {
	...
	for (unsigned iE(0); iE < nE; ++iE) { //loop on energy points
		...
		TH1F *p_logmomentum = new TH1F("p_logmomentum",
				";log(p) (log(GeV));particles", 100, 1, 3);
		TH1F *p_momentum = new TH1F("p_momentum", ";p (GeV);particles", 5000, 0,
				500);
		...
		for (unsigned iF(0); iF < nFiles; ++iF) {
			...
			label
					<< "/afs/cern.ch/work/a/amagnan/public/HGCalMuons/git_V06b-03-05/version_33/model_2/mu-/BON//et_"
					<< et[iE] << "/eta_" << eta << "00/run_" << iF
					<< "/momentum_list_layer51.dat";
			...
			while (!input.eof()) {
				...
				if (event.find("Event") != event.npos) {
					std::istringstream(event.substr(6, event.npos - 6))
							>> evtid;
				...
				iss >> volStart >> volEnd >> trkid >> pdgid >> time >> x >> y
						>> z >> px >> py >> pz;
