...
int compareSquareHexa() { //main
	...
	fin[0] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalJack/gitV06-03-04/version100/model3/gamma/eta20_et20_pu0_IC3_Si2.root");
	fin[1] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalHexa/gittestHexa/version100/model3/gamma/eta20_et20_pu0_IC3_Si2.root");
	...
	for (unsigned iF(0); iF < nF; ++iF) { //loop on files
		...
		if (!tree[iF]) {
			std::cout << " Tree not found for file " << fin[iF]->GetName()
					<< std::endl;
			return 1;
		}
		lname.str("");
		lname << "Etot" << label[iF];
		Etot[iF] = new TH1F(lname.str().c_str(), ";Etot (mips);showers", 100,
				6000, 9500);
		...
		lname.str("");
		lname << "ESR2" << label[iF];
		ESR2[iF] = new TH1F(lname.str().c_str(), ";ESR2 (mips);showers", 100,
				4700, 7100);
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			if (iL == 0)
				...
				lvar << "+" << "absweight_" << iL << "*energy_" << iL << "_SR"
						<< iSR;
		}
		lvar << ">>" << lname.str();
		tree[iF]->Draw(lvar.str().c_str());
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			...
			E1[iF][iL] = new TH1F(lname.str().c_str(), ";ESR0 (mips);showers",
					200, 0, 500);
			...
			E1over7[iF][iL] = new TH1F(lname.str().c_str(),
					";ESR0/ESR2;showers", 50, 0, 1.);
			...
			for (unsigned iS(iL); iS < nLayers; ++iS) {
				lvar.str("");
				lvar << "energy_" << iS << "_SR0/energy_" << iS << "_SR2>>+"
						<< lname.str();
				lcut.str("");
				lcut << "energy_" << iS << "_SR0 > 5";
				if (iS > 0)
					lcut << " && energy_" << iS - 1 << "_SR0 < 5";
				tree[iF]->Draw(lvar.str().c_str(), lcut.str().c_str());
			}

			lname.str("");
			lname << "E7overtot" << label[iF] << "_" << iL;
			E7overtot[iF][iL] = new TH1F(lname.str().c_str(),
					";ESR2/Etot;showers", 50, 0, 1.);
			...
			for (unsigned iS(iL); iS < nLayers; ++iS) {
				lvar.str("");
				lvar << "energy_" << iS << "_SR2/energy_" << iS << "_SR5>>+"
						<< lname.str();
				lcut.str("");
				lcut << "energy_" << iS << "_SR0 > 5";
				if (iS > 0)
			...
			EmaxOut7[iF][iL] = new TH1F(lname.str().c_str(),
					";max Ehit outside SR2 (mips);showers", 100, 0, 150.);
			...
			transverseCor[iF][iL] = new TH2F(lname.str().c_str(),
					";ESR0/ESR2;ESR2/Etot;showers", 50, 0., 1., 50,
					iL < 14 ? 0.5 : 0, 1.);
			...
			for (unsigned iS(iL); iS < nLayers; ++iS) {
				lvar.str("");
				lvar << "energy_" << iS << "_SR2/energy_" << iS
						<< "_SR5:energy_" << iS << "_SR0/energy_" << iS
						<< "_SR2>>+" << lname.str();
				lcut.str("");
				lcut << "energy_" << iS << "_SR0 > 5";
				if (iS > 0)
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		for (unsigned iF(0); iF < nF; ++iF) { //loop on files
			E1[iF][iL]->Scale(
					E1[1][iL]->GetEntries() / E1[iF][iL]->GetEntries());
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		for (unsigned iF(0); iF < nF; ++iF) { //loop on files
			E1over7[iF][iL]->Scale(
					E1over7[1][iL]->GetEntries()
							/ E1over7[iF][iL]->GetEntries());
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		myc7all->cd(iL + 1);
		for (unsigned iF(0); iF < nF; ++iF) { //loop on files
			E7overtot[iF][iL]->Scale(
					E7overtot[1][iL]->GetEntries()
							/ E7overtot[iF][iL]->GetEntries());
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		gPad->SetGridx(1);
		for (unsigned iF(0); iF < nF; ++iF) { //loop on files
			EmaxOut7[iF][iL]->Scale(
					EmaxOut7[1][iL]->GetEntries()
							/ EmaxOut7[iF][iL]->GetEntries());
	...
	mycmaxout7->Update();
	mycmaxout7->Print(
			"ComparisonSquareHexa_EmaxHitOutOfSR2_gamma_pt20_eta2.pdf");

	myc2D->Divide(7, 4);
	for (unsigned iL(0); iL < nLayers; ++iL) {
		myc2D->cd(iL + 1);
		for (unsigned iF(0); iF < nF; ++iF) { //loop on files
			transverseCor[iF][iL]->Scale(
					transverseCor[1][iL]->GetEntries()
							/ transverseCor[iF][iL]->GetEntries());
			lname.str("");
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		for (unsigned iF(0); iF < nF; ++iF) {    //loop on files
			prof[iF][iL]->Rebin(2);
			if (iL < 16)
				prof[iF][iL]->GetYaxis()->SetRangeUser(0.5, 1.);
			else
				prof[iF][iL]->GetYaxis()->SetRangeUser(0.1, 1.);
			prof[iF][iL]->Draw(iF == 0 ? "PE" : "PEsame");
			std::ostringstream funcName;
			if (iL < 8 || iL > 20)
				funcName << "pol0";
			else
				funcName << "pol" << nPars - 1;
	...
	for (unsigned iF(0); iF < nF; ++iF) {    //loop on files
		...
		lname.str("");
		lname << "ECor" << label[iF];
		ECor[iF] = new TH1F(lname.str().c_str(), ";Ecor (mips);showers", 100,
				5000, 15000);
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			...
			lvar << "absweight_" << iL << "*energy_" << iL << "_SR" << iSR
					<< "/(" << par[iF][iL][0];
			if (iL > 7 && iL < 21)
				lvar << "+" << par[iF][iL][1] << "*energy_" << iL
						<< "_SR0/energy_" << iL << "_SR2";
			if (nPars > 2)
				lvar << "+" << par[iF][iL][2] << "*pow(energy_" << iL
						<< "_SR0/energy_" << iL << "_SR2,2)";
			if (nPars > 3)
				lvar << "+" << par[iF][iL][3] << "*pow(energy_" << iL
						<< "_SR0/energy_" << iL << "_SR2,3)";
			lvar << ")";
		}
		lvar << ">>" << lname.str();
		tree[iF]->Draw(lvar.str().c_str());
