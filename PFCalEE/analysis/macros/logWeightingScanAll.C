...
int logWeightingScanAll() { //main
	...
	TString plotDir =
			"/afs/cern.ch/work/a/amagnan/PFCalEEAna/PLOTS/gitV00-02-12/version12/gamma/200um/";
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		...
		lName << "mycx" << iC;
		mycx[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		mycx[iC]->Divide(3, 2);
		lName.str("");
		lName << "mycy" << iC;
		mycy[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1500,
				1000);
		mycy[iC]->Divide(3, 2);
	}
	...
	for (unsigned ieta(0); ieta < neta; ++ieta) {
		...
		if (savePoint) {
			fout =
					TFile::Open(
							("PLOTS/LogWeightingStudy_" + pteta.str() + ".root").c_str(),
							"RECREATE");
			fout->mkdir("scan");
		...
		if (savePoint) {
			...
			p_intercalibSigmaSquare[0] = new TH1F("p_intercalibSigmaSquare_0",
					";#sigma_{E}^{2} (2% intercalib) (GeV^2)", 3000, 0, 30000);
			p_intercalibSigmaSquare[1] = new TH1F("p_intercalibSigmaSquare_140",
					";#sigma_{E}^{2} (2% intercalib) (GeV^2)", 3000, 0, 30000);
		...
		for (unsigned ipu(0); ipu < nPu; ++ipu) {  //loop on pu
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {
				...
				if (savePoint) {
					...
					p_Exy[ipu][iL] = new TH2F(label.str().c_str(),
							";x idx;y idx; E (mips)", 5, 0, 5, 5, 0, 5);
				}
				label.str("");
				label << "deltavsreco_x_pu" << pu[ipu] << "_" << iL;
				p_deltavsreco_x[ipu][iL] = new TH2F(label.str().c_str(),
						";x reco (mm);x_{reco}-x_{truth} (mm);", 30, -15, 15,
						100, -10, 10);
				label.str("");
				label << "deltavsreco_y_pu" << pu[ipu] << "_" << iL;
				p_deltavsreco_y[ipu][iL] = new TH2F(label.str().c_str(),
						";y reco (mm);y_{reco}-y_{truth} (mm);", 30, -15, 15,
						100, -10, 10);
				if (savePoint) {
					label.str("");
					label << "recovstruth_x_pu" << pu[ipu] << "_" << iL;
					p_recovstruth_x[ipu][iL] = new TH2F(label.str().c_str(),
							";x_{truth} (mm);x reco (mm)", 30, -15, 15, 30, -15,
							15);
					label.str("");
					label << "recovstruth_y_pu" << pu[ipu] << "_" << iL;
					p_recovstruth_y[ipu][iL] = new TH2F(label.str().c_str(),
							";y_{truth} (mm);y reco (mm)", 30, -15, 15, //200,1170,1370,
							30, -15, 15);
				...
				for (unsigned iS(0); iS < nScans; ++iS) {
					...
					p_posx[ipu][iL][iS] = new TH1F(label.str().c_str(),
							";x-x_{truth} (mm);events", 100, -10, 10);
					label.str("");
					...
					p_posy[ipu][iL][iS] = new TH1F(label.str().c_str(),
							";y-y_{truth} (mm);events", 100, -10, 10);
				}
			}  //loop on layers
			...
			for (unsigned ipt(0); ipt < npt; ++ipt) {
				...
				linputStr << plotDir << "/" << "eta" << eta[ieta] << "_et"
						<< pt[ipt] << "_pu" << pu[ipu];
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {
					...
					for (unsigned iy(0); iy < 5; ++iy) {
						for (unsigned ix(0); ix < 5; ++ix) {
							...
							label.str("");
							label << "E_" << iL << "_" << idx;
							tree->SetBranchAddress(label.str().c_str(),
									&Exy[iL][idx]);
						}
					}
				}  //loop on layers
				...
				for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
					...
					if (ievt % 50 == 0)
						std::cout << "... Processing entry: " << ievt
								<< std::endl;
					...
					for (unsigned iL(0); iL < nLayers; ++iL) {
						...
						Etotsq += pow(
								calibratedE(
										Etot * absWeight(iL, eta[ieta] / 10.),
										eta[ieta] / 10.), 2);

						if (iL > 22) {
							Ex[0] = Exy[iL][0] + Exy[iL][5] + Exy[iL][10]
									+ Exy[iL][15] + Exy[iL][20];
							Ex[1] = Exy[iL][1] + Exy[iL][6] + Exy[iL][11]
									+ Exy[iL][16] + Exy[iL][21];
							Ex[2] = Exy[iL][2] + Exy[iL][7] + Exy[iL][12]
									+ Exy[iL][17] + Exy[iL][22];
							Ex[3] = Exy[iL][3] + Exy[iL][8] + Exy[iL][13]
									+ Exy[iL][18] + Exy[iL][23];
							Ex[4] = Exy[iL][4] + Exy[iL][9] + Exy[iL][14]
									+ Exy[iL][19] + Exy[iL][24];
							Ey[0] = Exy[iL][0] + Exy[iL][1] + Exy[iL][2]
									+ Exy[iL][3] + Exy[iL][4];
							Ey[1] = Exy[iL][5] + Exy[iL][6] + Exy[iL][7]
									+ Exy[iL][8] + Exy[iL][9];
							Ey[2] = Exy[iL][10] + Exy[iL][11] + Exy[iL][12]
									+ Exy[iL][13] + Exy[iL][14];
							Ey[3] = Exy[iL][15] + Exy[iL][16] + Exy[iL][17]
									+ Exy[iL][18] + Exy[iL][19];
							Ey[4] = Exy[iL][20] + Exy[iL][21] + Exy[iL][22]
									+ Exy[iL][23] + Exy[iL][24];
						} else {
						...
						unsigned cellCenter =
								static_cast<unsigned>((truthPosY[iL] + 5) / 10.)
										* 10;
						double yt = 0;
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {
				for (unsigned iS(0); iS < nScans; ++iS) {
					...
					double xval =
							useFit ?
									fitx->GetParameter(2) :
									p_posx[ipu][iL][iS]->GetRMS();
					if (savePoint)
						p_chi2ndf->Fill(fitx->GetChisquare() / fitx->GetNDF());

					grX[ipu][iL]->SetPoint(iS + 1, w0, fitx->GetParameter(2));
					grX[ipu][iL]->SetPointError(iS + 1, 0,
							fitx->GetParError(2));
					grXrms[ipu][iL]->SetPoint(iS + 1, w0,
							p_posx[ipu][iL][iS]->GetRMS());
					grXrms[ipu][iL]->SetPointError(iS + 1, 0,
							p_posx[ipu][iL][iS]->GetRMSError());
					...
					double yval =
							useFit ?
									fity->GetParameter(2) :
									p_posy[ipu][iL][iS]->GetRMS();
					if (savePoint)
						p_chi2ndf->Fill(fity->GetChisquare() / fity->GetNDF());
					grY[ipu][iL]->SetPoint(iS + 1, w0, fity->GetParameter(2));
					grY[ipu][iL]->SetPointError(iS + 1, 0,
							fity->GetParError(2));
					grYrms[ipu][iL]->SetPoint(iS + 1, w0,
							p_posy[ipu][iL][iS]->GetRMS());
					grYrms[ipu][iL]->SetPointError(iS + 1, 0,
							p_posy[ipu][iL][iS]->GetRMSError());
		...
		leg->SetFillColor(0);
		if (grX[0][10])
			leg->AddEntry(grX[0][10], "fit pu=0", "P");
		if (grXrms[0][10])
			leg->AddEntry(grXrms[0][10], "RMS pu=0", "P");
		if (grX[1][10])
			leg->AddEntry(grX[1][10], "fit pu=140", "P");
		if (grXrms[1][10])
			leg->AddEntry(grXrms[1][10], "RMS pu=140", "P");
		for (unsigned iC(0); iC < nCanvas; ++iC) {
			mycx[iC]->cd(1);
			leg->Draw("same");
			mycx[iC]->Update();
			std::ostringstream lsave;
			lsave << "PLOTS/logWeighted_x_" << 6 * iC << "_" << 6 * iC + 5
					<< "_" << pteta.str();
			lsave << ".pdf";
			...
			leg->Draw("same");
			lsave.str("");
			lsave << "PLOTS/logWeighted_y_" << 6 * iC << "_" << 6 * iC + 5
					<< "_" << pteta.str();
			lsave << ".pdf";
		...
		for (unsigned ipu(0); ipu < nPu; ++ipu) {	//loop on pu
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {
				std::cout << " if (layer==" << iL << ") return "
						<< (wxmin[ipu][iL] + wymin[ipu][iL]) / 2. << ";"
