...
int logWeightingScan() { //main
	...
	TFile *fout = TFile::Open(
			("PLOTS/LogWeightingStudy_" + pteta + ".root").c_str(), "RECREATE");
	fout->mkdir("scan");
	...
	for (unsigned ipu(0); ipu < nPu; ++ipu) { //loop on pu
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			...
			p_deltavsreco_x[ipu][iL] = new TProfile(label.str().c_str(),
					";x reco (mm);x_{reco}-x_{truth} (mm);", 30, -15, 15, -100,
					100);
			label.str("");
			label << "deltavsreco_y_pu" << pu[ipu] << "_" << iL;
			p_deltavsreco_y[ipu][iL] = new TProfile(label.str().c_str(),
					";y reco (mm);y_{reco}-y_{truth} (mm);", 30, -15, 15, -100,
					100);
			label.str("");
			label << "deltavsreco_y_wmin_pu" << pu[ipu] << "_" << iL;
			p_deltavsreco_y_wmin[ipu][iL] = new TProfile(label.str().c_str(),
					";y reco (mm);y_{reco}-y_{truth} (mm);", 30, -15, 15, -100,
					100);
			label.str("");
			label << "recovstruth_x_pu" << pu[ipu] << "_" << iL;
			p_recovstruth_x[ipu][iL] = new TH2F(label.str().c_str(),
					";x_{truth} (mm);x reco (mm)", 30, -15, 15, 30, -15, 15);
			label.str("");
			...
			for (unsigned i(0); i < 5; ++i) {
				...
				p_wx[ipu][iL][i] = new TH1F(label.str().c_str(), ";wx;events",
						100, -10, 0);
				label.str("");
				label << "wy_pu" << pu[ipu] << "_" << iL << "_" << i;
				p_wy[ipu][iL][i] = new TH1F(label.str().c_str(), ";wy;events",
						100, -10, 0);
			}
			for (unsigned iS(0); iS < nScans; ++iS) {
				label.str("");
		...
		for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {
				...
				for (unsigned idx(0); idx < 25; ++idx) {
					if (iL > 22)
						Etot += Exy[iL][idx];
					else if ((idx > 5 && idx < 9) || (idx > 10 && idx < 14)
							|| (idx > 15 && idx < 19))
						Etot += Exy[iL][idx];
				}
				if (iL > 22) {
					Ex[0] = Exy[iL][0] + Exy[iL][5] + Exy[iL][10] + Exy[iL][15]
							+ Exy[iL][20];
					Ex[1] = Exy[iL][1] + Exy[iL][6] + Exy[iL][11] + Exy[iL][16]
							+ Exy[iL][21];
					Ex[2] = Exy[iL][2] + Exy[iL][7] + Exy[iL][12] + Exy[iL][17]
							+ Exy[iL][22];
					Ex[3] = Exy[iL][3] + Exy[iL][8] + Exy[iL][13] + Exy[iL][18]
							+ Exy[iL][23];
					Ex[4] = Exy[iL][4] + Exy[iL][9] + Exy[iL][14] + Exy[iL][19]
							+ Exy[iL][24];
					Ey[0] = Exy[iL][0] + Exy[iL][1] + Exy[iL][2] + Exy[iL][3]
							+ Exy[iL][4];
					Ey[1] = Exy[iL][5] + Exy[iL][6] + Exy[iL][7] + Exy[iL][8]
							+ Exy[iL][9];
				...
				if (Etot != 0) {
					if (iL > 22) {
						simplex = 10 * (2 * Ex[4] + Ex[3] - Ex[1] - 2 * Ex[0])
								/ Etot;
						simpley = 10 * (2 * Ey[4] + Ey[3] - Ey[1] - 2 * Ey[0])
								/ Etot;
					} else {
				...
				unsigned cellCenter = static_cast<unsigned>((truthPosY[iL] + 5)
						/ 10.) * 10;
				double yt = 0;
				...
				for (unsigned iS(0); iS < nScans; ++iS) {
					...
					if (wx[5][iS] != 0) {
						if (iL > 22)
							x = 10
									* (2 * wx[4][iS] + wx[3][iS] - wx[1][iS]
											- 2 * wx[0][iS]) / wx[5][iS];
						else
							x = 10 * (wx[2][iS] - wx[0][iS]) / wx[5][iS];
					}
					double y = 0;	    //10*(wy[2][iS]-wy[0][iS])/wy[3][iS];
					if (wy[5][iS] != 0) {
						if (iL > 22)
							y = 10
									* (2 * wy[4][iS] + wy[3][iS] - wy[1][iS]
											- 2 * wy[0][iS]) / wy[5][iS];
						else
							y = 10 * (wy[2][iS] - wy[0][iS]) / wy[5][iS];
					}
					...
					if (iS == 16)
						p_deltavsreco_y_wmin[ipu][iL]->Fill(yt, y - yt);
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			for (unsigned iS(0); iS < nScans; ++iS) {
				...
				double xval = 0;
				if (fitx->GetChisquare() / fitx->GetNDF() < 20
						&& fitx->GetParameter(2)
				...
				double yval = 0;
				if (fity->GetChisquare() / fity->GetNDF() < 20
						&& fity->GetParameter(2)
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		...
		lsave << "PLOTS/logWeighted_x_" << 6 * iC << "_" << 6 * iC + 5 << "_"
				<< pteta << ".pdf";
		mycx[iC]->Print(lsave.str().c_str());
		mycx[iC]->Update();

		lsave.str("");
		lsave << "PLOTS/logWeighted_y_" << 6 * iC << "_" << 6 * iC + 5 << "_"
				<< pteta << ".pdf";
	...
	for (unsigned ipu(0); ipu < nPu; ++ipu) {      //loop on pu
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			std::cout << " Layer " << iL << " wmin " << wxmin[ipu][iL] << " "
					<< wymin[ipu][iL] << " "
