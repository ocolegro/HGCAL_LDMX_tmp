...
int plotMomentumMinbias() {
	...
	std::string label[2 * nF] = { "hadronMomentumAtIP",
			"hadronMomentumAtShowerMax", "hadronMomentumAtIP_log",
			"hadronMomentumAtShowerMax_log" };

	for (unsigned iF(0); iF < nF; ++iF) { //loop on files
		if (!f[iF]) {
			std::cout << " File not found. Label: " << label[iF]
					<< " Continue..." << std::endl;
			continue;
		}
		for (unsigned dolog(0); dolog < 2; ++dolog) {
			f[iF]->cd();
			if (iF == 0) {
				hP = (TH1F*) gDirectory->Get(
						dolog == 0 ? "hProton" : "hProtonLog");
				hPi = (TH1F*) gDirectory->Get(
						dolog == 0 ? "hPipm" : "hPipmLog");
				hN = (TH1F*) gDirectory->Get(
						dolog == 0 ? "hNeutron" : "hNeutronLog");
				if (!hP || !hN || !hPi) {
					std::cout << " -- histos not found ! Continue..."
							<< std::endl;
					continue;
				}
			} else {
				f[iF]->cd("protons");
				hP = (TH1F*) gDirectory->Get(
						dolog == 0 ? "p_momentum_p" : "p_logmomentum_p");
				f[iF]->cd();
				f[iF]->cd("neutrons");
				hN = (TH1F*) gDirectory->Get(
						dolog == 0 ? "p_momentum_n" : "p_logmomentum_n");
				f[iF]->cd();
				f[iF]->cd("pipm");
				hPi = (TH1F*) gDirectory->Get(
						dolog == 0 ? "p_momentum_pi" : "p_logmomentum_pi");
				f[iF]->cd();
				f[iF]->cd("Si");
				hSi = (TH1F*) gDirectory->Get(
						dolog == 0 ? "p_momentum_Si" : "p_logmomentum_Si");
				if (!hP || !hN || !hPi) {
					std::cout << " -- histos not found ! Continue..."
							<< std::endl;
					continue;
			...
			//hP->SetMaximum(max);
			if (dolog)
				hP->GetYaxis()->SetRangeUser(0.9, 3. * 1e6);
			else
				hP->GetYaxis()->SetRangeUser(0.9, 3. * 1e7);
			if (dolog == 0)
				hP->GetXaxis()->SetRangeUser(0, 30);
			...
			myc[2 * dolog + iF]->Print(
					("PLOTS/" + label[2 * dolog + iF] + silabel + ".pdf").c_str());
			myc[2 * dolog + iF]->Print(
					("PLOTS/" + label[2 * dolog + iF] + silabel + ".png").c_str());
	...
	std::cout << " parts n=" << part.size() << " axis n = " << ax->GetNbins()
			<< " gr n=" << gr->GetN() << " x1=" << x1 << " x2=" << x2
			<< std::endl;
	...
	myc[5]->Update();
	myc[5]->Print(
			("PLOTS/ShowerMaxNeutronTimevsLogp" + silabel + ".pdf").c_str());
	myc[5]->Print(
			("PLOTS/ShowerMaxNeutronTimevsLogp" + silabel + ".png").c_str());
