...
int plotMipHistos() {
	...
	outFilePath
			<< "/afs/cern.ch/work/a/amagnan/PFCalEEAna/PLOTS/gitV00-02-12/version12/MinBias/Histos"
			<< suffix.str() << "_0_21.root";
	...
	for (unsigned ie(0); ie < nEta; ++ie) {
		...
		for (unsigned in(0); in < nNoise; ++in) {
			...
			label.str("");
			label << "grNhits_eta" << eta[ie] * 10 << "_noise"
					<< noise[in] * 100;
			...
			label.str("");
			label << "grSigma_eta" << eta[ie] * 10 << "_noise"
					<< noise[in] * 100;
			...
			label << "EmaxNeighbour_eta" << eta[ie] * 10 << "_noise"
					<< noise[in] * 100;
			p_EmaxNeighbour[ie][in] = (TH1F*) gDirectory->Get(
					label.str().c_str());
			for (unsigned il(0); il < nLayers; ++il) {
				label.str("");
				label << "hitSpectrum";
				if (doSignalOnly)
					label << "Sig";
				label << "_eta" << eta[ie] * 10 << "_noise" << noise[in] * 100
						<< "_layer" << il;
				hitSpectrum[ie][in][il] = (TH1F*) gDirectory->Get(
						label.str().c_str());
				hitSpectrum[ie][in][il]->SetTitle(
						";E_{1#times1cm^{2}}(mips);N_{cells}");
				label.str("");
				label << "Nhits_eta" << eta[ie] * 10 << "_noise"
						<< noise[in] * 100 << "_layer" << il;
				p_nHits[ie][in][il] = (TH1F*) gDirectory->Get(
						label.str().c_str());

				if (il == 0)
					hitSpectrumAll[ie][in] =
							(TH1F*) hitSpectrum[ie][in][il]->Clone(
									label.str().c_str());
				else
					hitSpectrumAll[ie][in]->Add(hitSpectrum[ie][in][il]);
				hitSpectrumAll[ie][in]->SetTitle(
						";E_{1#times1cm^{2}}(mips);N_{cells}");
	...
	for (unsigned ie(0); ie < nEta; ++ie) {
		...
		for (unsigned in(0); in < nNoise; ++in) {
			...
			if (ie == 0 && in == 0)
				p_EmaxNeighbour[ie][in]->Draw("PEL");
			else
				p_EmaxNeighbour[ie][in]->Draw("PELsame");
			label.str("");
			label << "#sigma_{N} = " << noise[in] << " mips";
			if (ie == 0)
				legN->AddEntry(p_EmaxNeighbour[ie][in], label.str().c_str(),
						"P");
		}
	}
	legN->Draw("same");
	...
	for (unsigned ie(0); ie < nEta; ++ie) {
		for (unsigned in(0); in < nNoise; ++in) {
			for (unsigned il(0); il < nLayers; ++il) {
				...
				if (in == 0) {
					hitSpectrum[ie][in][il]->Fit("landau", "R+", "same", minfit,
							maxfit);
					fit = (TF1*) hitSpectrum[ie][in][il]->GetFunction("landau");
					if (!fit)
						continue;
					landaugaus->SetParameters(fit->GetParameter(2),
							fit->GetParameter(1), fit->GetParameter(0),
							noise[in], fit->GetParameter(0), 0, noise[in]);
					if (doSignalOnly) {
					...
					if (doSignalOnly)
						landaugaus->FixParameter(6, 0.);
					else {
						landaugaus->FixParameter(6, noise[in]);
						minfit = 0.;
					}
					hitSpectrum[ie][in][il]->Fit("landaugaus", "BR+", "same",
							minfit, maxfit);
					fit = (TF1*) hitSpectrum[ie][in][il]->GetFunction(
							"landaugaus");
					if (!fit)
						continue;
				...
				if (il % 3 == 0) {
					...
					sprintf(buf, "%3.2f < #eta < %3.2f", eta[ie] - deta,
							eta[ie] + deta);
				...
				grNhits[ie][in]->SetPoint(il, il,
						p_nHits[ie][in][il]->GetMean());
				grNhits[ie][in]->SetPointError(il, 0,
						p_nHits[ie][in][il]->GetRMS());
			...
			label.str("");
			label << "PLOTS/" << suffix.str() << "/MipSpectrum_eta"
					<< eta[ie] * 10 << "_noise" << noise[in];
			//label << "_layer" << il;
			//label << suffix.str();
			label << "_perlayer.pdf";
		...
		for (unsigned in(0); in < nNoise; ++in) {
			...
			if (in == 0) {
				minfit = 0.75;
				maxfit = 1.4;
				hitSpectrumAll[ie][in]->Fit("landau", "R+", "same", minfit,
						maxfit);
				TF1 *fit = (TF1*) hitSpectrumAll[ie][in]->GetFunction("landau");
				if (!fit)
					continue;
				landaugaus->SetParameters(fit->GetParameter(2),
						fit->GetParameter(1), fit->GetParameter(0), noise[in],
						fit->GetParameter(0), 0, noise[in]);
				if (doSignalOnly) {
					landaugaus->SetParameter(6, 0);
					landaugaus->SetParameter(4, 0);
					landaugaus->FixParameter(4, 0);
				...
				hitSpectrumAll[ie][in]->Fit("landaugaus", "BR+", "same", minfit,
						maxfit);
			}
		...
		for (unsigned in(0); in < nNoise; ++in) {
			...
			label << "#eta = " << eta[ie];
			if (in == 0)
				leg->AddEntry(grMPV[ie][in], label.str().c_str(), "P");
			if (ie == nEta - 1)
				leg->Draw("same");
			sprintf(buf, "Noise = %3.2f MIPs", noise[in]);
			lat.DrawLatexNDC(0.2, 0.85, buf);
