...
int plotMaxEhitHCAL() {
	...
	unsigned genEt[nGenEn][nHists] = { { 320, 320 }, //,170,110},
			{ 650, 650 }, //,320,220},
			{ 1000, 1000 } //,500,330}
	};
		...
		for (unsigned iS(0); iS < nS; ++iS) { //loop on scenarios
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				for (unsigned iH(0); iH < nHists; ++iH) {
					...
					if (iH == 0)
						linputStr << "/" << scenario[iS] << genEt[iE][0];
					else
						linputStr << "/pi-";
					linputStr << "/validation_" << etastr[iH];
					if (iH == 1)
						linputStr << "_et" << genEt[iE][iH];
					linputStr << ".root";
					...
					gStyle->SetStatW(0.3);
					maxEhit[iH]->Rebin(
							genEn[iE] > 500 ? 20 * iE : (iE > 0 ? 2 * iE : 1));
					maxEhit[iH]->GetXaxis()->SetRange(
							maxEhit[iH]->FindFirstBinAbove(0),
							maxEhit[iH]->FindLastBinAbove(0));
					maxEhit[iH]->GetXaxis()->SetTitle("max E_{hit} (MIPS)");
					maxEhit[iH]->Draw();

					sprintf(buf, "Maximum = %3.0f MIPs",
							maxEhit[iH]->GetXaxis()->GetBinCenter(
									maxEhit[iH]->FindLastBinAbove(0)));
					lat.SetTextSize(0.05);
					lat.DrawLatexNDC(0.12, 0.85, buf);
					//sprintf(buf,"%s #mu m Si",pSuffix.c_str());
					lat.DrawLatexNDC(0.12, 0.75, label[iH].c_str());
					sprintf(buf, "Geant4 Standalone, %3.3f TeV",
							genEt[iE][iH] * cosh(leta[iH]));
					...
					mean3rms[iH][iE] = maxEhit[iH]->GetMean()
							+ 3 * maxEhit[iH]->GetRMS();
					mean3rmserr[iH][iE] = maxEhit[iH]->GetMeanError()
							+ 3 * maxEhit[iH]->GetRMSError();
					...
					for (int iB(0); iB < maxEhit[iH]->GetNbinsX() + 2; ++iB) {
						...
						if (integral / total > 0.95) {
							int95[iH][iE] =
									maxEhit[iH]->GetXaxis()->GetBinCenter(iB);
							int95err[iH][iE] =
									maxEhit[iH]->GetXaxis()->GetBinWidth(iB);
							break;
						}
					}
					...
					std::cout << label[iH] << " E="
							...
							<< mean3rms[iH][iE] << " #pm "
							<< mean3rmserr[iH][iE] << "$ & $" << int95[iH][iE]
							<< " #pm " << int95err[iH][iE] << "$ \\\\"
							<< std::endl;

					std::ostringstream lsave;
					lsave << plotDir.str() << pSuffix << "/FHmaxEhit_"
							<< etastr[iH] << "_" << genEn[iE] << "TeV.pdf";
			...
			for (unsigned i(0); i < nP; ++i) {
				...
				for (unsigned iH(0); iH < nHists; ++iH) {
					if (i == 0)
						gr[iH][i] = new TGraphErrors(nGenEn, genEnD, mean[iH],
								genEnErr, rms[iH]);
					else if (i == 1)
						gr[iH][i] = new TGraphErrors(nGenEn, genEnD,
								mean3rms[iH], genEnErr, mean3rmserr[iH]);
					else
						gr[iH][i] = new TGraphErrors(nGenEn, genEnD, int95[iH],
								genEnErr, int95err[iH]);
					gr[iH][i]->GetXaxis()->SetTitle("Beam energy (TeV)");
					if (i == 0)
						gr[iH][i]->GetYaxis()->SetTitle("<maxEhit> (MIPs)");
					else if (i == 1)
						gr[iH][i]->GetYaxis()->SetTitle(
								"<maxE>+3#times RMS (MIPs)");
					else
						gr[iH][i]->GetYaxis()->SetTitle("95% content  (MIPs)");
					gr[iH][i]->SetTitle("");
					...
					//else 
					sprintf(buf,
							"<maxE> = (%3.1f #pm %3.1f)  + (%3.2f #pm %3.2f)#times E",
							fitfunc[iH]->GetParameter(0),
							fitfunc[iH]->GetParError(0),
							fitfunc[iH]->GetParameter(1),
							fitfunc[iH]->GetParError(1));
