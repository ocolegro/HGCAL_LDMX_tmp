...
int plotXY() { //main  
	...
	const unsigned nS = 1;
	std::string scenario[nS] = { "pi-/twiceSampling/GeVCal/EarlyDecay/"
	//"pi+/PU/eta20/",
	//"pi+/PU/eta25/",
	//"pi+/PU/eta30/"
	//"e-/noWeights/"
		...
		for (unsigned iS(0); iS < nS; ++iS) {    //loop on scenarios
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {
					...
					p_Etot[iE][iL] = (TH1F*) gDirectory->Get(
							lName.str().c_str());
					if (!p_Etot[iE][iL]) {
						std::cout
								<< " -- ERROR, pointer for Etot histogram is null for layer: "
								<< iL << ". Exiting..." << std::endl;
						return 1;
					}
					if (p_Etot[iE][iL]->GetMean() > EmaxLayer)
						EmaxLayer = p_Etot[iE][iL]->GetMean()
								+ 5 * p_Etot[iE][iL]->GetRMS();
				}
				if (stopProcessing)
					continue;
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {      //loop on layers
					...
					if (doXYplots) {
						...
						if (iL % 3 == 2 && counter < 10 && iL < nEcalLayers) {
							mycECAL->cd(counter);
							counter++;
						} else if (iL % 2 == 1 && counter < 19
								&& iL >= nEcalLayers) {
							std::cout << " -- Hcal layer " << iL
					...
					if (doXYplots) {
						...
						myc->Update();
						saveName.str("");
						saveName << plotDir << "/xySimHits/xySimHits_layer"
								<< iL << "_" << genEn[iE] << "GeV";
				...
				saveName.str("");
				if (doXYplots)
					saveName << plotDir << "/xySimHits_ECAL_" << genEn[iE]
							<< "GeV";
				else
					saveName << plotDir << "/xySimHits_ECAL_" << genEn[iE]
							<< "GeV_subset";
				...
				saveName.str("");
				if (doXYplots)
					saveName << plotDir << "/xySimHits_HCAL_" << genEn[iE]
							<< "GeV";
				else
					saveName << plotDir << "/xySimHits_HCAL_" << genEn[iE]
							<< "GeV_subset";
				...
				if (iE == 5) {
					p_EvsX0 = new TH2F("EvsX0", ";Absorber X_{0}; E (MeV)", 50,
							0, 25,
							//30,X0,
							100, 0, 200);
					...
					for (int iX(0); iX < nEcalLayers; ++iX) {
						for (int iY(1); iY < p_EvsLayer[iE]->GetNbinsY() + 1;
								++iY) {
							if (iY == 200)
								std::cout << iX << " " << iY << " " << X0[iX]
										<< " "
										<< p_EvsLayer[iE]->GetYaxis()->GetBinCenter(
												iY) << " "
										<< p_EvsLayer[iE]->GetBinContent(iX + 1,
												iY) << std::endl;
							p_EvsX0->Fill(X0[iX],
									p_EvsLayer[iE]->GetYaxis()->GetBinCenter(
											iY),
									p_EvsLayer[iE]->GetBinContent(iX + 1, iY)
											< 0 ?
											0 :
											p_EvsLayer[iE]->GetBinContent(
													iX + 1, iY));
