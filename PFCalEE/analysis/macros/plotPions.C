...
int plotPions() { //main  
	...
	for (unsigned iE(0); iE < nGenEn; ++iE) {
		...
		linputStr << plotDir << "validation_e" << genEn[iE] << pSuffix
				<< ".root";
		...
		if (!p_HCALvsECAL[iE]) {
			std::cout << " -- ERROR, pointer for histogram is null for energy: "
					<< genEn[iE] << ". Exiting..." << std::endl;
			return 1;
		}
		...
		double Xintercept = p_HCALvsECAL[iE]->GetXaxis()->GetBinLowEdge(
				binXint);
		...
		for (unsigned iX(1); iX < p_HCALvsECAL[iE]->GetNbinsX() + 1; ++iX) {
			...
			for (unsigned iY(1); iY < p_HCALvsECAL[iE]->GetNbinsY() + 1; ++iY) {
				double tmpY = p_HCALvsECAL[iE]->GetYaxis()->GetBinLowEdge(
						iY + 1);
		...
		std::cout << " -- minX = " << minX << " maxX = " << maxX << " "
				<< " maxY = " << maxY << " " << " Xintercept = " << Xintercept
				<< " " << std::endl;
		...
		prof[iE]->Fit("mypol1", "R+", "same", minX + tmpRange / 5.,
				Xintercept - tmpRange / 5.);  //,maxX/20,maxX);
		...
		sprintf(buf, "#alpha = %3.3f #pm %3.3f", mypol1->GetParameter(0),
				mypol1->GetParError(0));
		lat.DrawLatex(xpos, ypos * 0.8, buf);
		sprintf(buf, "#beta = %3.3f #pm %3.3f", mypol1->GetParameter(1),
				mypol1->GetParError(1));
		lat.DrawLatex(xpos, ypos * 0.7, buf);
		sprintf(buf, "Chi2/NDF = %3.1f/%d = %3.1f", mypol1->GetChisquare(),
				mypol1->GetNDF(), mypol1->GetChisquare() / mypol1->GetNDF());
	...
	TGraphErrors *gr = new TGraphErrors(nGenEn, grgenEn, slope, genEnErr,
			slopeErr);
	myc->cd();
	myc->SetLogx(1);
	//gr->SetMinimum(-2);
	...
	if (pDetector.find("BHCALvsFHCAL") != pDetector.npos)
		gr->GetYaxis()->SetTitle("Slope Scint/Si");
	gr->SetTitle("");
	gr->Draw("AP");
