...
int plotSR() {
	...
	TH2F *h5 = new TH2F("h5", "SR 5;width (pads);layer", 5, -2.5, 2.5, nLayers,
			0, nLayers);
	TH2F *h6 = new TH2F("h6", "SR 6;width (pads);layer", 5, -2.5, 2.5, nLayers,
			0, nLayers);
	...
	std::cout << " -- Proba to pass 0.5 MIP cut just from gaussian noise : "
			<< proba << std::endl;
	...
	for (unsigned iSR(0); iSR < nSR; ++iSR) {
		...
		std::cout << " SR " << iSR << " " << nCells[iSR] << " " << noiseMIP
				<< " MIPs, " << noiseGeV << " GeV." << std::endl;
