...
int plotClusters() { //main
	...
	for (unsigned ipu(0); ipu < nPu; ++ipu) { //loop on pu
		...
		p_nClusters[ipu] =
				(TH1F*) ((TH1F*) (gDirectory->Get("p_nClusters"))->Clone(
						"p_nClusters" + puStr[ipu]));
		...
		p_seedEoverE_all[ipu] =
				(TH1F*) gDirectory->Get("p_seedEoverE_all")->Clone(
						"p_seedEoverE_all" + puStr[ipu]);
		p_clusLayer_all[ipu] =
				(TH1F*) gDirectory->Get("p_clusLayer_all")->Clone(
						"p_clusLayer_all" + puStr[ipu]);
		p_clusWidth_all[ipu] =
				(TH1F*) gDirectory->Get("p_clusWidth_all")->Clone(
						"p_clusWidth_all" + puStr[ipu]);
		p_seeddeta_all[ipu] = (TH1F*) gDirectory->Get("p_seeddeta_all")->Clone(
				"p_seeddeta_all" + puStr[ipu]);
		p_seeddphi_all[ipu] = (TH1F*) gDirectory->Get("p_seeddphi_all")->Clone(
				"p_seeddphi_all" + puStr[ipu]);

		p_clusnHits_sel[ipu] =
				(TH1F*) gDirectory->Get("p_clusnHits_sel")->Clone(
						"p_clusnHits_sel" + puStr[ipu]);
		p_clusnHits_sel[ipu]->SetTitle(";n_{hits};n_{events}");
		p_seedEoverE_sel[ipu] =
				(TH1F*) gDirectory->Get("p_seedEoverE_sel")->Clone(
						"p_seedEoverE_sel" + puStr[ipu]);
		p_clusLayer_sel[ipu] =
				(TH1F*) gDirectory->Get("p_clusLayer_sel")->Clone(
						"p_clusLayer_sel" + puStr[ipu]);
		p_clusWidth_sel[ipu] =
				(TH1F*) gDirectory->Get("p_clusWidth_sel")->Clone(
						"p_clusWidth_sel" + puStr[ipu]);
		p_seeddeta_sel[ipu] = (TH1F*) gDirectory->Get("p_seeddeta_sel")->Clone(
				"p_seeddeta_sel" + puStr[ipu]);
		p_seeddphi_sel[ipu] = (TH1F*) gDirectory->Get("p_seeddphi_sel")->Clone(
				"p_seeddphi_sel" + puStr[ipu]);
	}
