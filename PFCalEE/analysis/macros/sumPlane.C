...
void sumPlane(const char *glob) {
	...
	TH1D *hsumplane = new TH1D("hsumplane", "#Sigma_{all cells} E(cell);MIPs",
			55, -50, 500); //80,0,800); 
	TH1D *hsum7cellE = new TH1D("hsum7cellE",
			"#Sigma_{7 central cells} E(cell);MIPs", 55, -50, 500); //80,0,800);
	TH1D *hcentralcellE = new TH1D("hcentralcellE", "E(central cell); MIPs", 55,
			-50, 500); //80,0,800);

	TH1I *hnum1cell = new TH1I("hnum1cell", "hnum1cell", 10, 0, 10);
	TH1I *hnum7cell = new TH1I("hnum7cell", "hnum7cell", 10, 0, 10);
	TH1I *hnumplanecells = new TH1I("hnumplanecells", "hnumplanecells", 100, 0,
			100);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
		...
		for (int i = 0; i < rechits->size(); i++) {
			...
			planeE += hit.energy();
			numplanecells++;
			double celldistfromO = hit.get_x() * hit.get_x()
					+ hit.get_y() * hit.get_y();
