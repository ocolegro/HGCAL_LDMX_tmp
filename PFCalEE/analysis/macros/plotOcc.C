...
double getEta(double ypos, std::string etaStr, const double xLaymm) {
	double eta0 = 0;
	if (etaStr.find("20") != etaStr.npos)
		eta0 = 2.0;
	else if (etaStr.find("25") != etaStr.npos)
		eta0 = 2.5;
	else if (etaStr.find("30") != etaStr.npos)
		eta0 = 3.0;
	else if (etaStr.find("35") != etaStr.npos)
		eta0 = 3.5;
	else {
...
int plotOcc() {  //main  
	...
	TString etaStr[nS] = { "#eta = 2.0", "#eta = 2.5", "#eta = 3.0",
			"#eta = 3.5", };
		...
		for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
			...
			for (unsigned iO(0); iO < nOcc; ++iO) {
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {
					...
					p_occupancy[iL] = (TH2F*) gDirectory->Get(
							lName.str().c_str());
					if (!p_occupancy[iL]) {
						std::cout
								<< " -- ERROR, pointer for histogram is null for layer: "
								<< iL << ". Exiting..." << std::endl;
						return 1;
					}
					...
					int binNominal = 1;
					double minDiff = 100;
					for (int iB(1); iB < p_occupancy[iL]->GetNbinsY() + 1;
							++iB) {
						char buf[500];
						double ypos = p_occupancy[iL]->GetYaxis()->GetBinCenter(
								iB);
					...
					double content0p = p_occupancy[iL]->GetBinContent(bin0Xp,
							binNominal);
					double content0n = p_occupancy[iL]->GetBinContent(bin0Xn,
							binNominal);
					int bin0X = content0p > content0n ? bin0Xp : bin0Xn;
					maxOcc[iO][iL] = p_occupancy[iL]->GetBinContent(bin0X,
							binNominal);
					maxOccErr[iO][iL] = p_occupancy[iL]->GetBinError(bin0X,
							binNominal);
					double ypos = p_occupancy[iL]->GetYaxis()->GetBinCenter(
							binNominal);
					double lEta = getEta(ypos, scenario[iS], xLay[iL]);
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {
					...
					TPaletteAxis *palette =
							(TPaletteAxis*) p_occupancy[iL]->GetListOfFunctions()->FindObject(
									"palette");
					...
					if (iL % nPads == nPads - 1) {
						...
						saveName.str("");
						saveName << plotDir << "/occupancy_" << occThreshold[iO]
								<< "_" << counter;
						myc[nPlots * iS + counter]->Print(
								(saveName.str() + ".png").c_str());
						myc[nPlots * iS + counter]->Print(
								(saveName.str() + ".pdf").c_str());
						counter++;
					}
			...
			for (unsigned iO(0); iO < nOcc; ++iO) {
				gr[iO] = new TGraphErrors(nLayers, lay, maxOcc[iO], layerr,
						maxOccErr[iO]);
