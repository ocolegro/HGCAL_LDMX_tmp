...
void drawLayerWithGap(TLatex & lat, bool random, double ypos = 0) {
	...
	for (unsigned il(0); il < 28; ++il) {
		if (random)
			...
			sprintf(buf, "Layer %d-%d", il, il + 1);
		double offset = random ? 10. * ((7 * il) % 31) : il / 2 * 30;
		if (-230 + offset < -170)
			continue;
		if (random || (!random && il % 2 == 0))
			lat.DrawLatex(-230. + offset, ypos, buf);
	}
	for (unsigned il(0); il < 28; ++il) {
		if (random)
			sprintf(buf, "Layer %d", il);
...
int plotSigmaEffvsVtxPos() { //main
	...
	fcalib[0] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalCracks/gitV06-03-04/version100/model3/gamma/eta20_et60_pu0_IC3_Si2.root");
	fcalib[1] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalCracks/gitV06-03-04/version100/model3/gamma/phi_0.440pi/eta20_et60_pu0_IC3_Si2.root");
	...
	fin[0] =
			TFile::Open(
					("/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalCracks/git"
							+ version
							+ "/version100/model4/gamma/eta20_et60_pu0_IC3_Si2.root").c_str());
	fin[1] =
			TFile::Open(
					("/afs/cern.ch/work/a/amagnan/PFCalEEAna/HGCalCracks/git"
							+ version
							+ "/version100/model4/gamma/phi_0.440pi/eta20_et60_pu0_IC3_Si2.root").c_str());
	...
	for (unsigned iF(0); iF < 3 * nF; ++iF) { //loop on files
		...
		lname << "mycCalib" << iF;
		if (iF < 2)
			mycCalib[iF] = new TCanvas(lname.str().c_str(), lname.str().c_str(),
					1500, 1000);
		lname.str("");
		lname << "myc" << iF;
		myc[iF] = new TCanvas(lname.str().c_str(), lname.str().c_str(), 1500,
				1000);
	}
	...
	for (unsigned iF(0); iF < nF; ++iF) { //loop on files
		mycE[iF]->Print(
				("PlotsCracks/EtotalAll_" + label[iF] + ".pdf[").c_str());
		mycCalib[iF]->cd();
		...
		hECalib[iF] = new TH1F(lcalib.str().c_str(), ";E (Mips); showers", 100,
				20000, 28000);
		...
		sprintf(buf, "#sigma_{eff} = %3.3f #pm %3.3f GeV",
				sigmaeff_ref * normalisation,
				hECalib[iF]->GetRMSError() * normalisation);
		lat.DrawLatexNDC(0.2, 0.6, buf);
		sprintf(buf, "#sigma_{eff}/E_{gen} = %3.3f",
				sigmaeff_ref * normalisation / Egen);
		lat.DrawLatexNDC(0.2, 0.5, buf);
		mycCalib[iF]->Update();
		mycCalib[iF]->Print(
				("PlotsCracks/EtotalCalib_" + label[iF] + ".pdf").c_str());
		...
		hETot[iF] = new TH1F(lcalib.str().c_str(), ";E (GeV); showers", 6000, 0,
				300);
		for (unsigned iP(0); iP < nP; ++iP) { //loop on points
			...
			hE[iF][iP] = new TH1F(lname.str().c_str(), ";E (GeV); showers", 150,
					0, 300);

			lvar << "wgtEtotal*" << normalisation << ">>" << lname.str();
			lcut << "vtxX>" << xmin + iP * stepsize << " && vtxX<"
					<< xmin + (iP + 1) * stepsize;
			...
			mycE[iF]->Update();
			TPaveStats *stats =
					(TPaveStats*) (hE[iF][iP]->GetListOfFunctions()->FindObject(
							"stats"));
			stats->SetX1NDC(0.15);
			stats->SetX2NDC(0.51);
			stats->SetY1NDC(0.12);
			...
			sprintf(buf, "#sigma_{eff}=%3.3f #pm %3.3f GeV",
					effSigmaMacro(hE[iF][iP]), hE[iF][iP]->GetRMSError());
		...
		myc[iF]->Update();
		myc[iF]->Print(
				("PlotsCracks/EtotalPerVertexBin_" + label[iF] + ".pdf").c_str());

		mycE[iF]->cd();
		hETot[iF]->Draw();
		mycE[iF]->Update();
		TPaveStats* stats =
				(TPaveStats*) (hETot[iF]->GetListOfFunctions()->FindObject(
						"stats"));
		stats->SetX1NDC(0.15);
		stats->SetX2NDC(0.41);
		stats->SetY1NDC(0.12);
		stats->SetY2NDC(0.42);
		sprintf(buf, "Single #gamma, E_{gen} = %3.1f GeV, #eta=%3.1f", Egen,
				eta);
		...
		sprintf(buf, "#sigma_{eff}=%3.3f #pm %3.3f GeV", sigefftot,
				hETot[iF]->GetRMSError());
		...
		double errint = 0;
		double fraction = hETot[iF]->IntegralAndError(binmin, binmax, errint)
				/ hETot[iF]->Integral();
		binmin = hETot[iF]->FindBin(Egen - sigefftot);
		binmax = hETot[iF]->FindBin(Egen + sigefftot);
		double errintref = 0;
		double fractionref = hETot[iF]->IntegralAndError(binmin, binmax,
				errintref) / hETot[iF]->Integral();

		sprintf(buf,
				"f_{-#sigma_{ref}}^{#sigma_{ref}}=%3.3f #pm %3.3f (%3.3f #pm %3.3f)",
				fraction, errint / hETot[iF]->Integral(), fractionref,
				errintref / hETot[iF]->Integral());
		lat.DrawLatexNDC(0.2, 0.5, buf);

		mycE[iF]->Update();
		mycE[iF]->Print(
				("PlotsCracks/EtotalAllAligned_" + label[iF] + ".pdf").c_str());
		...
		TBox *cr1 = new TBox(-61.7, gr[iF]->GetMinimum(), -51.7,
				gr[iF]->GetMaximum());
		cr1->SetFillColor(7);
		//cr1->SetFillStyle(3004);
		if (linedup)
			cr1->Draw();
		TBox *cr2 = new TBox(41.7, gr[iF]->GetMinimum(), 51.7,
				gr[iF]->GetMaximum());
		cr2->SetFillColor(7);
		//cr2->SetFillStyle(3004);
		if (linedup)
			cr2->Draw();
		TBox *cr3 = new TBox(145, gr[iF]->GetMinimum(), 155,
				gr[iF]->GetMaximum());
		cr3->SetFillColor(7);
		...
		TLine *ref = new TLine(-170, sigmaeff_ref * normalisation / Egen, 290,
				sigmaeff_ref * normalisation / Egen);
		ref->SetLineColor(2);
		ref->Draw();
		gr[iF]->Draw("PE");
		...
		myc[2 + iF]->Update();
		myc[2 + iF]->Print(
				("PlotsCracks/SigmaEffvsVtxPos_" + label[iF] + ".pdf").c_str());
		...
		cr1 = new TBox(-61.7, grMean[iF]->GetMinimum(), -51.7,
				grMean[iF]->GetMaximum());
		cr1->SetFillColor(7);
		//cr1->SetFillStyle(3004);
		if (linedup)
			cr1->Draw();
		cr2 = new TBox(41.7, grMean[iF]->GetMinimum(), 51.7,
				grMean[iF]->GetMaximum());
		cr2->SetFillColor(7);
		//cr2->SetFillStyle(3004);
		if (linedup)
			cr2->Draw();
		cr3 = new TBox(145, grMean[iF]->GetMinimum(), 155,
				grMean[iF]->GetMaximum());
		cr3->SetFillColor(7);
		//cr3->SetFillStyle(3004);
		if (linedup)
		...
		myc[4 + iF]->Update();
		myc[4 + iF]->Print(
				("PlotsCracks/MeanDiffvsVtxPos_" + label[iF] + ".pdf").c_str());
