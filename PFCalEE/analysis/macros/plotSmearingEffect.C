...
int plotSmearingEffect() {  //main
	...
	const unsigned nS = 1;
	std::string scenario[nS] = { "gamma/200um/" };
	...
	for (unsigned ip(startIdx); ip < (startIdx + nPts); ++ip) {  //loop on tries
		...
		for (unsigned ic(0); ic < nIC; ++ic) {  //loop on intercalib
			...
			for (unsigned iE(0); iE < nGenEnAll; ++iE) {
				...
				outtree[iE] = new TTree(label.str().c_str(),
						"Etot calibrated and smeared");
				label.str("");
				label << "Etot";
			...
			for (unsigned iV(0); iV < nV; ++iV) {  //loop on versions
				for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
					...
					TString plotDir =
							"/afs/cern.ch/work/a/amagnan/PFCalEEAna/PLOTS/gitV00-02-12/version"
					...
					for (unsigned ieta(0); ieta < neta; ++ieta) {  //loop on eta
						...
						for (unsigned ipu(0); ipu < nPu; ++ipu) {  //loop on pu
							...
							for (unsigned iE(0); iE < nGenEnAll; ++iE) {
								...
								linputStr << "eta" << eta[ieta] << "_et"
										<< genEnAll[iE] << "_pu" << pu[ipu]
										<< "_IC0";  // << ICval[ic];
								linputStr << ".root";
								inputFile[ieta][ipu][iE] = TFile::Open(
										linputStr.str().c_str());
								if (!inputFile[ieta][ipu][iE]) {
									...
									ltree[ieta][ipu][iE] =
											(TTree*) gDirectory->Get("Ereso");

									if (!ltree[ieta][ipu][iE]) {
										std::cout << " -- File "
												<< inputFile[ieta][ipu][iE]->GetName()
							...
							calibRecoDelta[0] =
									(TGraphErrors *) calibRecoFit[0]->Clone(
											"calibRecoDelta" + srStr);

							for (unsigned iE(0); iE < nGenEn; ++iE) {

								std::cout << "- Processing energy : "
										<< genEn[iE] << std::endl;

								inputFile[ieta][ipu][oldIdx[iE]]->cd(
										"Energies");
								...
								for (unsigned iL(0); iL < nLayers; ++iL) {
									...
									if (iL == 0)
										lName << fact << "*subtractedenergy_"
												<< iL << "_SR4";
									else
										lName << "+" << fact
												<< "*subtractedenergy_" << iL
												<< "_SR4";
								}
								...
								p_Ereco[iE][0] = (TH1F*) (gPad->GetPrimitive(
								...
								std::cout << " --- Reco E = entries "
										<< p_Ereco[iE][0]->GetEntries()
										<< " mean " << p_Ereco[iE][0]->GetMean()
										<< " rms " << p_Ereco[iE][0]->GetRMS()
										<< " overflows "
										<< p_Ereco[iE][0]->GetBinContent(
												p_Ereco[iE][0]->GetNbinsX() + 1)
										<< std::endl;

								p_Ereco[iE][0]->SetTitle(
										(";E (" + unit + ");events").c_str());
								...
								FitResult lres;
								if (fitEnergy(p_Ereco[iE][0], lpad, unit, lres,
										iSR) != 0)
									return 1;
								lpad->cd();
								char buf[500];
								...
								if (ipu > 0) {
									...
									p_Ereco[iE][0]->Draw(
											ipu == 1 ? "" : "same");
									TF1 *fite = p_Ereco[iE][0]->GetFunction(
											"fitResult");
									...
									lat.SetTextSize(0.05);
									lat.DrawLatexNDC(0.25, 0.965, buf);
									sprintf(buf, "#eta=%3.1f, IC %d %%",
											etaval[ieta], ICval[ic]);
									lat.SetTextSize(0.06);
									lat.DrawLatexNDC(0.15, 0.87, buf);
									lat.SetTextColor(ipu + 1);
									sprintf(buf, "#sigma/E = %3.1f/%3.1f=%3.3f",
											fite->GetParameter(2),
											fite->GetParameter(1),
											fite->GetParameter(2)
													/ fite->GetParameter(1));
									lat.DrawLatexNDC(0.15, 0.85 - 0.1 * ipu,
											buf);
									lat.SetTextColor(1);
								...
								if (!dovsE)
									calibRecoFit[0]->SetPoint(np, genEn[iE],
											lres.mean);
								else
									calibRecoFit[0]->SetPoint(np,
											E(genEn[iE], eta[ieta]), lres.mean);
								calibRecoFit[0]->SetPointError(np, 0.0,
										lres.meanerr);
							...
							TPad *upper = plotCalibration(calibRecoFit[0], lpad,
									true, calibRecoDelta[0], unit,
									calib[ipu][ieta][0], calibErr[ipu][ieta][0],
									offset[ipu][ieta][0],
									offsetErr[ipu][ieta][0], eta[ieta], dovsE);
							upper->cd();
							char buf[500];
							...
							for (unsigned iE(0); iE < nGenEn; ++iE) {
								...
								for (unsigned iL(0); iL < nLayers; ++iL) {
									for (unsigned i(0); i < 5; ++i) {
										...
										ltree[ieta][ipu][oldIdx[iE]]->SetBranchAddress(
												label.str().c_str(),
												&energySR[iL][i]);
										label.str("");
										label << "subtractedenergy_" << iL
												<< "_SR" << i;
										ltree[ieta][ipu][oldIdx[iE]]->SetBranchAddress(
												label.str().c_str(),
												&subtractedenergySR[iL][i]);
									}
								}

								for (unsigned ievt(0); ievt < nEvts; ++ievt) {//loop on entries

									ltree[ieta][ipu][oldIdx[iE]]->GetEntry(
											ievt);

									Etot[oldIdx[iE]] = 0;
									Esmear[oldIdx[iE]] = 0;
									for (unsigned iL(0); iL < nLayers; ++iL) {
										double fact = absWeight(iL,
												etaval[ieta]);
										Etot[oldIdx[iE]] += fact
												* subtractedenergySR[iL][4];
										Esmear[oldIdx[iE]] += fact
												* subtractedenergySR[iL][4]
												* smearFact[iL];
									}
						...
						saveName.str("");
						saveName << plotDir << "/Ereco_eta" << eta[ieta];
						saveName << "_SR7" << "_IC" << ICval[ic] << "_try"
								<< ip;
						mycEtot->Update();
						mycEtot->Print(
