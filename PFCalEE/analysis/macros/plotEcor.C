...
int plotEcor() {    //main
	...
	unsigned ElistHGCAL[] = { 15, 20, 25, 30, 40, 50, 60, 80, 100, 150, 200,
			300, 400, 500 };  //150,200,300,500};
	unsigned ElistCalice[] = { 10, 15, 18, 20, 25, 30, 35, 40, 45, 50, 60, 80 };

	unsigned genEn[
			doCalice ?
					sizeof(ElistCalice) / sizeof(unsigned) :
					sizeof(ElistHGCAL) / sizeof(unsigned)];
	if (doCalice) {
		for (unsigned ie(0); ie < sizeof(ElistCalice) / sizeof(unsigned);
				++ie) {
	...
	unsigned rebin[20] = { 4, 4, 4, 6, 6, 6, 6, 8, 8, 10, 10, 10, 100, 100, 100,
			6, 6, 6, 6, 6 };
	...
	TCanvas *mycPU = 0;
	if (isPU)
		mycPU = new TCanvas("mycPU", "mycPU", 1500, 1000);
	...
	for (unsigned iV(0); iV < nV; ++iV) {  //loop on versions
		for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
			...
			if (scenario[iS].find("PU") != scenario[iS].npos)
				isPU = true;

			TString plotDir = "../PLOTS/gitV00-02-04/version" + version[iV]
					+ "/" + scenario[iS] + "/";
			//plotDir += "noWeights/";
			...
			for (unsigned iSm(0); iSm < nSmear; ++iSm) {      //loop on smear
				...
				TH2F *p_meanFrac = new TH2F("p_meanFrac",
						";layer;gen E (GeV);<E_{layer}>/E_{tot}", nLayers, 0,
						nLayers, nGenEn, 0, nGenEn);
				TH2F *p_rmsFrac = new TH2F("p_rmsFrac",
						";layer;gen E (GeV);#sigma(E_{layer}/E_{tot})", nLayers,
						0, nLayers, nGenEn, 0, nGenEn);

				bool isG4File = false;
				for (unsigned iE(0); iE < nGenEn; ++iE) {

					if (scenario[iS].find("PedroPU") != scenario[iS].npos)
						genEn[iE] = iE;
					...
					if (doShower && doCalice)
						linputStr << plotDir << "validateCalice_e" << genEn[iE]
								<< pSuffix << ".root";
					else if (doShower)
						linputStr << plotDir << "globalCompensation_e"
								<< genEn[iE] << pSuffix << ".root";
					else
						linputStr << plotDir << "validation_e" << genEn[iE]
								<< pSuffix << ".root";
					inputFile = TFile::Open(linputStr.str().c_str());
					if (!inputFile) {
						std::cout << " -- Error, input file " << linputStr.str()
								<< " cannot be opened. Exiting..." << std::endl;
						return 1;
					} else
						std::cout << " -- File " << inputFile->GetName()
								<< " sucessfully opened." << std::endl;

					if (isPU)
						rebin[iE] = 1;      //rebin[iE]*4;
					...
					if (doFrac) {
						for (unsigned iL(0); iL < nLayers; ++iL) {
							...
							p_Efrac[iE][iL] = (TH1F*) gDirectory->Get(
									lName.str().c_str());
							if (!p_Efrac[iE][iL]) {
								std::cout
										<< " -- ERROR, pointer for histogram Efrac is null for energy "
										<< genEn[iE] << " layer: " << iL
										<< ". Exiting..." << std::endl;
								return 1;
							}
							...
							p_meanFrac->SetBinContent(iL + 1, iE + 1,
									p_Efrac[iE][iL]->GetMean());
							p_rmsFrac->SetBinContent(iL + 1, iE + 1,
									p_Efrac[iE][iL]->GetRMS());

							if (genEn[iE] == 30 && iL < 36) {
								mycL->cd(iL + 1);
								char buf[500];
								sprintf(buf, "E=%d GeV, layer %d", genEn[iE],
										iL);
						...
						if (genEn[iE] == 30) {
							saveName.str("");
							saveName << plotDir << "/SimEfraction_" << genEn[iE]
									<< "GeV";
							mycL->Update();
					...
					//if (doShower) lName << "p_Eshower_" << genEn[iE] << "_" << 5;
						...
						lName << "p_Esim" << pDetector;
					p_Etotal[iE] = (TH1F*) gDirectory->Get(lName.str().c_str());
					if (!p_Etotal[iE]) {
						std::cout << " -- ERROR, pointer for histogram "
								<< lName.str() << " is null. Exiting..."
								<< std::endl;
						return 1;
					}
					p_Etotal[iE]->Sumw2();
					...
					std::cout << " --- Sim E = entries "
							<< p_Etotal[iE]->GetEntries() << " mean "
							<< p_Etotal[iE]->GetMean() << " rms "
							<< p_Etotal[iE]->GetRMS() << " overflows "
							<< p_Etotal[iE]->GetBinContent(
									p_Etotal[iE]->GetNbinsX() + 1) << std::endl;

					//p_Etotal[iE]->Rebin(rebin[iE]);
					p_Etotal[iE]->Rebin(
							genEn[iE] < 25 ? rebinSim :
							genEn[iE] < 80 ? 2 * rebinSim :
							genEn[iE] < 250 ? 4 * rebinSim : 8 * rebinSim);
					if (doCalice)
						p_Etotal[iE]->Rebin(
								genEn[iE] < 50 ? rebinSim :
								genEn[iE] < 150 ? 2 * rebinSim :
								genEn[iE] < 250 ? 4 * rebinSim : 8 * rebinSim);
					...
					if (!p_Ereco[iE]) {
						std::cout
								<< " -- ERROR, pointer for histogram Ereco is null. Running on G4 file before digitizer."
								<< std::endl;
						isG4File = true;
					} else {
						if (doReco) {
							std::cout << " --- Reco E = entries "
									<< p_Ereco[iE]->GetEntries() << " mean "
									<< p_Ereco[iE]->GetMean() << " rms "
									<< p_Ereco[iE]->GetRMS() << " overflows "
									<< p_Ereco[iE]->GetBinContent(
											p_Etotal[iE]->GetNbinsX() + 1)
									<< std::endl;

							//p_Ereco[iE]->Rebin(rebin[iE]);
							p_Ereco[iE]->Rebin(
									genEn[iE] < 50 ? rebinReco / 2 :
									genEn[iE] < 150 ?
											rebinReco : 4 * rebinReco);
						} else
							isG4File = true;
					}
				...
				if ((version[iV] == "21" || version[iV] == "22")
						&& pDetector == "ECAL")
					pDetector = "HCAL";
				if (version[iV] == "21" && pDetector == "Total")
					pDetector = "SiSci";
				if (version[iV] == "23" && pDetector == "Total")
					pDetector = "Calice";

				if (isPU) {      //isPU
					p_EtotalPU = new TH1F("p_EtotalPU", ";Etot (MeV)", 100,
							p_Etotal[0]->GetBinLowEdge(1),
							p_Etotal[nGenEn - 1]->GetBinLowEdge(
									p_Etotal[nGenEn - 1]->GetNbinsX() + 1));
					p_EtotalPU->Sumw2();
					for (unsigned iE(0); iE < nGenEn; ++iE) {
						for (int iB(1); iB < p_Etotal[iE]->GetNbinsX() + 1;
								++iB) {      //loop on bins
							p_EtotalPU->Fill(p_Etotal[iE]->GetBinCenter(iB),
									p_Etotal[iE]->GetBinContent(iB));
						}
						if (!isG4File) {
							if (iE == 0) {
								p_ErecoPU = new TH1F("p_ErecoPU",
										";Etot (MIPs)", 100,
										p_Ereco[0]->GetBinLowEdge(1),
										p_Ereco[nGenEn - 1]->GetBinLowEdge(
												p_Ereco[nGenEn - 1]->GetNbinsX()
														+ 1));
								p_ErecoPU->Sumw2();
							}
							for (int iB(1); iB < p_Ereco[iE]->GetNbinsX() + 1;
									++iB) {      //loop on bins
								p_ErecoPU->Fill(p_Ereco[iE]->GetBinCenter(iB),
										p_Ereco[iE]->GetBinContent(iB));
							}
						}
					}
					...
					if (!isG4File) {
						...
						saveName.str("");
						saveName << plotDir << "/PUTotalRecoE_smear"
								<< smearOption;
						mycPU->Update();
				...
				TGraphErrors *calibFit = (TGraphErrors *) calib->Clone(
						"calibFit");
				TGraphErrors *deltaFit = (TGraphErrors *) calib->Clone(
						"deltaFit");
				TGraphErrors *resoFit = (TGraphErrors *) calib->Clone(
						"resoFit");
				TGraphErrors *calibReco = (TGraphErrors *) calib->Clone(
						"calibReco");
				calibReco->SetMarkerStyle(22);
				calibReco->SetMarkerColor(2);
				calibReco->SetLineColor(2);
				TGraphErrors *resoReco = (TGraphErrors *) calibReco->Clone(
						"resoReco");
				TGraphErrors *calibRecoFit = (TGraphErrors *) calibReco->Clone(
						"calibRecoFit");
				TGraphErrors *deltaRecoFit = (TGraphErrors *) calibReco->Clone(
						"deltaRecoFit");
				TGraphErrors *resoRecoFit = (TGraphErrors *) calibReco->Clone(
						"resoRecoFit");
				...
				for (unsigned iE(0); iE < nGenEn; ++iE) {
					...
					mycE->cd(iE + 1);
					gStyle->SetOptFit(0);
					double eMin = p_Etotal[iE]->GetMean()
							- 10 * p_Etotal[iE]->GetRMS();
					double eMax = p_Etotal[iE]->GetMean()
							+ 10 * p_Etotal[iE]->GetRMS();
					...
					p_Etotal[iE]->Fit("gaus", "LR+", "same",
							fitResult->GetParameter(1)
									- 2 * fitResult->GetParameter(2),
							fitResult->GetParameter(1)
									+ 2 * fitResult->GetParameter(2));
					fitResult = p_Etotal[iE]->GetFunction("gaus");
					TLatex lat;
					double latx = std::max(0.,
							p_Etotal[iE]->GetMean()
									- 5 * p_Etotal[iE]->GetRMS());
					double laty = p_Etotal[iE]->GetMaximum();
					sprintf(buf, "<E> = %3.2f %s", p_Etotal[iE]->GetMean(),
							doMIPconv ? "MIP" : unitStr);
					lat.DrawLatex(latx, laty * 0.9, buf);
					sprintf(buf, "RMS = %3.2f #pm %3.1f %s",
							p_Etotal[iE]->GetRMS(), p_Etotal[iE]->GetRMSError(),
							doMIPconv ? "MIP" : unitStr);
					lat.DrawLatex(latx, laty * 0.8, buf);
					sprintf(buf, "RMS/mean = %3.2f",
							p_Etotal[iE]->GetRMS() / p_Etotal[iE]->GetMean());
					lat.DrawLatex(latx, laty * 0.7, buf);
					sprintf(buf, "<Efit> = %3.2f +/- %3.2f %s",
							fitResult->GetParameter(1),
							fitResult->GetParError(1),
							doMIPconv ? "MIP" : unitStr);
					lat.DrawLatex(latx, laty * 0.6, buf);
					sprintf(buf, "RMSfit = %3.2f +/- %3.2f %s",
							fitResult->GetParameter(2),
							fitResult->GetParError(2),
							doMIPconv ? "MIP" : unitStr);
					lat.DrawLatex(latx, laty * 0.5, buf);
					sprintf(buf, "RMS/meanfit = %3.2f",
							fitResult->GetParameter(2)
									/ fitResult->GetParameter(1));
					lat.DrawLatex(latx, laty * 0.4, buf);

					sprintf(buf, "#chi^{2}/N = %3.2f/%d",
							fitResult->GetChisquare(), fitResult->GetNDF()); //,fitResult->GetChisquare()/fitResult->GetNDF());
					lat.DrawLatex(latx, laty * 0.3, buf);

					Int_t np = calib->GetN();
					calib->SetPoint(np, genEn[iE],
							p_Etotal[iE]->GetMean() / G4MIPtoGeV);
					calib->SetPointError(np, 0.0,
							p_Etotal[iE]->GetMeanError() / G4MIPtoGeV);
					reso->SetPoint(np, doVsE ? genEn[iE] : 1 / sqrt(genEn[iE]),
							p_Etotal[iE]->GetRMS() / p_Etotal[iE]->GetMean());
					reso->SetPointError(np, 0,
							p_Etotal[iE]->GetRMSError()
									/ p_Etotal[iE]->GetMean());
					calibFit->SetPoint(np, genEn[iE],
							fitResult->GetParameter(1) / G4MIPtoGeV);
					calibFit->SetPointError(np, 0.0,
							fitResult->GetParError(1) / G4MIPtoGeV);
					deltaFit->SetPoint(np, genEn[iE],
							(((fitResult->GetParameter(1) - offset) / MIPtoGeV)
									- genEn[iE]) / genEn[iE]);
					deltaFit->SetPointError(np, 0.0,
							fitResult->GetParError(1) / MIPtoGeV * 1.
									/ genEn[iE]);
					resoFit->SetPoint(np,
							doVsE ? genEn[iE] : 1 / sqrt(genEn[iE]),
							fitResult->GetParameter(2)
									/ fitResult->GetParameter(1));
					double errFit =
							fitResult->GetParameter(2)
									...
									* sqrt(
											pow(
													fitResult->GetParError(2)
															/ fitResult->GetParameter(
																	2), 2)
													+ pow(
															fitResult->GetParError(
																	1)
																	/ fitResult->GetParameter(
																			1),
															2));
				...
				if (!isG4File) {
					...
					for (unsigned iE(0); iE < nGenEn; ++iE) {
						...
						p_Ereco[iE]->GetXaxis()->SetRangeUser(
								p_Ereco[iE]->GetMean()
										- 10 * p_Ereco[iE]->GetRMS(),
								p_Ereco[iE]->GetMean()
										+ 10 * p_Ereco[iE]->GetRMS());
						p_Ereco[iE]->Draw("PE");
						char buf[500];
						sprintf(buf, "%s, E=%d GeV", particle.c_str(),
								genEn[iE]);
						...
						if (doLinCor) {
							fitResult->SetParameter(1,
									(fitResult->GetParameter(1) - linOffset)
											/ linScale);
						}
						TLatex lat;
						double latx = std::max(0.,
						...
						sprintf(buf, "<E> = %3.2f %s", p_Ereco[iE]->GetMean(),
								doMIPconv ? "MIP" : unitStr);
						lat.DrawLatex(latx, laty * 0.9, buf);
						sprintf(buf, "RMS = %3.2f #pm %3.1f %s",
								p_Ereco[iE]->GetRMS(),
								p_Ereco[iE]->GetRMSError(),
								doMIPconv ? "MIP" : unitStr);
						lat.DrawLatex(latx, laty * 0.8, buf);
						sprintf(buf, "RMS/mean = %3.2f",
								p_Ereco[iE]->GetRMS() / p_Ereco[iE]->GetMean());
						lat.DrawLatex(latx, laty * 0.7, buf);
						sprintf(buf, "<Efit> = %3.2f +/- %3.2f %s",
								fitResult->GetParameter(1),
								fitResult->GetParError(1),
						...
						calibReco->SetPoint(np, genEn[iE],
								p_Ereco[iE]->GetMean());
						calibReco->SetPointError(np, 0.0,
								p_Ereco[iE]->GetMeanError());
						resoReco->SetPoint(np,
								doVsE ? genEn[iE] : 1 / sqrt(genEn[iE]),
								p_Ereco[iE]->GetRMS() / p_Ereco[iE]->GetMean());
						resoReco->SetPointError(np, 0,
								p_Ereco[iE]->GetRMSError()
										/ p_Ereco[iE]->GetMean());
						calibRecoFit->SetPoint(np, genEn[iE],
								fitResult->GetParameter(1));
						calibRecoFit->SetPointError(np, 0.0,
								fitResult->GetParError(1));
						deltaRecoFit->SetPoint(np, genEn[iE],
								(((fitResult->GetParameter(1) - offset)
										/ MIPtoGeV) - genEn[iE]) / genEn[iE]);
						deltaRecoFit->SetPointError(np, 0.0,
								fitResult->GetParError(1) / MIPtoGeV * 1.
										/ genEn[iE]);
						resoRecoFit->SetPoint(np,
								doVsE ? genEn[iE] : 1 / sqrt(genEn[iE]),
								fitResult->GetParameter(2)
										/ fitResult->GetParameter(1));
						double errFit =
				...
				for (unsigned i = 0; i < imax; i++) {
					if (i == 1 || i == 5)
						...
						myc[i % 4]->cd();
					type[i] = i == 0 ? "calib" : i == 1 ? "calibFit" :
								i == 2 ? "reso" : "resoFit";
					if (!isG4File && i > 3) {
						type[i] = i == 4 ? "calibReco" :
									i == 5 ? "calibRecoFit" :
									i == 6 ? "resoReco" : "resoRecoFit";
						addNoiseTerm = true;
					}
					...
					TGraphErrors * gr = (i == 0 ? calib : i == 1 ? calibFit :
											i == 2 ? reso : resoFit);
					...
					if (!isG4File && i > 3)
						gr = i == 4 ? calibReco : i == 5 ? calibRecoFit :
								i == 6 ? resoReco : resoRecoFit;
					if (i == 3 && nSmear > 1)
						gr->SetTitle(smearFact[iSm]);
					else
						gr->SetTitle("");
					if (i % 4 > 1)
						gr->SetMaximum(
								std::max(resoFit->GetMaximum(),
										resoRecoFit->GetMaximum()));
					gr->SetMinimum(0);
					gr->Draw(i < 4 ? "ap" : "p");
					...
					if (i < 2 || (!isG4File && (i == 4 || i == 5))) {
						...
						if (i > 2 || (doMIPconv && i > 0))
							gr->GetYaxis()->SetTitle(
									"Average energy deposited [MIPs]");
						else
							gr->GetYaxis()->SetTitle(
						...
						gr->GetXaxis()->SetTitle(
								doVsE ? "E (GeV)" : "1/#sqrt{Beam energy} [1/#sqrt{GeV}]");
					...
					if (i < 2 || (!isG4File && (i == 4 || i == 5))) {
						...
						if (i < 2)
							lat.DrawLatex(genEn[0],
									gr->GetYaxis()->GetXmax() * 0.9, buf);
						sprintf(buf, "a = %3.2f #pm %3.2f %s",
								fitFunc->GetParameter(0),
								fitFunc->GetParError(0), unitStr);
						lat.DrawLatex(genEn[0] + i / 4 * 50,
								gr->GetYaxis()->GetXmax()
										* (0.8 - i / 2 * 0.25), buf);
						sprintf(buf, "b = %3.2f #pm %3.2f %s/GeV",
								fitFunc->GetParameter(1),
								fitFunc->GetParError(1), unitStr);
						lat.DrawLatex(genEn[0] + i / 4 * 50,
								gr->GetYaxis()->GetXmax()
										* (0.7 - i / 2 * 0.25), buf);
						sprintf(buf, "#chi^{2}/N = %3.2f/%d",
								fitFunc->GetChisquare(), fitFunc->GetNDF());//,fitFunc->GetChisquare()/fitFunc->GetNDF());
						lat.DrawLatex(genEn[0] + i / 4 * 50,
								gr->GetYaxis()->GetXmax()
										* (0.6 - i / 2 * 0.25), buf);
						...
						TF1 *fitFunc2;
						if (doVsE) {
							fitFunc2 = new TF1("reso", "sqrt([0]/x+[1])",
									gr->GetXaxis()->GetXmin(),
									gr->GetXaxis()->GetXmax());
							if (addNoiseTerm)
								fitFunc2 = new TF1("reso",
										"sqrt([0]/x+[1]+[2]/(x*x))",
										gr->GetXaxis()->GetXmin(),
										gr->GetXaxis()->GetXmax());
						} else {
							fitFunc2 = new TF1("reso", "sqrt([0]*x*x+[1])",
									gr->GetXaxis()->GetXmin(),
									gr->GetXaxis()->GetXmax());
							if (addNoiseTerm)
								fitFunc2 = new TF1("reso",
										"sqrt([0]*x*x+[1]+[2]*x*x*x*x)",
										gr->GetXaxis()->GetXmin(),
										gr->GetXaxis()->GetXmax());
						}
						...
						if (addNoiseTerm) {
							if (!isEM)
								...
								fitref->SetParameter(2, 0.06 * 0.06);
							if (!isEM)
								fitFunc2->SetParameter(2,
										doCalice ? 0.18 * 0.18 : 0.02 * 0.02);
							else
								fitFunc2->SetParameter(2, 0.06 * 0.06);
							fitFunc2->SetParLimits(2, 0, 2);
							if (!isEM)
								fitFunc2->FixParameter(2,
										doCalice ? 0.18 * 0.18 : 0.02 * 0.02);
							else
								fitFunc2->FixParameter(2, 0.06 * 0.06);
						}
						if (i < 4) {
						...
						gr->Fit(fitFunc2, "RME");
						sigmaStoch[iSm][iS][i] = sqrt(
								fitFunc2->GetParameter(0));
						sigmaStochErr[iSm][iS][i] = fitFunc2->GetParError(0)
								/ (2 * sigmaStoch[iSm][iS][i]);
						sigmaConst[iSm][iS][i] = sqrt(
								fitFunc2->GetParameter(1));
						sigmaConstErr[iSm][iS][i] = fitFunc2->GetParError(1)
								/ (2 * sigmaConst[iSm][iS][i]);
						if (addNoiseTerm) {
							sigmaNoise[iSm][iS][i] = sqrt(
									fitFunc2->GetParameter(2));
							sigmaNoiseErr[iSm][iS][i] = fitFunc2->GetParError(2)
									/ (2 * sigmaNoise[iSm][iS][i]);
						}
						//TLatex lat;
						double Emin =
								doVsE ? (doCalice ? 55 : 300) : 1
												/ sqrt(genEn[nGenEn - 1]);
						TPaveText *pt = new TPaveText(Emin,
								gr->GetYaxis()->GetXmin() * 1.8,
								genEn[nGenEn - 1],
								gr->GetYaxis()->GetXmax() * 0.75);
						pt->SetFillColor(10);
						pt->SetLineColor(1);
						pt->SetShadowColor(0);
						pt->SetTextFont(42);
						...
						sprintf(buf,
								"#frac{#sigma}{E} = #frac{s}{#sqrt{E}} #oplus c #oplus #frac{n}{E}");
						if (i < 4)
							pt->AddText(buf);
						//(Emin,gr->GetYaxis()->GetXmax()*0.9,buf);
						//else pt->AddText(doVsE?Emin+20:Emin+0.1,gr->GetYaxis()->GetXmax()*0.95,buf);
						sprintf(buf, "s=%3.3f #pm %3.3f",
								sigmaStoch[iSm][iS][i],
								sigmaStochErr[iSm][iS][i]);
						if (i < 4)
							pt->AddText(buf);
						//(Emin,gr->GetYaxis()->GetXmax()*(0.8-i/6*0.3),buf);
						sprintf(buf, "c=%3.3f #pm %3.3f",
								sigmaConst[iSm][iS][i],
								sigmaConstErr[iSm][iS][i]);
						if (i < 4)
							pt->AddText(buf);
						...
						sprintf(buf, "#chi^{2}/N = %3.1f/%d",
								fitFunc2->GetChisquare(), fitFunc2->GetNDF());//,fitFunc2->GetChisquare()/fitFunc2->GetNDF());
						if (i < 4)
							pt->AddText(buf);
						//(Emin,gr->GetYaxis()->GetXmax()*(0.65-i/6*0.3),buf);

						//lat.SetTextColor(4);
						sprintf(buf, "CALICE s=%3.2f, c=%3.2f",
								sqrt(fitref->GetParameter(0)),
								sqrt(fitref->GetParameter(1)));
						if (addNoiseTerm) {
							sprintf(buf, "CALICE s=%3.2f, c=%3.2f, n=%3.2f",
									sqrt(fitref->GetParameter(0)),
									sqrt(fitref->GetParameter(1)),
									sqrt(fitref->GetParameter(2)));
						}
						if (i < 4)
							pt->Draw();
						TLatex lat;
						...
						if (i > 3 && doCalice) {
							lat.DrawLatex(8, gr->GetYaxis()->GetXmin() * 0.9,
									buf);
						}
					}
					...
					lat.DrawLatexNDC(0.02, 0.02,
							"HGCAL Geant4 standalone simulation");
					...
					if (doShower) {
						myc[i % 4]->Print(
								plotDir + "/" + type[i] + "_Shower.pdf");
						myc[i % 4]->Print(
								plotDir + "/" + type[i] + "_Shower.png");
					} else if (i % 4 > 1 && doVsE) {
		...
		for (unsigned iS(0); iS < nS; ++iS) {
			for (unsigned i(2); i < MAX; ++i) {
				for (unsigned iSm(0); iSm < nSmear; ++iSm) {	//loop on smear
					if (i == 4 || i == 5)
						continue;
					std::cout << scenario[iS] << " & " << type[i] << " & "
							<< std::setprecision(3) << "$"
							<< sigmaStoch[iSm][iS][i] << "\\pm"
							<< sigmaStochErr[iSm][iS][i] << "$ & " << "$"
							<< sigmaConst[iSm][iS][i] << "\\pm"
							<< sigmaConstErr[iSm][iS][i] << "$";
					if (addNoiseTerm)
						std::cout << " & $" << sigmaNoise[iSm][iS][i] << "\\pm"
								<< sigmaNoiseErr[iSm][iS][i] << "$";
					std::cout << "\n";
				}
				std::cout << "\\hline\n";
			}
	...
	if (nSmear > 1) {
		...
		TF1 *BE = new TF1("BE", "sqrt([0]*[0] + pow(x/100.*1/sqrt([1]),2))", 0,
				20);
		...
		TGaxis *ys = new TGaxis(22, 0, 22, 0.04, lShift, lShift + 0.04, 510,
				"+L");
		...
		lat.SetTextColor(2);
		char buf[500];
		sprintf(buf, "<s> = %1.4f #pm %1.4f", pol0->GetParameter(0) + lShift,
				pol0->GetParError(0));
		lat.DrawLatex(3, 0.037, buf);

		lat.SetTextColor(1);
		sprintf(buf, "c = c_{0} #oplus #frac{x}{#sqrt{n}}, n=%3.0f #pm %3.0f",
				BE->GetParameter(1), BE->GetParError(1));
		...
		saveName.str("");
		saveName << "../PLOTS/gitV00-01-02/version" << version[0] << "/"
				<< scenario[0] << "/Intercalibration";
		mycF->Update();
		mycF->Print((saveName.str() + ".png").c_str());
		mycF->Print((saveName.str() + ".pdf").c_str());
	}
