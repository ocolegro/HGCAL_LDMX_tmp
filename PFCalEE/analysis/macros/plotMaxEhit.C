...
int plotMaxEhit() {
	...
	unsigned genEn[] = { 5, 10, 15, 20, 25, 30, 40, 50, 60, 100, 150, 200, 300,
			400, 500, 1000, 2000 };
	...
	for (unsigned iV(0); iV < nV; ++iV) { //loop on versions
		for (unsigned iS(0); iS < nS; ++iS) { //loop on scenarios
			...
			TString plotDir = "../PLOTS/gitV00-02-02/version" + version[iV]
					+ "/" + scenario[iS] + "/";
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				linputStr << plotDir << "validation_" << pSuffix << "um_e"
						<< genEn[iE] << ".root";
				...
				for (unsigned iH(0); iH < nHists; ++iH) {
					...
					sprintf(buf, "Geant4 Standalone, e- %d GeV, pad size = %s",
							genEn[iE], label[iH].c_str());
					...
					lsave << plotDir << pSuffix << "um/maxEhit_" << label[iH]
							<< "_" << genEn[iE] << "GeV.pdf";
			...
			for (unsigned i(0); i < 2; ++i) {
				...
				for (unsigned iH(0); iH < nHists; ++iH) {
					...
					if (i == 0)
						sprintf(buf,
								"<maxE> = (%3.0f #pm %3.0f)  + (%3.1f #pm %3.1f)#times E + (%1.0e#pm%1.0e) #times E^{2}",
								fitfunc[iH]->GetParameter(0),
								fitfunc[iH]->GetParError(0),
								fitfunc[iH]->GetParameter(1),
								fitfunc[iH]->GetParError(1),
								fitfunc[iH]->GetParameter(2),
								fitfunc[iH]->GetParError(2));
					else
						sprintf(buf,
