...
int plotXYZ() {    //main  
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		...
		lName << "myc" << iC;
		myc[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1300,
				800);
	}
	...
	for (unsigned iV(0); iV < nV; ++iV) {  //loop on versions
		for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
			...
			TString inputDir =
					"/afs/cern.ch/work/a/amagnan/public/HGCalGeant4/git_V00-03-06/version_"
							+ version[iV] + "/model_2/" + scenario[iS]
							+ "/BON/";
			...
			for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on events
				std::cout << " -- Processing event " << event[ievt]
						<< std::endl;

				std::ostringstream lcutSmall;
				lcutSmall
						<< "(HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))*(event_=="
						<< event[ievt]
						<< " && (TMath::Sqrt(TMath::Power(HGCSSRecoHitVec.xpos_,2)+TMath::Power(HGCSSRecoHitVec.ypos_,2))<minR(HGCSSRecoHitVec.layer_)) && (TMath::Sqrt(TMath::Power(HGCSSRecoHitVec.xpos_,2)+TMath::Power(HGCSSRecoHitVec.ypos_,2))>"
						<< minRadius
						<< ") && (HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))>"
						<< mipThresh << ")";
				std::ostringstream lcut;
				lcut
						<< "(HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))*(event_=="
						<< event[ievt]
						<< " && (TMath::Sqrt(TMath::Power(HGCSSRecoHitVec.xpos_,2)+TMath::Power(HGCSSRecoHitVec.ypos_,2))>=minR(HGCSSRecoHitVec.layer_)) && (HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))>"
						<< mipThresh << ")";
				...
				for (unsigned iL(0); iL < nLayers; ++iL) {
					...
					lvar.str("");
					lvar << "HGCSSRecoHitVec.ypos_:HGCSSRecoHitVec.xpos_>>"
							<< lName.str();
					std::ostringstream lcutLayer;
					lcutLayer
							<< "(HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))*(event_=="
							<< event[ievt] << " && HGCSSRecoHitVec.layer_=="
							<< iL
							<< " && (TMath::Sqrt(TMath::Power(HGCSSRecoHitVec.xpos_,2)+TMath::Power(HGCSSRecoHitVec.ypos_,2))>=minR(HGCSSRecoHitVec.layer_)) && (HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))>"
							<< mipThresh << ")";
					...
					p_xySmall[iL] = new TH2F(lName.str().c_str(),
							";x(mm);y(mm)", nBins, min, max, nBins, min, max);
					...
					lcutLayer
							...
							<< ") && (HGCSSRecoHitVec.energy_*absWeight(HGCSSRecoHitVec.layer_))>"
							<< mipThresh << ")";
					tree->Draw(lvar.str().c_str(),
							lcutLayerSmall.str().c_str());

					double Etot = p_xy[iL]->GetMaximum();
					if (Etot > EmaxEcal && iL < nEcalLayers)
						EmaxEcal = Etot;
					if (Etot > EmaxHcal && iL >= nEcalLayers)
						EmaxHcal = Etot;

				}  //loop on layers

				std::cout << " -- max E found: ECAL: " << EmaxEcal << ", HCAL: "
						<< EmaxHcal << std::endl;

				nBins = 339;
				TH2F *p_xytot = new TH2F("p_xytot", ";x(mm);y(mm)", nBins, min,
						max, nBins, min, max);
				...
				TH2F *p_xztot = new TH2F("p_xztot", ";x(mm);z(mm)", nBins, min,
						max, nZ, minZ, maxZ);
				lvar.str("");
				lvar << "HGCSSRecoHitVec.zpos_:HGCSSRecoHitVec.xpos_>>p_xztot";
				tree->Draw(lvar.str().c_str(), lcut.str().c_str());

				TH2F *p_zytot = new TH2F("p_zytot", ";z(mm);y(mm)", nZ, minZ,
						maxZ, nBins, min, max);
				lvar.str("");
				...
				lvar.str("");
				lvar
						<< "HGCSSRecoHitVec.ypos_:HGCSSRecoHitVec.xpos_>>p_xytotSmall";
				tree->Draw(lvar.str().c_str(), lcutSmall.str().c_str());

				TH2F *p_xztotSmall = new TH2F("p_xztotSmall", ";x(mm);z(mm)",
						nBins, min, max, nZ, minZ, maxZ);
				lvar.str("");
				lvar
						<< "HGCSSRecoHitVec.zpos_:HGCSSRecoHitVec.xpos_>>p_xztotSmall";
				tree->Draw(lvar.str().c_str(), lcutSmall.str().c_str());

				TH2F *p_zytotSmall = new TH2F("p_zytotSmall", ";z(mm);y(mm)",
						nZ, minZ, maxZ, nBins, min, max);
				lvar.str("");
				lvar
						<< "HGCSSRecoHitVec.ypos_:HGCSSRecoHitVec.zpos_>>p_zytotSmall";
				tree->Draw(lvar.str().c_str(), lcutSmall.str().c_str());
				...
				myc[iS]->Update();
				saveName.str("");
				saveName << plotDir << "/xzSimHits_evt" << event[ievt]
						<< "_mipThresh" << mipThresh;
				...
				myc[iS]->Update();
				saveName.str("");
				saveName << plotDir << "/zySimHits_evt" << event[ievt]
						<< "_mipThresh" << mipThresh;
				myc[iS]->Print((saveName.str() + ".png").c_str());
				myc[iS]->Print((saveName.str() + ".pdf").c_str());
				...
				myc[iS]->Update();
				saveName.str("");
				saveName << plotDir << "/xySimHits_evt" << event[ievt]
						<< "_mipThresh" << mipThresh;
				myc[iS]->Print((saveName.str() + ".png").c_str());
				myc[iS]->Print((saveName.str() + ".pdf").c_str());
				...
				for (unsigned iL(1); iL < nLayers; ++iL) {	//loop on layers
					...
					gPad->SetLogz(1);
					p_xy[iL]->SetMaximum(
							iL < nEcalLayers ? EmaxEcal : EmaxHcal);
				...
				saveName.str("");
				saveName << plotDir << "/xySimHits_ECAL_evt" << event[ievt]
						<< "_subset";
				mycECAL->Update();
				mycECAL->Print((saveName.str() + ".png").c_str());
				mycECAL->Print((saveName.str() + ".pdf").c_str());
				saveName.str("");
				saveName << plotDir << "/xySimHits_HCAL_evt" << event[ievt]
						<< "_subset2";
				mycHCAL->Update();
				mycHCAL->Print((saveName.str() + ".png").c_str());
				mycHCAL->Print((saveName.str() + ".pdf").c_str());
				...
				for (unsigned iL(1); iL < nEcalLayers; ++iL) { //loop on ecal layers
					...
					//p_xy[iL]->Draw("LEGO2z");
					p_xy[iL]->GetXaxis()->SetRangeUser(minXecal[ievt],
							maxXecal[ievt]);
					p_xy[iL]->GetYaxis()->SetRangeUser(minYecal[ievt],
							maxYecal[ievt]);
					p_xy[iL]->Draw("colz");
