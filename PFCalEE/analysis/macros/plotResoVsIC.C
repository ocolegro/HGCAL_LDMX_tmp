...
int plotResoVsIC() {
	...
	TGraphErrors *samplingSR7 = (TGraphErrors *) constantSR7->Clone(
			"samplingSR7");
	...
	for (unsigned ic(0); ic < nIC; ++ic) { //loop on intercalib
		...
		constant->SetPointError(ic, 0,
				fit->GetParameter(1) * fit->GetParError(1) / cval);
		...
		constantSR7->SetPointError(ic, 0,
				fit7->GetParameter(1) * fit7->GetParError(1) / cval);
	...
	sprintf(buf, "c_{ic}=#frac{x}{#sqrt{n}}, n=%3.1f #pm %3.1f",
