...
bool checkHistPointer(TH1 *pointer, std::string name) {
	if (!pointer) {
		std::cout << " -- ERROR, pointer for histogram " << name
				<< " is null. Exiting..." << std::endl;
		return false;
	}
	return true;
}
...
int plotGlobalCor() { //main  
	...
	unsigned genEn[] = {  //10,30,80};
			10, 15, 18, 20, 25, 30, 35, 40, 45, 50, 60, 80 }; //,100,200,300,400,500
	//,1000,2000};
	...
	for (unsigned iC(0); iC < nCtot; ++iC) {
		lName.str("");
		lName << "Canvas_" << iC;
		myc[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 1000,
				700);
	}
	myc[0]->Divide(3, 2);
	myc[1]->Divide(3, 3);
	...
	for (unsigned iV(0); iV < nV; ++iV) {  //loop on versions
		for (unsigned iS(0); iS < nS; ++iS) {  //loop on scenarios
			...
			unsigned lColor[20] = { 1, 2, 3, 4, 5, 6, 7, 8, 9, 209, 210, 211,
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				lName.str("");
				lName << plotDir << "globalCompensation_e" << genEn[iE]
						<< pSuffix << ".root";
				TFile *inputFile = TFile::Open(lName.str().c_str());
				if (!inputFile) {
					std::cout << " -- Error, input file " << lName.str()
							<< " cannot be opened. Going to next..."
							<< std::endl;
					continue;
					//return 1;
				}
				...
				p_hitSpectrum_lowTail[iE] = (TH1F*) gDirectory->Get(
						lName.str().c_str());
				if (!checkHistPointer(p_hitSpectrum_lowTail[iE], lName.str()))
					return 1;
				p_hitSpectrum_lowTail[iE]->Scale(
						1. / p_hitSpectrum_lowTail[iE]->GetEntries());

				lName.str("");
				lName << "p_hitSpectrum_highTail";
				p_hitSpectrum_highTail[iE] = (TH1F*) gDirectory->Get(
						lName.str().c_str());
				if (!checkHistPointer(p_hitSpectrum_highTail[iE], lName.str()))
					return 1;
				p_hitSpectrum_highTail[iE]->Scale(
						1. / p_hitSpectrum_highTail[iE]->GetEntries());

				lName.str("");
				lName << "p_hitSpectrum_ratio";
				p_hitSpectrum_ratio[iE] =
						(TH1F*) p_hitSpectrum_highTail[iE]->Clone(
								lName.str().c_str());
				p_hitSpectrum_ratio[iE]->Divide(p_hitSpectrum_highTail[iE],
						p_hitSpectrum_lowTail[iE]);

				lName.str("");
				lName << "p_meanHitSpectrum_lowTail";
				p_meanHitSpectrum_lowTail[iE] = (TH1F*) gDirectory->Get(
						lName.str().c_str());
				if (!checkHistPointer(p_meanHitSpectrum_lowTail[iE],
						lName.str()))
					return 1;
				p_meanHitSpectrum_lowTail[iE]->Scale(
						1. / p_meanHitSpectrum_lowTail[iE]->GetEntries());

				lName.str("");
				lName << "p_meanHitSpectrum_highTail";
				p_meanHitSpectrum_highTail[iE] = (TH1F*) gDirectory->Get(
						lName.str().c_str());
				if (!checkHistPointer(p_meanHitSpectrum_highTail[iE],
						lName.str()))
					return 1;
				p_meanHitSpectrum_highTail[iE]->Scale(
						1. / p_meanHitSpectrum_highTail[iE]->GetEntries());

				for (unsigned iLim(0); iLim < nLimits; ++iLim) {
					lName.str("");
					lName << "p_Cglobal_" << iLim;
					p_Cglobal[iE][iLim] = (TH1F*) gDirectory->Get(
							lName.str().c_str());
					if (!checkHistPointer(p_Cglobal[iE][iLim], lName.str()))
						return 1;

					lName.str("");
					lName << "p_Eshower_" << iLim;
					p_Eshower[iE][iLim] = (TH1F*) gDirectory->Get(
							lName.str().c_str());
					if (!checkHistPointer(p_Eshower[iE][iLim], lName.str()))
						return 1;

					lName.str("");
					lName << "p_EvsCglobal_" << iLim;
					p_EvsCglobal[iE][iLim] = (TH2F*) gDirectory->Get(
							lName.str().c_str());
					if (!checkHistPointer(p_EvsCglobal[iE][iLim], lName.str()))
						return 1;
				}
				...
				if (!checkHistPointer(p_Etotal[iE], lName.str()))
					return 1;
				...
				p_hitSpectrum_lowTail[iE]->GetYaxis()->SetTitle(
						"bin Entries / Entries");
				...
				p_hitSpectrum_ratio[iE]->GetYaxis()->SetTitle(
						"Ratio highTail/LowTail");
				...
				if (genEn[iE] == 10) {
					...
					leg->AddEntry(p_hitSpectrum_lowTail[iE],
							"Eevent < Emean-sigma", "L");
					leg->AddEntry(p_hitSpectrum_highTail[iE],
							"Eevent > Emean+sigma", "L");
					leg->Draw("same");
					myc[0]->cd(4);
					gPad->SetGridx(1);
				...
				double minx = std::min(
						p_Eshower[iE][limRef]->GetMean()
								- 5 * p_Eshower[iE][limRef]->GetRMS(),
				...
				p_Eshower[iE][limRef]->SetMaximum(
						std::max(p_Eshower[iE][limRef]->GetMaximum(),
								p_Etotal[iE]->GetMaximum()));
				...
				std::cout << "shower " << p_Eshower[iE][limRef]->GetEntries()
						<< " reco " << p_Etotal[iE]->GetEntries() << std::endl;
				...
				for (unsigned iLim(0); iLim < nLimits; ++iLim) {
					...
					hTmp->GetXaxis()->SetRangeUser(
							hTmp->GetMean() - 5 * hTmp->GetRMS(),
							hTmp->GetMean() + 5 * hTmp->GetRMS());
					if (iLim == limRef) {
						myc[3]->cd(iE + 1);
						hTmp->Draw("PE");
					...
					if (iLim == limRef) {
						...
						calib->SetPoint(npc, fitResult->GetParameter(1),
								genEn[iE]);
						calib->SetPointError(npc, fitResult->GetParError(1),
								0.01 * genEn[iE]);
					}
					Int_t np = reso[iLim]->GetN();
					double lreso = fitResult->GetParameter(2)
				...
				if (genEn[iE] == 40) {
					for (unsigned iLim(0); iLim < nLimits; ++iLim) {
						...
						p_EvsCglobal[iE][iLim]->GetXaxis()->SetRangeUser(
								p_Cglobal[iE][iLim]->GetMean()
										- 5 * p_Cglobal[iE][iLim]->GetRMS(),
								p_Cglobal[iE][iLim]->GetMean()
										+ 5 * p_Cglobal[iE][iLim]->GetRMS());
						p_EvsCglobal[iE][iLim]->GetYaxis()->SetRangeUser(
								p_Etotal[iE]->GetMean()
				...
				reso[nLimits]->SetPoint(np, 1 / sqrt(genEn[iE]),
						p_Etotal[iE]->GetRMS() / p_Etotal[iE]->GetMean());
				reso[nLimits]->SetPointError(np, 0,
						p_Etotal[iE]->GetRMSError() / p_Etotal[iE]->GetMean());
				...
				p_meanHitSpectrum_lowTail[iE]->GetYaxis()->SetTitle(
						"Normalised entries");
			...
			gr->Draw("ap");
			TF1 *fitFunc = new TF1("calibFunc", "[0]*x+[1]*x*x+[2]*x*x*x",
			...
			sprintf(buf, "a = %3.3f #pm %3.3f ", fitFunc->GetParameter(0),
					fitFunc->GetParError(0));
			lat.DrawLatex(genEn[0], gr->GetYaxis()->GetXmax() * (0.8), buf);
			sprintf(buf, "b = %3.3f #pm %3.3f", fitFunc->GetParameter(1),
					fitFunc->GetParError(1));
			lat.DrawLatex(genEn[0], gr->GetYaxis()->GetXmax() * (0.7), buf);
			sprintf(buf, "c = %3.1e #pm %3.1e", fitFunc->GetParameter(2),
			...
			gr->SetMaximum(0.25);
			gr->Draw("ap");
			TF1 *fitFunc2 = new TF1("resoFunc", "sqrt([0]*[0]*x*x+[1]*[1])",
					gr->GetXaxis()->GetXmin(), gr->GetXaxis()->GetXmax());
