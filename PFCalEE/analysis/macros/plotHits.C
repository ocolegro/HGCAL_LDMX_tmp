...
int plotHits() {
	...
	TFile *in3 = TFile::Open(
			"../PLOTS/gitV00-02-03/version23/pi-/validation_e30.root");
	in3->cd();
	...
	std::cout << " -- entries: " << sim->GetEntries() << " "
			<< simnoise->GetEntries() << " " << xtalknoise->GetEntries() << " "
	...
	lat.SetTextColor(2);
	char buf[500];
	sprintf(buf, "<E>=%3.3f #pm %3.3f MIPs", (sim->GetMean()),
			(sim->GetMeanError()));
	//lat.DrawLatexNDC(0.15,0.8,buf);
	sprintf(buf, "#sigma(E)=%3.3f #pm %3.3f MIPs", (sim->GetRMS()),
			(sim->GetRMSError()));
	//lat.DrawLatexNDC(0.15,0.75,buf);
	lat.SetTextColor(4);
	sprintf(buf, "<E>=%3.3f #pm %3.3f MIPs", (rec->GetMean()),
			(rec->GetMeanError()));
	//lat.DrawLatexNDC(0.15,0.7,buf);
	sprintf(buf, "#sigma(E)=%3.3f #pm %3.3f MIPs", (rec->GetRMS()),
			(rec->GetRMSError()));
