...
int plotHiggsMass() { //main
	...
	std::string label[nV] = { "True E, Shower pos", "True E, Vtx smear",
	//"True E, Shower angle",
			"Reco E, True pos", "Reco E, Shower pos", "Reco E, Vtx smear",
	//"Reco E, Shower angle",
			};
	...
	f[0] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/gitV00-02-14/version12/Hgg/dec14pt20/pu0.root");
	f[1] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/gitV00-02-14/version12/Hgg/dec14pt20/pu140.root");
	f[2] =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/gitV00-02-14/version12/Hgg/dec14pt20/pu200.root");
	...
	for (unsigned ipu(0); ipu < nP; ++ipu) {  //loop on pu
		if (!f[ipu]) {
			std::cout << " -- Input file for pu " << puVal[ipu]
					<< " not found ! " << std::endl;
			return 1;
		}
		f[ipu]->cd("HiggsMass");
	...
	for (unsigned iH(0); iH < 2; ++iH) {
		...
		for (unsigned iP(0); iP < nP; ++iP) {
			...
			grMass[iP] = new TGraphErrors(nV, val, mean[iP], valerr,
					meanerr[iP]);
			grSigma[iP] = new TGraphErrors(nV, val, sigma[iP], valerr,
					sigmaerr[iP]);
			...
			for (Int_t k = 0; k < nV; k++) {
				gr[iP]->GetHistogram()->GetXaxis()->SetBinLabel(k + 1,
						label[k].c_str());
			}
		...
		TLatex lat;
		lat.DrawLatexNDC(0.1, 0.92,
				"Pythia gg#rightarrow Higgs, H#rightarrow #gamma#gamma");
		lat.DrawLatexNDC(0.2, 0.85,
				"1.5 < #eta_{#gamma^{1}} < 2.8, p_{T}>20 GeV");
		lat.DrawLatexNDC(0.2, 0.8,
				"1.5 < #eta_{#gamma^{2}} < 2.8, p_{T}>20 GeV");
		myc->Update();
		if (iH == 0)
			myc->Print("PLOTS/SummaryHiggsMass.pdf");
		else {
