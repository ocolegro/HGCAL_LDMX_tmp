...
Result plotOnePoint(unsigned etabin, unsigned pt, unsigned pu) { //main
	...
	plotDir << "../PLOTS/gitV00-02-12/version12/gamma/200um/eta" << etabin
			<< "_et" << pt << "_pu" << pu;

	TFile *file = TFile::Open((plotDir.str() + ".root").c_str());

	if (!file) {
		std::cout << " -- Error, input file " << plotDir.str()
				<< ".root cannot be opened. Skipping..." << std::endl;
		return res;
	} else
		std::cout << " -- input file " << plotDir.str()
				<< ".root successfully opened." << std::endl;
	...
	mycL->Update();
	mycL->Print(
			(plotDir.str() + "/PositionFitSummary" + suffix + ".pdf").c_str());
	...
	mycD->Update();
	mycD->Print(
			(plotDir.str() + "/PositionFitDebug" + suffix + ".pdf").c_str());
	...
	p_angularReso->Draw();
	mycR->cd(2);
	p_impactX_residual->GetXaxis()->SetRangeUser(
			p_impactX_residual->GetMean() - 5 * p_impactX_residual->GetRMS(),
			p_impactX_residual->GetMean() + 5 * p_impactX_residual->GetRMS());
	p_impactX_residual->Draw();
	mycR->cd(5);
	p_impactY_residual->GetXaxis()->SetRangeUser(
			p_impactY_residual->GetMean() - 5 * p_impactY_residual->GetRMS(),
			p_impactY_residual->GetMean() + 5 * p_impactY_residual->GetRMS());
	p_impactY_residual->Draw();
	mycR->cd(3);
	p_tanAngleX_residual->GetXaxis()->SetRangeUser(
			p_tanAngleX_residual->GetMean()
					- 5 * p_tanAngleX_residual->GetRMS(),
			p_tanAngleX_residual->GetMean()
					+ 5 * p_tanAngleX_residual->GetRMS());
	p_tanAngleX_residual->Draw();
	mycR->cd(6);
	p_tanAngleY_residual->GetXaxis()->SetRangeUser(
			p_tanAngleY_residual->GetMean()
					- 5 * p_tanAngleY_residual->GetRMS(),
			p_tanAngleY_residual->GetMean()
					+ 5 * p_tanAngleY_residual->GetRMS());
	...
	for (unsigned ipt = 0; ipt < (runAll ? npt : 1); ++ipt) { //loop on pt values
		for (unsigned ipu(0); ipu < npu; ++ipu) {  //loop on pu values
			...
			for (unsigned iP(0); iP < nP; ++iP) {
				...
				if ((fabs(residual_x[iP]) + residual_x_rms[iP]) > max_pos)
					max_pos = fabs(residual_x[iP]) + residual_x_rms[iP];
				if ((fabs(residual_y[iP]) + residual_y_rms[iP]) > max_pos)
					max_pos = fabs(residual_y[iP]) + residual_y_rms[iP];
			}

			grEvts[ipt][ipu] = new TGraphErrors(nP, etaval[ipu], nEvts, etaerr,
					etaerr);
			grFit[ipt][ipu] = new TGraphErrors(nP, etaval[ipu], fitQuality,
					etaerr, fitQuality_rms);

			grResidualX[ipt][ipu] = new TGraphErrors(nP, etaval[ipu],
					residual_x, etaerr, residual_x_rms);
			grResidualY[ipt][ipu] = new TGraphErrors(nP, etaval[ipu],
					residual_y, etaerr, residual_y_rms);
			grResidualTanX[ipt][ipu] = new TGraphErrors(nP, etaval[ipu],
					residual_tanx, etaerr, residual_tanx_rms);
			grResidualTanY[ipt][ipu] = new TGraphErrors(nP, etaval[ipu],
					residual_tany, etaerr, residual_tany_rms);
			...
			if (runAll)
				...
				mycRA->cd();
			grResidualTanX[ipt][ipu]->SetTitle(
					";#eta;tanA_{reco}-tanA_{truth} (mrad)");
