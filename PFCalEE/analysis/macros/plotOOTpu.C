...
int plotOOTpu() { //main
	...
	std::string histname[nH] = { "ErecooverEtrue", "ErecooverEtrueOOTpast",
			"ErecooverEtrueOOT" };

	std::string labelStr[nH] = { "no PU BX 0", "PU BX -1", "PU BX -1+1-10" };
	...
	lat.DrawLatexNDC(0.3, 0.94,
			"Pythia H#rightarrow#gamma#gamma, 1.5 < #eta^{#gamma} < 2.9");
	...
	for (unsigned ibx(1); ibx < nbx; ++ibx) {
		...
		if (ibx > 3)
			nAbove_perbx[ibx]->GetXaxis()->SetRangeUser(0, 50);
		if (ibx > 8)
			nAbove_perbx[ibx]->GetXaxis()->SetRangeUser(0, 20);
