...
int plotResoVsTries() {
	...
	TH1F *sigma[nE][2];
	TH1F *reso[nE][2];
	TH1F *diffMean = new TH1F("diffMean",
			";#DeltaE/E(400 GeV) - #DeltaE/E(20 GeV)", 100, -0.1, 0.1);
	TH2F *h400vs20_mean = new TH2F("h400vs20_mean",
			";#DeltaE/E(20 GeV);#DeltaE/E(400 GeV);trials", 100, -0.5, 0.5, 100,
			-0.5, 0.5);
	TH2F *h400vs20_sigma = new TH2F("h400vs20_sigma",
			";#Delta#sigma/#sigma(20 GeV);#Delta#sigma/#sigma(400 GeV);trials",
			100, -3, 3, 100, -3, 3);
	TH2F *h400vs20_reso = new TH2F("h400vs20_reso",
			";#Delta Reso/Reso(20 GeV);#Delta Reso/Reso(400 GeV);trials", 100,
			-3, 3, 100, -3, 3);
	...
	for (unsigned ie(0); ie < nE; ++ie) {
		...
		for (unsigned ip(0); ip < 2; ++ip) {
			label.str("");
			label << "mean_" << ie << "_" << ip;
			mean[ie][ip] = new TH1F(label.str().c_str(), ";<E> (GeV);events",
					100, 0.75 * genEn[ie], 1.25 * genEn[ie]);
			label.str("");
			label << "sigma_" << ie << "_" << ip;
			sigma[ie][ip] = new TH1F(label.str().c_str(),
					";#sigma (GeV);events", 100, 0, 15);
			label.str("");
			label << "reso_" << ie << "_" << ip;
			reso[ie][ip] = new TH1F(label.str().c_str(), ";#sigma/<E>;events",
					100, 0, 0.06);
		}
	}
	for (unsigned ic(0); ic < nPts; ++ic) {  //loop on intercalib
		...
		if (!fcalib[ic]) {
			std::cout << " -- failed to open file: " << label.str()
					<< std::endl;
			continue;
		} else {
			std::cout << " -- file " << label.str() << " successfully opened."
					<< std::endl;
		}
		fcalib[ic]->cd();
		TH1F *Ereco[nE];
		...
		for (unsigned ie(0); ie < nE; ++ie) {
			...
			if (!tree) {
				std::cout << " -- Tree " << label.str() << " not found."
						<< std::endl;
				return 1;
			}
			...
			Ereco[ie] = (TH1F*) (gPad->GetPrimitive("htemp"))->Clone(
					label.str().c_str());
			if (!Ereco[ie]) {
				std::cout << " -- ERROR, pointer for histogram " << label.str()
						<< " is null." << std::endl;
				return 1;
			}
			Ereco[ie]->SetTitle(";E (GeV);events");
			TPad *lpad = (TPad*) (mycE->cd());
			if (fitEnergy(Ereco[ie], lpad, "GeV", lres[ie], 7) != 0)
				return 1;
			...
			if (fitEnergy(Ereco[ie], lpad, "GeV", lresSmear[ie], 7) != 0)
				return 1;
		...
		diffMean->Fill(
				(lresSmear[nE - 1].mean - lres[nE - 1].mean) / lres[nE - 1].mean
						- (lresSmear[0].mean - lres[0].mean) / lres[0].mean);
		h400vs20_mean->Fill((lresSmear[0].mean - lres[0].mean) / lres[0].mean,
				(lresSmear[nE - 1].mean - lres[nE - 1].mean)
						/ lres[nE - 1].mean);
		h400vs20_sigma->Fill(
				(lresSmear[0].sigma - lres[0].sigma) / lres[0].sigma,
				(lresSmear[nE - 1].sigma - lres[nE - 1].sigma)
						/ lres[nE - 1].sigma);
		h400vs20_reso->Fill(
				(lresSmear[0].reso() - lres[0].reso()) / lres[0].reso(),
				(lresSmear[nE - 1].reso() - lres[nE - 1].reso())
						/ lres[nE - 1].reso());
	...
	for (unsigned ie(0); ie < nE; ++ie) {
		for (int ip(1); ip >= 0; --ip) {
			...
			if (ie == 0) {
				if (ip == 0)
					leg->AddEntry(reso[ie][ip], "No smearing", "L");
				if (ip == 1)
					leg->AddEntry(reso[ie][ip], "10% smearing", "L");
			}
			if (ip == 1)
				leg->Draw("same");
