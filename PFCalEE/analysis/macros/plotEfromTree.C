...
void extractMeanEnergy(int *color, int *marker, TTree *tree, unsigned nPts,
		TString *Enames, unsigned genEn, float *meanE, float *rmsE,
		float *meanEerr, float *rmsEerr, float *meanFitE, float *rmsFitE,
		float *meanFitEerr, float *rmsFitEerr, std::string plotDir,
		double *offset, double *slope, double * a, double * b, double * c) {
	...
	for (unsigned iP(0); iP < nPts; ++iP) {
		...
		hrefit[iP] = (TH1F*) (gPad->GetPrimitive("htemp"))->Clone(
				varstr.str().c_str()); // 1D
		//hrefit[iP]->Rebin(2);
	...
	for (unsigned iP(0); iP < nPts; ++iP) {
		...
		if (iP == 0)
			...
			hrefit[iP]->Draw("PEsame");
		//fit
		hrefit[iP]->Fit("gaus", "LR0", "", meanE[iP] - 2 * rmsE[iP],
				meanE[iP] + 2 * rmsE[iP]);
...
int plotEfromTree() {    //main
	...
	for (unsigned i(0); i < nPts; ++i) {
		if (i < nEPts)
			...
			Enames[i] = EnamesC[i - nEPts];
	}

	int marker[14] = { 20, 28, 21, 22, 23, 21, 22, 23, 24, 24, 25, 25, 26, 27 };
	int color[14] = { 1, 1, kRed - 2, kRed, kRed + 2, kGreen - 2, kGreen, kGreen
			+ 2, kCyan, kBlue, kYellow, kYellow + 2, kViolet - 1, kViolet + 1 };
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		...
		if (iC < nPts) {
			lName << "mycinv" << iC;
			mycinv[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(),
					1);
		}
	}
	...
	for (unsigned iC(0); iC < 4; ++iC) {  //calib
		...
		for (unsigned iE(0); iE < nGenEn; ++iE) {
			...
			linputStr << baseDir << "validateCalice_e" << genEn[iE] << pSuffix
					<< ".root";
			...
			//extract histos
			extractMeanEnergy(color, marker, tree, nPts, Enames, genEn[iE],
					meanE[iE], rmsE[iE], meanEerr[iE], rmsEerr[iE],
					meanFitE[iE], rmsFitE[iE], meanFitEerr[iE], rmsFitEerr[iE],
					plotDir, offsetVal, slopeVal, aVal, bVal, cVal);
		...
		for (unsigned iP(0); iP < nPts; ++iP) {
			...
			calibinv[iP] = (TGraphErrors *) calibFit[iP]->Clone(
					"calibinv" + lSuf);
			deltaFit[iP] = (TGraphErrors *) calibFit[iP]->Clone(
					"deltaFit" + lSuf);
			sigmaFit[iP] = (TGraphErrors *) calibFit[iP]->Clone(
					"sigmaFit" + lSuf);

			resoFit[iP] = (TGraphErrors *) calibFit[iP]->Clone(
					"resoFit" + lSuf);
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				...
				reso[iP]->SetPoint(np, doVsE ? genEn[iE] : 1 / sqrt(genEn[iE]),
						rmsE[iE][iP] / meanE[iE][iP]);
				double err = rmsE[iE][iP] / meanE[iE][iP]
						* sqrt(
								pow(rmsEerr[iE][iP] / rmsE[iE][iP], 2)
										+ pow(meanEerr[iE][iP] / meanE[iE][iP],
												2));
				reso[iP]->SetPointError(np, 0, err);

				calibinv[iP]->SetPoint(np, meanFitE[iE][iP], genEn[iE]);
				calibinv[iP]->SetPointError(np, meanFitEerr[iE][iP],
						0.01 * genEn[iE]);
				...
				resoFit[iP]->SetPoint(np,
						doVsE ? genEn[iE] : 1 / sqrt(genEn[iE]),
						rmsFitE[iE][iP] / meanFitE[iE][iP]);
				double errFit = rmsFitE[iE][iP] / meanFitE[iE][iP]
						* sqrt(
								pow(rmsFitEerr[iE][iP] / rmsFitE[iE][iP], 2)
										+ pow(
												meanFitEerr[iE][iP]
														/ meanFitE[iE][iP], 2));
			...
			if (iC == 0)
				gr[iP]->GetXaxis()->SetTitle(
						"Corrected energy deposited [MIPs]");
			else
				gr[iP]->GetXaxis()->SetTitle(
						"Corrected energy deposited [GeV]");
			...
			TF1 *fitFuncinv = new TF1("calibinv", "[0]*x+[1]*x*x+[2]*x*x*x",
					gr[iP]->GetXaxis()->GetXmin(),
					gr[iP]->GetXaxis()->GetXmax());
			...
			lat.SetTextColor(1);
			sprintf(buf,
					"<E> #propto E #times (a + b #times E +c #times E^{2})");
			lat.DrawLatex(genEn[0], gr[iP]->GetYaxis()->GetXmax() * 0.9, buf);
			sprintf(buf, "a = %3.3f #pm %3.3f %s", fitFuncinv->GetParameter(0),
					fitFuncinv->GetParError(0), iC == 0 ? "MIPs" : "GeV");
			lat.DrawLatex(genEn[0], gr[iP]->GetYaxis()->GetXmax() * (0.8), buf);
			sprintf(buf, "b = %3.3f #pm %3.3f %s/GeV",
					fitFuncinv->GetParameter(1), fitFuncinv->GetParError(1),
					iC == 0 ? "MIPs" : "GeV");
			lat.DrawLatex(genEn[0], gr[iP]->GetYaxis()->GetXmax() * (0.7), buf);
			sprintf(buf, "c = %3.6f #pm %3.6f %s/GeV",
					fitFuncinv->GetParameter(2), fitFuncinv->GetParError(2),
					iC == 0 ? "MIPs" : "GeV");
			lat.DrawLatex(genEn[0], gr[iP]->GetYaxis()->GetXmax() * (0.6), buf);
			sprintf(buf, "chi2/NDF = %3.3f/%d = %3.3f",
					fitFuncinv->GetChisquare(), fitFuncinv->GetNDF(),
					fitFuncinv->GetChisquare() / fitFuncinv->GetNDF());
			...
			gr[iP]->SetTitle(Enames[iP]);
			gr[iP]->SetMaximum(
					std::max(calib[iP]->GetMaximum(),
							calibFit[iP]->GetMaximum()));
			gr[iP]->Draw("ap");
			calib[iP]->Draw("pl");
			gr[iP]->GetXaxis()->SetTitle("");
			if (iC == 0)
				gr[iP]->GetYaxis()->SetTitle("Average energy deposited [MIPs]");
			else
				gr[iP]->GetYaxis()->SetTitle("Average energy deposited [GeV]");
			//gr[iP]->GetXaxis()->SetTitle(doVsE?"Beam energy [GeV]" :"1/#sqrt{Beam energy} [1/#sqrt{GeV}]"); 
			...
			sprintf(buf, "a = %3.3f #pm %3.3f %s", fitFunc->GetParameter(0),
					fitFunc->GetParError(0), iC == 0 ? "MIPs" : "GeV");
			lat.DrawLatex(genEn[0], gr[iP]->GetYaxis()->GetXmax() * (0.8), buf);
			sprintf(buf, "b = %3.3f #pm %3.3f %s/GeV", fitFunc->GetParameter(1),
					fitFunc->GetParError(1), iC == 0 ? "MIPs" : "GeV");
			lat.DrawLatex(genEn[0], gr[iP]->GetYaxis()->GetXmax() * (0.7), buf);
			sprintf(buf, "chi2/NDF = %3.3f/%d = %3.3f", fitFunc->GetChisquare(),
					fitFunc->GetNDF(),
			...
			for (unsigned iE(0); iE < nGenEn; ++iE) {
				if (iC < 1) {
					deltaFit[iP]->SetPoint(iE, genEn[iE],
							(((meanFitE[iE][iP] - fitFunc->GetParameter(0))
									/ fitFunc->GetParameter(1)) - genEn[iE])
									/ genEn[iE]);
					deltaFit[iP]->SetPointError(iE, 0.0,
							meanFitEerr[iE][iP] / fitFunc->GetParameter(1) * 1.
									/ genEn[iE]);
				} else {
					deltaFit[iP]->SetPoint(iE, genEn[iE],
							(meanFitE[iE][iP] - genEn[iE]) / genEn[iE]);
					deltaFit[iP]->SetPointError(iE, 0.0,
							meanFitEerr[iE][iP] / genEn[iE]);
				}
			}
			...
			if (iP == 0)
				...
				grDelta[iP]->Draw("psame");

			myc[nPts + 1]->cd();
			gPad->SetGridx(1);
			gPad->SetGridy(1);
			sigmaFit[iP]->SetTitle("");
			sigmaFit[iP]->SetMaximum(
					std::max(sigma[iP]->GetMaximum(),
							sigmaFit[iP]->GetMaximum()));
			if (iP == 0)
				sigmaFit[iP]->Draw("ap");
			else
				sigmaFit[iP]->Draw("p");
			...
			if (iC == 0)
				sigmaFit[iP]->GetYaxis()->SetTitle(
						"RMS energy deposited [MIPs]");
			else
				sigmaFit[iP]->GetYaxis()->SetTitle(
						"RMS energy deposited [GeV]");
			...
			TF1* fitref = new TF1("reso", "sqrt([0]/x+[1]+[2]/(x*x))",
					resoFit[iP]->GetXaxis()->GetXmin(),
					resoFit[iP]->GetXaxis()->GetXmax());
			if (isEM)
				fitref->SetParameters(0.215 * 0.215, 0.007 * 0.007,
						0.06 * 0.06);
			else {
				if (iP < nEPts)
					fitref->SetParameters(0.518 * 0.518, 0.04 * 0.04,
							0.18 * 0.18);
				else
					fitref->SetParameters(0.436 * 0.436, 0.0 * 0.0,
							0.18 * 0.18);
			}
			...
			if (iP == 0)
				lat.DrawLatex(30, resoFit[iP]->GetYaxis()->GetXmax() * 0.9,
						buf);
			...
			gPad->SetGridx(1);
			gPad->SetGridy(1);
			resoFit[iP]->SetMaximum(
					std::max(reso[iP]->GetMaximum(),
							resoFit[iP]->GetMaximum()));
			...
			if (doVsE) {
				fitFunc2 = new TF1("reso", "sqrt([0]/x+[1])",
						...
						resoFit[iP]->GetXaxis()->GetXmax());
				if (addNoiseTerm)
					fitFunc2 = new TF1("reso", "sqrt([0]/x+[1]+[2]/(x*x))",
							resoFit[iP]->GetXaxis()->GetXmin(),
							resoFit[iP]->GetXaxis()->GetXmax());
			} else {
				fitFunc2 = new TF1("reso", "sqrt([0]*x*x+[1])",
						...
						resoFit[iP]->GetXaxis()->GetXmax());
				if (addNoiseTerm)
					fitFunc2 = new TF1("reso", "sqrt([0]*x*x+[1]+[2]*x*x*x*x)",
							resoFit[iP]->GetXaxis()->GetXmin(),
							resoFit[iP]->GetXaxis()->GetXmax());
			}
			...
			if (addNoiseTerm) {
				...
				lat.DrawLatex(Emin, resoFit[iP]->GetYaxis()->GetXmax() * 0.64,
						buf);
			}
			lat.SetTextColor(4);
			...
			lat.SetTextColor(1);
			lat.DrawLatex(20, resoFit[iP]->GetYaxis()->GetXmax() * 1.03,
					"Geant4 QGSP_BERT, #pi^{-} gun");
