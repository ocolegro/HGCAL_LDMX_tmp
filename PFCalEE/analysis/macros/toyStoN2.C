...
int toyStoN2() {
	...
	for (unsigned ip(0); ip < nPts + 1; ++ip) {
		...
		mip->Draw("same");
		landaugaus->SetParameters(fit->GetParameter(2), mpc,
				fit->GetParameter(0), noise, nEvtsNoise, 0, noise);
		...
		sprintf(buf, "signal+noise MPV = %3.3f #pm %3.3f",
				landaugaus->GetParameter(1), landaugaus->GetParError(1));
		...
		label << "PLOTS/ToyStoN2_purity" << std::setprecision(2)
				<< 1.0 * nEvts / (nEvts + nEvtsNoise) << "_noise" << noise
				<< ".pdf";
