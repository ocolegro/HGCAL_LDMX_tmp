...
SignalRegion::SignalRegion(const std::string inputFolder,
		const unsigned nLayers, const unsigned nevt,
		const HGCSSGeometryConversion & geomConv,
		const HGCSSPUenergy & puDensity, const bool applyPuMixFix,
		const unsigned versionNumber, const bool doHexa) {
	...
	if (!fzpos.is_open()) {
		...
		std::cout << " -- z positions taken from file " << finname.str()
				<< std::endl;
	}
...
bool SignalRegion::initialiseFitPositions() {
	...
	std::cout << " -- Accurate positions found in file " << finname.str()
			<< std::endl;
	...
	while (!fxypos.eof()) {
		...
		fxypos >> eventIndex >> xpos >> fitMatrix[0] >> xangle >> fitMatrix[1]
				>> ypos >> fitMatrix[2] >> yangle >> fitMatrix[3] >> xpost
				>> xanglet >> ypost >> yanglet;
		//testing for nan
		if (eventIndex != eventIndex || xpos != xpos
				|| fitMatrix[0] != fitMatrix[0] || xangle != xangle
				|| fitMatrix[1] != fitMatrix[1] || ypos != ypos
				|| fitMatrix[2] != fitMatrix[2] || yangle != yangle
				|| fitMatrix[3] != fitMatrix[3]) {
			std::cout << " Found nan ! Fix code !" << std::endl;
			std::cout << eventIndex << " " << xpos << " " << fitMatrix[0] << " "
					<< xangle << " " << fitMatrix[1] << " " << ypos << " "
					<< fitMatrix[2] << " " << yangle << " " << fitMatrix[3]
					<< std::endl;
			exit(1);
		}
		if (eventIndex < nevt_) {
	...
	if (nfound < nevt_) {
		std::cout << " Warning, file " << finname.str() << " contains only "
				<< nfound << " events, program running on " << nevt_ << "."
				<< std::endl;
		if (nfound * 1. / nevt_ < 0.5)
			return false;
	}
...
void SignalRegion::initialise(TFile *outputFile, const std::string outputDir) {
...
ROOT::Math::XYZPoint SignalRegion::getAccuratePos(const unsigned ievt,
		const unsigned iL) const {
	return getAccuratePos(accurateFit_[ievt], iL);
}

ROOT::Math::XYZPoint SignalRegion::getAccuratePos(const FitResult& fit,
		const unsigned iL) const {
	return ROOT::Math::XYZPoint(
			fit.pos_x + fit.tanangle_x * (zPos_[iL] - fit.pos_z),
			fit.pos_y + fit.tanangle_y * (zPos_[iL] - fit.pos_z), zPos_[iL]);
}

Direction SignalRegion::getAccurateDirection(const unsigned ievt) const {
	return Direction(accurateFit_[ievt].tanangle_x,
			accurateFit_[ievt].tanangle_y);
}

bool SignalRegion::fillEnergies(const unsigned ievt, const HGCSSEvent & event,
		...
		const std::vector<HGCSSRecoHit> & rechitvec, const unsigned nPuVtx) {
	bool found = false;
	FitResult fit;
	...
	if (!found) {
		std::cout
				<< " - Info: no photon G4trackID=1 found, already converted or outside of acceptance ..."
				<< std::endl;
		fit.found = false;
	}
...
bool SignalRegion::fillEnergies(const unsigned ievt,
	...
	if (absweight_.size() != nLayers_) {
		std::cout << " -- Error! Not all layers found! Only: "
				<< absweight_.size() << ". Fix code." << std::endl;
		exit(1);
	}
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {
		int refid = 0;
		if (doHexa_)
			refid = geomConv_.hexagonMap()->FindBin(eventPos[iL].X(),
					eventPos[iL].Y());
		else
			refid = geomConv_.squareMap()->FindBin(eventPos[iL].X(),
					eventPos[iL].Y());
		refx[iL] =
				doHexa_ ?
						geomConv_.hexaGeom[refid].first :
						geomConv_.squareGeom[refid].first;
		refy[iL] =
				doHexa_ ?
						geomConv_.hexaGeom[refid].second :
						geomConv_.squareGeom[refid].second;
	}
	...
	for (unsigned iH(0); iH < rechitvec.size(); ++iH) {  //loop on hits
		...
		double puE = puDensity_.getDensity(leta, layer,
				geomConv_.cellSizeInCm(layer, lradius), nPuVtx);
		...
		double halfCelly =
				doHexa_ ?
						geomConv_.cellSize(layer, lradius) :
						geomConv_.cellSize(layer, lradius) / 2.;
		//std::cout << " halfcell = " << halfCell << std::endl;
		//SR0-4
		for (unsigned isr(0); isr < nSR_; ++isr) {
			double dx =
					isr % 2 == 0 ?
							posx - refx[layer] : posx - eventPos[layer].x();
			double dy =
					isr % 2 == 0 ?
							posy - refy[layer] : posy - eventPos[layer].y();
			double dr = sqrt(dx * dx + dy * dy);
			if (isr == nSR_ - 1) {
				energySR_[layer][isr] += energy;
				subtractedenergySR_[layer][isr] += subtractedenergy;
			} else if ((doHexa_
					&& ((isr % 2 == 0 && dr < (isr * halfCelly + 0.1))
							|| (isr % 2 == 1 && dr <= ((isr + 1) * halfCellx)
									&& (fabs(dx) <= ((isr + 1) * halfCellx))
									&& (fabs(dy) <= ((isr + 1) * halfCelly)))))
					|| (!doHexa_ && (fabs(dx) <= ((isr + 1) * halfCelly))
							&& (fabs(dy) <= ((isr + 1) * halfCelly)))) {
				...
				if (isr == 2) {
					...
					if (((doHexa_ && (ix > 2 || ix < -2))
						...
						std::cout << " error, check ix=" << ix << " iy=" << iy
								...
								<< " step ";
						if (!doHexa_)
							std::cout << (isr + 1) * halfCelly;
						else
							std::cout << isr * halfCelly + 0.1;
						std::cout << std::endl;
						continue;
					} else {
						if (!doHexa_)
				...
				if (energy > maxhitEoutside_[layer][isr])
					maxhitEoutside_[layer][isr] = energy;
			}
		}	//loop on SR
	}	//loop on hits
...
void SignalRegion::finalise() {
	outputFile_->Flush();
	std::cout << " -- Histograms for signal regions have been filled !"
			<< std::endl;
...
void SignalRegion::initialiseHistograms() {
	...
	if (zPos_.size() != nLayers_) {
		std::cout << " -- ERROR! z positions not filled properly. Exiting..."
				<< std::endl;
		exit(1);
	}
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {
		...
		for (unsigned iSR(0); iSR < nSR_; ++iSR) {
			...
			outtree_->Branch(label.str().c_str(),
					&subtractedenergySR_[iL][iSR]);
			label.str("");
	...
	p_wgtEtotal = new TH1F("p_wgtEtotal", "Total weighted E (MIP)", 5000, 0,
			200000);
	...
	for (unsigned iSR(0); iSR < nSR_; ++iSR) {
		...
		label.str("");
		label << "wgtESR" << iSR;
		p_wgtESR[iSR] = new TH1F(("p_" + label.str()).c_str(),
				";E_{SR} (MIPs);events", 5000, 0, 200000);
		...
		p_wgtSubtractESR[iSR] = new TH1F(("p_" + label.str()).c_str(),
				";E_{SR}^{PUsubtr} (MIPs);events", 5000, 0, 200000);
