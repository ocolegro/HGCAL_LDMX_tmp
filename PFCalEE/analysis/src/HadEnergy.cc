...
HadEnergy::HadEnergy(const std::vector<unsigned> & dropLay,
		HGCSSDetector & myDetector, TChain* lSimTree, TChain* lRecTree,
		TFile *outputFile, const unsigned pNevts) :
		myDetector_(myDetector), lSimTree_(lSimTree), lRecTree_(lRecTree), pNevts_(
				pNevts) {
	dropLay_ = dropLay;
...
void HadEnergy::bookHist(TFile *outputFile) {
	...
	p_spectrumByLayer = new TH2F("p_spectrumByLayer", "p_spectrumByLayer",
			nLayers_, 0, nLayers_, 1000, 0, 10000);
	p_spectrum_hightail = new TH1F("p_spectrum_hightail", "p_spectrum_hightail",
			1000, 0, 250);
	p_spectrum_lowtail = new TH1F("p_spectrum_lowtail", "p_spectrum_lowtail",
			1000, 0, 250);
...
bool HadEnergy::fillEnergies() {
	...
	const unsigned nEvts = (
			(pNevts_ > lRecTree_->GetEntries() || pNevts_ == 0) ?
					static_cast<unsigned>(lRecTree_->GetEntries()) : pNevts_);

	std::cout << "- Processing = " << nEvts << " events out of "
			<< lRecTree_->GetEntries() << std::endl;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { // loop on entries
		...
		wgttotalE_ = (EE_ - ECALoffset_) / ECALslope_
				+ ((EFHCAL_ - FHtoEoffset_) / FHtoEslope_
						+ (EBHCAL_ - BHtoEoffset_)
								/ (BHtoEslope_ * FHtoBHslope_)) / EEtoHslope_;
		Etotal.push_back(wgttotalE_);
		p_Etotal->Fill(wgttotalE_);
		if (nhitsFH_ != 0)
			EmipMeanFH_ = EmipMeanFH_ / nhitsFH_;
		...
		for (unsigned iLim(0); iLim < LimMIP_.size(); iLim++) {
			Cglobal_[iLim] = calcGlobalC(LimMIP_[iLim], EmipMeanFH_,
					spectrumCollection[ievt]);
			correctedtotalE_[iLim] = (EE_ - ECALoffset_) / ECALslope_
					+ ((EFHCAL_ - FHtoEoffset_) / FHtoEslope_ * Cglobal_[iLim]
							+ (EBHCAL_ - BHtoEoffset_)
									/ (BHtoEslope_ * FHtoBHslope_))
							/ (EEtoHslopePar0_ / wgttotalE_ + EEtoHslopePar1_); //wgttotalE_*Cglobal_[iLim];
		}
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {
		if (!spectrumCollection[ievt])
			continue;
		p_spectrum->Add(spectrumCollection[ievt]);
		if (Etotal[ievt] < EMean - ERMS)
			p_spectrum_lowtail->Add(spectrumCollection[ievt]);
		else if (Etotal[ievt] > EMean + ERMS)
			p_spectrum_hightail->Add(spectrumCollection[ievt]);
	}
...
double HadEnergy::calcGlobalC(const double LimMIP, const double EmipMean,
		TH1F* spectrum) {
