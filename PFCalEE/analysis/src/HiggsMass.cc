...
void HiggsMass::setRecoInfo(const TLorentzVector & g1,
		const TLorentzVector & g2, const ROOT::Math::XYZPoint & posFF1,
...
void HiggsMass::setTruthInfo(const TLorentzVector & g1,
		const TLorentzVector & g2, const ROOT::Math::XYZPoint & vtx1,
...
void HiggsMass::initialiseHistograms(TFile *fout, const std::string folder) {

	fout->mkdir(folder.c_str());
	fout->cd(folder.c_str());

	p_trueDir_trueE = new TH1F("p_trueDir_trueE", ";true M_{H} (GeV)", 400, 0,
			200);
	p_trueDir_recoE = new TH1F("p_trueDir_recoE",
			";M_{H} (GeV) [P_{true},E_{reco}]", 400, 0, 200);
	p_position_trueE = new TH1F("p_position_trueE",
			";M_{H} (GeV) [P_{pos},E_{true}]", 400, 0, 200);
	p_position_recoE = new TH1F("p_position_recoE",
			";M_{H} (GeV) [P_{pos},E_{reco}]", 400, 0, 200);
	p_angle_trueE = new TH1F("p_angle_trueE",
			";M_{H} (GeV) [P_{angle},E_{true}]", 400, 0, 200);
	p_angle_recoE = new TH1F("p_angle_recoE",
			";M_{H} (GeV) [P_{angle},E_{reco}]", 400, 0, 200);
	p_position_vtxsmear_trueE = new TH1F("p_position_vtxsmear_trueE",
			";M_{H} (GeV) [P_{pos,vtx smeared},E_{true}]", 400, 0, 200);
	p_position_vtxsmear_recoE = new TH1F("p_position_vtxsmear_recoE",
			";M_{H} (GeV) [P_{pos,vtx smeared},E_{reco}]", 400, 0, 200);

	p_ErecooverEtrue = new TH1F("p_ErecooverEtrue",
	...
	p_pTvseta[0] = new TH2F("p_pTvseta", ";#eta^{H};p_{T}^{H} (GeV)", 150, 1.3,
			6, 200, 0, 400);
	p_MvspT[0] = new TH2F("p_MvspT", ";p_{T}^{H} (GeV);M_{H} (GeV)", 200, 0,
			400, 100, 105, 145);

	p_pT_gamma1[0] = new TH1F("p_pT_gamma1", ";p_{T}^{#gamma1}", 200, 0, 300);
	p_eta_gamma1[0] = new TH1F("p_eta_gamma1", ";#eta^{#gamma1}", 120, 1.3,
			3.5);
	p_phi_gamma1[0] = new TH1F("p_phi_gamma1", ";#phi^{#gamma1}", 120, -3.1416,
			3.1416);

	p_pT_gamma2[0] = new TH1F("p_pT_gamma2", ";p_{T}^{#gamma2}", 200, 0, 300);
	p_eta_gamma2[0] = new TH1F("p_eta_gamma2", ";#eta^{#gamma2}", 120, 1.3,
			3.5);
	p_phi_gamma2[0] = new TH1F("p_phi_gamma2", ";#phi^{#gamma2}", 120, -3.1416,
			3.1416);

	p_pTvseta[1] = new TH2F("p_pTvseta_truth", ";#eta^{H};p_{T}^{H} (GeV)", 150,
			1.3, 6, 200, 0, 400);
	p_MvspT[1] = new TH2F("p_MvspT_truth", ";p_{T}^{H} (GeV);M_{H} (GeV)", 200,
			0, 400, 100, 105, 145);

	p_pT_gamma1[1] = new TH1F("p_pT_gamma1_truth", ";p_{T}^{#gamma1}", 200, 0,
			300);
	p_eta_gamma1[1] = new TH1F("p_eta_gamma1_truth", ";#eta^{#gamma1}", 120,
			1.3, 3.5);
	p_phi_gamma1[1] = new TH1F("p_phi_gamma1_truth", ";#phi^{#gamma1}", 120,
			-3.1416, 3.1416);

	p_pT_gamma2[1] = new TH1F("p_pT_gamma2_truth", ";p_{T}^{#gamma2}", 200, 0,
			300);
	p_eta_gamma2[1] = new TH1F("p_eta_gamma2_truth", ";#eta^{#gamma2}", 120,
			1.3, 3.5);
	p_phi_gamma2[1] = new TH1F("p_phi_gamma2_truth", ";#phi^{#gamma2}", 120,
			-3.1416, 3.1416);
...
void HiggsMass::fillHistograms() {
	...
	//true dir, reco E
	lg1.SetPtEtaPhiE(tg1_.Pt() * g1_.E() / tg1_.E(), tg1_.Eta(), tg1_.Phi(),
			g1_.E());
	lg2.SetPtEtaPhiE(tg2_.Pt() * g2_.E() / tg2_.E(), tg2_.Eta(), tg2_.Phi(),
			g2_.E());
	...
	//reco angle true E
	lg1.SetPtEtaPhiE(g1_.Pt() * tg1_.E() / g1_.E(), g1_.Eta(), g1_.Phi(),
			tg1_.E());
	lg2.SetPtEtaPhiE(g2_.Pt() * tg2_.E() / g2_.E(), g2_.Eta(), g2_.Phi(),
			tg2_.E());
	lh = lg1 + lg2;
