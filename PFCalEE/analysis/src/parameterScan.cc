...
bool parameterScan::processModel(const unsigned nBlocks,
	...
	//filter
	if (totthick > maxThick || xtot < minX0 || ltot > maxLambda)
		return false;
	//reject obvious wrong models...
	if (iS[idx[0]] > iS[idx[1]] && iS[idx[1]] > iS[idx[2]]
			&& iS[idx[2]] > iS[idx[3]] && iS[idx[3]] > iS[idx[4]])
		return false;
	if (iS[idx[0]] > iS[idx[4]] || iS[idx[1]] > iS[idx[4]])
		return false;
	...
	if (iS[idx[0]] < iS[idx[1]] && iS[idx[1]] < iS[idx[2]]
		...
		/*std::cout << " --- " << xtot << " " << totthick << " " << ltot 
		 ...
		 << " " << iS[9] << " " << iS[12]
		 << std::endl;*/
	} else if (iS[idx[0]] > iS[idx[1]] && iS[idx[1]] > iS[idx[2]]
			&& iS[idx[2]] < iS[idx[3]] && iS[idx[3]] <= iS[idx[4]]) {
		tmp->SetTitle("1>2>3<4<5;block;extra thickness (X0)");
		myc[1]->cd();
	...
	isInserted = modelMap.insert(
			std::pair<double, unsigned>(
					static_cast<unsigned>(xtot * 100000) / 100000., 1));
...
void parameterScan::print() {
	...
	std::cout << "Step size for W (mm / X0)  = " << stepSizeW << " / "
			<< stepSizeW / x0w << std::endl;
	std::cout << "Step size for Pb (mm / X0) = " << stepSizePb << " / "
			<< stepSizePb / x0pb << std::endl;
