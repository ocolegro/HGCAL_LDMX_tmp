...
std::pair<unsigned, std::pair<double, double> > PositionFit::findMajorityValue(
		std::vector<std::pair<double, double> > & values) const {

	unsigned lTot = values.size();
	if (!lTot)
		return std::pair<unsigned, std::pair<double, double> >(0,
	...
	return std::pair<unsigned, std::pair<double, double> >(lMajorityCounter,
			lMaj);

}

PositionFit::PositionFit(const unsigned nSR, const double & residualMax,
		const unsigned nLayers, const unsigned nSiLayers,
		const bool applyPuMixFix, const unsigned debug, const bool doMatrix,
		const double& vtxx, const double& vtxy) {
	nSR_ = nSR;
...
void PositionFit::initialise(TFile *outputFile, const std::string outputDir,
		const std::string outFolder, const HGCSSGeometryConversion & geomConv,
...
void PositionFit::initialiseClusterHistograms() {
	...
	p_nClusters = new TH1F("p_nClusters", ";n_{clusters};n_{events}", 1000, 0,
			3000);

	p_clusnHits_all = new TH1F("p_clusnHits_all", ";n_{hits};n_{clusters}", 500,
			0, 1000);
	p_seedEoverE_all = new TH1F("p_seedEoverE_all", ";seedE/E;n_{clusters}",
			100, 0, 1);
	p_clusLayer_all = new TH1F("p_clusLayer_all", ";cluster layer;n_{clusters}",
			nLayers_, 0, nLayers_);
	p_clusWidth_all = new TH1F("p_clusWidth_all",
			";cluster width (layers);n_{clusters}", nLayers_, 0, nLayers_);
	p_seeddeta_all = new TH1F("p_seeddeta_all",
			";#Delta#eta(seed,cluster);n_{clusters}", 100, -0.5, 0.5);
	p_seeddphi_all = new TH1F("p_seeddphi_all",
			";#Delta#phi(seed,cluster);n_{clusters}", 100, -0.5, 0.5);

	p_clusnHits_sel = new TH1F("p_clusnHits_sel", ";n_{hits};n_{events}", 500,
			0, 1000);
	p_seedEoverE_sel = new TH1F("p_seedEoverE_sel", ";seedE/E;n_{events}", 100,
			0, 1);
	p_clusLayer_sel = new TH1F("p_clusLayer_sel", ";cluster layer;n_{events}",
			nLayers_, 0, nLayers_);
	p_clusWidth_sel = new TH1F("p_clusWidth_sel",
			";cluster width (layers);n_{events}", nLayers_, 0, nLayers_);
	p_seeddeta_sel = new TH1F("p_seeddeta_sel",
			";#Delta#eta(seed,cluster);n_{events}", 100, -0.1, 0.1);
	p_seeddphi_sel = new TH1F("p_seeddphi_sel",
			";#Delta#phi(seed,cluster);n_{events}", 100, -0.1, 0.1);

	p_mindRtruth = new TH1F("p_mindRtruth", ";mindR(cluster,truth);n_{events}",
			100, 0, 0.5);
...
void PositionFit::initialisePositionHistograms() {
	...
	if (saveEtree_) {
		outputFile_->cd(outputDir_.c_str());
		outtree_ = new TTree("EcellsSR2",
				"Tree to save energies in each cell of SR2 and PCA info");
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {
		lName.str("");
		lName << "p_genxy_" << iL;
		p_genxy[iL] = new TH2F(lName.str().c_str(), ";x(mm);y(mm)", nX * 10,
				minX, maxX, nY * 10, minY, maxY);
		lName.str("");
		lName << "p_recoxy_" << iL;
		p_recoxy[iL] = new TH2F(lName.str().c_str(), ";x(mm);y(mm)", nX, minX,
				maxX, nY, minY, maxY);
		lName.str("");
		lName << "p_dRmin_" << iL;
		p_dRmin[iL] = new TH1F(lName.str().c_str(), ";dRmin;events", 100, 0, 1);

		lName.str("");
		lName << "p_diffXpos_" << iL;
		p_diffXpos[iL] = new TH1F(lName.str().c_str(),
				";#Deltax(fit,max);events", 100, -50, 50);
		lName.str("");
		lName << "p_diffYpos_" << iL;
		p_diffYpos[iL] = new TH1F(lName.str().c_str(),
				";#Deltax(fit,max);events", 100, -50, 50);

	}

	p_hitMeanPuContrib = new TH2F("p_hitMeanPuContrib",
			";layer;E_{PU} (MIPs) from mean;hits", nLayers_, 0, nLayers_, 1000,
			0, 50);
	p_hitMeanPuContrib->StatOverflows();

	p_hitEventPuContrib = new TH2F("p_hitEventPuContrib",
			";layer;E_{PU} (MIPs) from RC;hits", nLayers_, 0, nLayers_, 1000, 0,
			50);
	p_hitEventPuContrib->StatOverflows();

	p_diffPuContrib = new TH1F("p_diffPuContrib",
			";E_{PU}^{RC}-E_{PU}^{avg} (MIPs) in SR2;events", 1000, -2000,
			2000);
	p_diffPuContrib->StatOverflows();

	p_etavsphi = new TH2F("p_etavsphi", ";#phi_{hit};#eta_{hit};n_{hits}", 900,
			-3.1416, 3.1416, 250, 1.4, 3.0);

	p_etavsphi_max = new TH2F("p_etavsphi_max",
			";#phi_{max};#eta_{max};n_{events}", 900, -3.1416, 3.1416, 250, 1.4,
			3.0);
	p_etavsphi_truth = new TH2F("p_etavsphi_truth",
			";#phi_{gen};#eta_{gen};n_{events}", 900, -3.1416, 3.1416, 250, 1.4,
			3.0);

	p_yvsx_max = new TH2F("p_yvsx_max", ";x_{max};y_{max};n_{events}", nX, minX,
			maxX, nY, minY, maxY);
	p_yvsx_truth = new TH2F("p_yvsx_truth", ";x_{gen};y_{gen};n_{events}", nX,
			minX, maxX, nY, minY, maxY);

	p_residuals_x = new TH1F("p_residuals_x", ";xreco-xtruth (mm)", 1000, -50,
			50);
	p_residuals_y = new TH1F("p_residuals_y", ";yreco-ytruth (mm)", 1000, -50,
			50);
...
void PositionFit::initialiseFitHistograms() {
	...
	if (!p_chi2[0]) {
		...
		p_recoXvsLayer = new TH2F("p_recoXvsLayer",
				";layer;weighted x (mm);n_{events}", nLayers_, 0, nLayers_, 200,
				-100, 100);
		p_recoYvsLayer = new TH2F("p_recoYvsLayer",
				";layer;weighted y (mm);n_{events}", nLayers_, 0, nLayers_,
				1200, 300, 1500);
		p_recoZvsLayer = new TH2F("p_recoZvsLayer",
				";layer;avg z (mm);n_{events}", nLayers_, 0, nLayers_, 3000,
				3170, 3470);
		p_truthXvsLayer = new TH2F("p_truthXvsLayer",
				";layer;weighted x (mm);n_{events}", nLayers_, 0, nLayers_, 200,
				-100, 100);
		p_truthYvsLayer = new TH2F("p_truthYvsLayer",
				";layer;weighted x (mm);n_{events}", nLayers_, 0, nLayers_,
				1200, 300, 1500);
		p_fitXvsLayer = new TH2F("p_fitXvsLayer",
				";layer;fit x (mm);n_{events}", nLayers_, 0, nLayers_, 200,
				-100, 100);
		p_fitYvsLayer = new TH2F("p_fitYvsLayer",
				";layer;fit y (mm);n_{events}", nLayers_, 0, nLayers_, 1200,
				300, 1500);
		p_nLayersFit = new TH1F("p_nLayersFit", ";#layers in fit;n_{events}",
				31, -0.5, 30.5);

		p_chi2[0] = new TH1F("p_chi2", ";#chi^{2};n_{events}", 1000, 0, 5000);
		p_chi2[1] = new TH1F("p_chi2_truth", ";#chi^{2};n_{events}", 1000, 0,
				5000);
		p_chi2overNDF[0] = new TH1F("p_chi2overNDF", ";#chi^{2}/NDF;n_{events}",
				1000, 0, 500);
		p_chi2overNDF[1] = new TH1F("p_chi2overNDF_truth",
				";#chi^{2}/NDF;n_{events}", 1000, 0, 500);
		for (unsigned rt(0); rt < 2; ++rt) {
			p_chi2[rt]->StatOverflows();
			p_chi2overNDF[rt]->StatOverflows();
		}

		p_impactXFF[0] = new TH1F("p_impactXFF",
				";x front face impact (mm);n_{events}", 500, -100, 100);
		p_impactXFF[1] = new TH1F("p_impactXFF_truth",
				";x front face impact (mm);n_{events}", 500, -100, 100);
		p_impactYFF[0] = new TH1F("p_impactYFF",
				";y front face impact (mm);n_{events}", 1200, 300, 1500);
		p_impactYFF[1] = new TH1F("p_impactYFF_truth",
				";y front face impact (mm);n_{events}", 1200, 300, 1500);
		p_impactX14[0] = new TH1F("p_impactX14",
				";x layer 14 impact (mm);n_{events}", 500, -100, 100);
		p_impactX14[1] = new TH1F("p_impactX14_truth",
				";x layer 14 impact (mm);n_{events}", 500, -100, 100);
		p_impactY14[0] = new TH1F("p_impactY14",
				";y layer 14 impact (mm);n_{events}", 1200, 300, 1500);
		p_impactY14[1] = new TH1F("p_impactY14_truth",
				";y layer 14 impact (mm);n_{events}", 1200, 300, 1500);
		p_tanAngleX[0] = new TH1F("p_tanAngleX",
				";x direction tanAngle (rad);n_{events}", 500, -1, 1);
		p_tanAngleX[1] = new TH1F("p_tanAngleX_truth",
				";x direction tanAngle (rad);n_{events}", 500, -1, 1);
		p_tanAngleY[0] = new TH1F("p_tanAngleY",
				";y direction tanAngle (rad);n_{events}", 500, -1, 1);
		p_tanAngleY[1] = new TH1F("p_tanAngleY_truth",
				";y direction tanAngle (rad);n_{events}", 500, -1, 1);

		p_impactXFF_residual = new TH1F("p_impactXFF_residual",
				";residual x front face impact (mm);n_{events}", 200, -10, 10);
		p_impactX14_residual = new TH1F("p_impactX14_residual",
				";residual x layer 14 impact (mm);n_{events}", 200, -10, 10);
		p_tanAngleX_residual = new TH1F("p_tanAngleX_residual",
				";residual x direction tanAngle (rad);n_{events}", 200, -0.1,
				0.1);
		p_angleX_residual = new TH1F("p_angleX_residual",
				";residual x direction angle (rad);n_{events}", 200, -0.1, 0.1);
		p_impactYFF_residual = new TH1F("p_impactYFF_residual",
				";residual y front face impact (mm);n_{events}", 200, -10, 10);
		p_impactY14_residual = new TH1F("p_impactY14_residual",
				";residual y layer 14 impact (mm);n_{events}", 200, -10, 10);
		p_tanAngleY_residual = new TH1F("p_tanAngleY_residual",
				";residual y direction tanAngle (rad);n_{events}", 200, -0.1,
				0.1);
		p_angleY_residual = new TH1F("p_angleY_residual",
				";residual y direction angle (rad);n_{events}", 200, -0.1, 0.1);

		p_eta_reco = new TH1F("p_eta_reco", ";reco #eta;n_{events}", 200, 1.4,
				3.0);
		p_phi_reco = new TH1F("p_phi_reco", ";reco #phi (rad);n_{events}", 200,
				1.4, 3);
		p_eta_truth = new TH1F("p_eta_truth", ";truth #eta;n_{events}", 200,
				-3.1416, 3.1416);
		p_phi_truth = new TH1F("p_phi_truth", ";truth #phi (rad);n_{events}",
				200, -3.1416, 3.1416);
		p_eta_residual = new TH1F("p_eta_residual", ";residual #eta;n_{events}",
				200, -0.1, 0.1);
		p_phi_residual = new TH1F("p_phi_residual",
				";residual #phi (rad);n_{events}", 200, -0.1, 0.1);

		p_positionReso = new TH1F("p_positionReso",
				";#sigma_{x,y} (mm);n_{events}", 500, 0, 50);
		p_angularReso = new TH1F("p_angularReso",
				";#sigma_{#theta} (rad);n_{events}", 500, 0, 1);
	}
}

bool PositionFit::getGlobalMaximum(const unsigned ievt, const unsigned nVtx,
		std::vector<HGCSSRecoHit> *rechitvec,
		const ROOT::Math::XYZVector & truthPos0, double & aPhimax,
		double & aEtamax) {

	bool oneresult = true;
	TH2F *letavsphi = new TH2F("letavsphi", ";#phi;#eta;hits", 900, -3.1416,
			3.1416, 250, 1.4, 3.0);
	...
	for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {   //loop on hits
		...
		if (debug_ > 1) {
			std::cout << " --  RecHit " << iH << "/" << (*rechitvec).size()
					<< " --" << std::endl << " --  position x,y " << posx << ","
					<< posy << std::endl;
			lHit.Print(std::cout);
		}
	...
	if (debug_ || (counter > 200))
		std::cout << " -- Maximum found after " << counter
				<< " trials, dRmin = " << dRmin;
	if (counter > 200)
		std::cout << " --> away from truth pos for evt " << ievt;
	if (debug_ || (counter > 200))
		std::cout << std::endl;
	...
	if (debug_)
		std::cout << " MaxE cell eta,phi = " << aEtamax << " " << aPhimax
				<< std::endl;
...
void PositionFit::findSeeds(std::vector<HGCSSRecoHit> *rechitvec,
	...
	for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {   //loop on hits
		const HGCSSRecoHit & lHit = (*rechitvec)[iH];
		if (lHit.energy() > seedMipThreshold_)
			seedable[iH] = true;
	}
...
bool PositionFit::setTruthInfo(std::vector<HGCSSGenParticle> *genvec,
		const int G4TrackID) {
	...
	for (unsigned iP(0); iP < (*genvec).size(); ++iP) { //loop on gen particles    
		...
		if ((*genvec)[iP].trackID() == G4TrackID) {
			found = true;
			//set direction
			truthDir_ = Direction((*genvec)[iP].px() / (*genvec)[iP].pz(),
					(*genvec)[iP].py() / (*genvec)[iP].pz());
			p_etavsphi_truth->Fill(truthDir_.phi(), truthDir_.eta());
			double xvtx = xvtx_, yvtx = yvtx_;
			double zvtx = (*genvec)[iP].z()
					- (((*genvec)[iP].x() + (*genvec)[iP].y() - xvtx - yvtx)
							/ (truthDir_.tanangle_x + truthDir_.tanangle_y));
			...
			if (debug_)
				std::cout << " Found truth vertex pos at: x=" << xvtx << ", y"
						<< yvtx << ", z=" << zvtx << " x,ypos was (expected): "
						<< (*genvec)[iP].x() << " (" << truthPos(0).X() << ") "
						<< (*genvec)[iP].y() << " (" << truthPos(0).Y() << ") "
						<< std::endl;
	...
	if (!found) {
		std::cout << " - Info: no photon G4trackID=" << G4TrackID
				<< " found, already converted or outside of acceptance ..."
				<< std::endl;
...
bool PositionFit::getZpositions(const unsigned versionNumber) {
	...
	if (!fin.is_open()) {
		std::cout << " Cannot open input file " << finname.str()
				<< "! Refilling now..." << std::endl;
		return false;
	}

	std::cout << " Reading z position per layer from input file "
			<< finname.str() << std::endl;
	...
	if (avgZ_.size() != nLayers_) {
		std::cout
				<< " -- Warning! Problem in extracting z positions, did not find one value per layer. Please check input file: "
				<< finname.str() << std::endl
...
void PositionFit::getZpositions(const unsigned versionNumber, TTree *aSimTree,
	...
	if (!fout.is_open()) {
		std::cout << " Cannot open outfile " << foutname.str()
				<< " for writing ! Exiting..." << std::endl;
		exit(1);
	}

	std::cout << "--- Filling z positions:" << std::endl << "- Processing = "
			<< nEvts << " events out of " << aSimTree->GetEntries()
			<< std::endl;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {     //loop on entries
		...
		for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) { //loop on rechits
			...
			if (lHit.silayer() >= nSiLayers_)
				continue;

			double posz = lHit.get_z();
			//get z position of hits, take right-most position
			if (avgZ_[layer] < posz || avgZ_[layer] == 0)
				avgZ_[layer] = posz;
		}
	}
...
void PositionFit::getInitialPositions(TTree *aSimTree, TTree *aRecTree,
		const unsigned nEvts, const unsigned G4TrackID) {
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {   //loop on entries
		...
		if (debug_) {
			std::cout << "... Size of hit vectors: sim = "
					<< (*simhitvec).size() << ", reco = " << (*rechitvec).size()
					<< " gen " << (*genvec).size() << std::endl;
		}
		...
		if (!getInitialPosition(ievt, nPuVtx, rechitvec, nTooFar, nNoCluster))
			continue;
	...
	std::cout << " -- Number of converted photons: " << nConvertedPhotons
			<< std::endl;
	std::cout << " -- Number of events with no cluster : " << nNoCluster
			<< std::endl;
	std::cout
			<< " -- Number of events with closest cluster away from truth within dR "
			<< maxdR_ << " : " << nTooFar << std::endl;

}

bool PositionFit::getInitialPosition(const unsigned ievt, const unsigned nPuVtx,
		std::vector<HGCSSRecoHit> *rechitvec, unsigned & nTooFar,
		unsigned & nNoCluster) {
	...
	for (unsigned iClus(0); iClus < nClusters; ++iClus) {
		...
		if (lCluster.energy() > 0)
			p_seedEoverE_all->Fill(lCluster.getSeedE() / lCluster.energy());
	...
	if (debug_) {
		...
		lCluster.Print(std::cout);
		std::cout << " Truth pos eta-phi = " << truthEta_ << " " << truthPhi_
				<< std::endl;
	}
	...
	if (lCluster.energy() > 0)
		p_seedEoverE_sel->Fill(lCluster.getSeedE() / lCluster.energy());
	...
	getMaximumCell(rechitvec, pcaPhi_, pcaEta_, lCluster.position(), xmax,
			ymax);
	...
	if (nPuVtx > 0) {
		...
		if (debug_)
			std::cout << "--- etamax = " << pcaEta_ << " phimax=" << pcaPhi_
					<< " phistep = " << phistep << std::endl;
		for (unsigned ipm(0); ipm < nRandomCones; ++ipm) {
			...
			if (phirc > TMath::Pi())
				phirc -= 2. * TMath::Pi();
			if (ipm % 2 == 0)
				phirc += ipm / 2 * phistep + phistep / 2.;
			else
				phirc = phirc - ipm / 2 * phistep - phistep / 2.;
			if (phirc < -1. * TMath::Pi())
				phirc += 2. * TMath::Pi();
			if (phirc > TMath::Pi())
				phirc -= 2. * TMath::Pi();
			...
			getMaximumCellFromGeom(phirc, pcaEta_, lCluster.position(), xmaxrc,
					ymaxrc);
			if (debug_ > 1)
				std::cout << "rc #" << ipm << " phirc=" << phirc << " xmax[10]="
						<< xmaxrc[10] << " ymax[10]=" << ymaxrc[10] << " r="
						<< sqrt(pow(xmaxrc[10], 2) + pow(ymaxrc[10], 2))
						<< std::endl;
	...
	getEnergyWeightedPosition(rechitvec, nPuVtx, xmax, ymax, recoPos, recoE,
			nHits, puE);
	...
	if (debug_)
		std::cout << " Summary of reco and truth positions:" << std::endl;
	for (unsigned iL(0); iL < nLayers_; ++iL) {    //loop on layers
		if (debug_)
			std::cout << iL << " nHits=" << nHits[iL] << " Max=(" << xmax[iL]
					<< "," << ymax[iL] << ")\t Reco=(" << recoPos[iL].X() << ","
					<< recoPos[iL].Y() << ")\t Truth=(" << truthPos(iL).X()
					<< "," << truthPos(iL).Y() << ")" << std::endl;
		fout << iL << " " << recoPos[iL].X() << " " << recoPos[iL].Y() << " "
				<< truthPos(iL).X() << " " << truthPos(iL).Y();
		if (!doMatrix_)
			fout << " " << recoE[iL];
		fout << std::endl;
	}
	...
	if (doMatrix_)
		fillErrorMatrix(recoPos, nHits);

	return true;

}

void PositionFit::getMaximumCellFromGeom(const double & phimax,
		const double & etamax, const ROOT::Math::XYZPoint & cluspos,
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {
		...
		if (xmax[iL] > 0)
			xmax[iL] = static_cast<int>((xmax[iL] + cs * 0.5) / cs) * cs;
		else
			xmax[iL] = static_cast<int>((xmax[iL] - cs * 0.5) / cs) * cs;
		if (ymax[iL] > 0)
			ymax[iL] = static_cast<int>((ymax[iL] + cs * 0.5) / cs) * cs;
		else
			ymax[iL] = static_cast<int>((ymax[iL] - cs * 0.5) / cs) * cs;
...
void PositionFit::getMaximumCell(std::vector<HGCSSRecoHit> *rechitvec,
		...
		const ROOT::Math::XYZPoint & cluspos, std::vector<double> & xmax,
		std::vector<double> & ymax) {
	...
	for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {  //loop on rechits
		...
		if (debug_ > 1) {
			std::cout << " --  RecoHit " << iH << "/" << (*rechitvec).size()
					<< " --" << std::endl << " --  position x,y " << posx << ","
					<< posy << std::endl;
			lHit.Print(std::cout);
		}
		...
		ROOT::Math::XYZVector pos(posx - cluspos.x() * 10,
				posy - cluspos.y() * 10, posz - cluspos.z() * 10);
...
void PositionFit::getEnergyWeightedPosition(
		std::vector<HGCSSRecoHit> *rechitvec, const unsigned nPU,
		const std::vector<double> & xmax, const std::vector<double> & ymax,
		std::vector<ROOT::Math::XYPoint> & recoPos, std::vector<double> & recoE,
		std::vector<unsigned> & nHits, std::vector<double> & puE,
	...
	for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {  //loop on rechits
		...
		if (fabs(posx - xmax[layer]) < step
			...
			if (puSubtracted) {
				...
				if (debug_ > 1)
					std::cout << " -- Hit " << iH << ", eta=" << leta
							<< ", energy before PU subtraction: " << energy
							<< " after: ";
				double lCorMean = puDensity_.getDensity(leta, layer,
						geomConv_.cellSizeInCm(layer, lR), nPU);
			...
			if (doLogWeight_) {
				...
				if ((ix > 1 || ix < -1) || (iy > 1 || iy < -1)) {
					std::cout << " error, check ix=" << ix << " iy=" << iy
							...
							<< posy - ymax[layer] << " step " << step
							<< std::endl;
					continue;
				} else
					idx = 3 * (iy + 1) + (ix + 1);
				Exy_[layer][idx] = energy;
				double realtime = calib_.correctTime(lHit.time(), posx, posy,
						avgZ_[layer], xvtx_, yvtx_, 0);
...
void PositionFit::getPuContribution(std::vector<HGCSSRecoHit> *rechitvec,
		const std::vector<double> & xmax, const std::vector<double> & ymax,
		std::vector<double> & puE) {
	...
	for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {  //loop on rechits
		...
		if (fabs(posx - xmax[layer]) < step
				&& fabs(posy - ymax[layer]) < step) {
			if (debug_ > 1)
				std::cout << "- iH " << iH << " x=" << posx << " xmax="
						<< xmax[layer] << " y=" << posy << " ymax="
						<< ymax[layer] << " step " << step << " --- Pass, layer"
						<< layer << " E=" << energy << std::endl;
			puE[layer] += energy;
		}
...
void PositionFit::fillErrorMatrix(
		const std::vector<ROOT::Math::XYPoint> & recoPos,
		const std::vector<unsigned> & nHits) {
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {  //loop on layers
		...
		if (fabs(residual_xi) > residualMax_
				|| fabs(residual_yi) > residualMax_)
			continue;
		...
		for (unsigned jL(0); jL < nLayers_; ++jL) {    //loop on layers
			...
			if (fabs(residual_xj) > residualMax_
					|| fabs(residual_yj) > residualMax_)
				continue;
...
void PositionFit::finaliseErrorMatrix(const bool doX) {
	...
	if (!fmatrix.is_open()) {
		std::cout << " Cannot open outfile " << fmatrixname.str()
				<< " for writing ! Exiting..." << std::endl;
		exit(1);
	}
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {  //loop on layers
		if (nL_mean_[iL] > 0)
			mean_[index][iL] = mean_[index][iL] / nL_mean_[iL];
		else
			mean_[index][iL] = 0;
	}
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {  //loop on layers
		for (unsigned jL(0); jL < nLayers_; ++jL) {  //loop on layers
			if (nL_sigma_[iL][jL] > 0)
				sigma_[index][iL][jL] = sigma_[index][iL][jL]
						/ nL_sigma_[iL][jL];
			else
				sigma_[index][iL][jL] = 0;
			...
			matrix_[index][iL][jL] = sigma_[index][iL][jL]
					- mean_[index][iL] * mean_[index][jL];
			//matrix_[jL][iL] = matrix_[iL][jL];
			if (matrix_[index][iL][jL] != matrix_[index][iL][jL])
				matrix_[index][iL][jL] = 0;
			fmatrix << iL << " " << jL << " " << std::setprecision(15)
					<< matrix_[index][iL][jL] << std::endl;
...
void PositionFit::fillCorrelationMatrix() {
	...
	p_errorMatrix_x = new TH2D("p_errorMatrix_x", ";i;j;M_{ij}", nLayers_, 0,
			nLayers_, nLayers_, 0, nLayers_);
	p_corrMatrix_x = new TH2D("p_corrMatrix_x", ";i;j;M_{ij}", nLayers_, 0,
			nLayers_, nLayers_, 0, nLayers_);
	p_errorMatrix_y = new TH2D("p_errorMatrix_y", ";i;j;M_{ij}", nLayers_, 0,
			nLayers_, nLayers_, 0, nLayers_);
	p_corrMatrix_y = new TH2D("p_corrMatrix_y", ";i;j;M_{ij}", nLayers_, 0,
			nLayers_, nLayers_, 0, nLayers_);
	...
	for (unsigned iL(0); iL < nLayers_; ++iL) {      //loop on layers
		for (unsigned jL(0); jL < nLayers_; ++jL) {      //loop on layers
			...
			if (matrix_[0][iL][iL] != 0 && matrix_[0][jL][jL] != 0) {
				corrMatrix_[0][iL][jL] = matrix_[0][iL][jL]
						/ sqrt(matrix_[0][iL][iL] * matrix_[0][jL][jL]);
				p_corrMatrix_x->Fill(iL, jL, corrMatrix_[0][iL][jL]);
			}
			if (matrix_[1][iL][iL] != 0 && matrix_[1][jL][jL] != 0) {
				corrMatrix_[1][iL][jL] = matrix_[1][iL][jL]
						/ sqrt(matrix_[1][iL][iL] * matrix_[1][jL][jL]);
...
bool PositionFit::fillMatrixFromFile(const bool doX, const bool old) {
	...
	while (!fmatrix.eof()) {
		...
		if (iL < nLayers_ && jL < nLayers_) {
			if (debug_ > 1)
				std::cout << std::setprecision(15) << iL << " " << jL << " "
						<< m << std::endl;
			matrix_[index][iL][jL] = m;
		} else if (debug_ > 1)
			std::cout << "!! out of bounds!" << iL << " " << jL << " " << m
					<< std::endl;
	}
...
bool PositionFit::initialiseLeastSquareFit() {
	...
	if (doMatrix_ && !fillMatrixFromFile())
		return false;
...
unsigned PositionFit::performLeastSquareFit(const unsigned ievt,
		FitResult & fit, const std::vector<unsigned> & lToRemove) {
...
void PositionFit::finaliseFit() {
	...
	std::cout << " -- Number of fits failed after cutting outliers: "
			<< nFailedFitsAfterCut_ << std::endl;

}

unsigned PositionFit::fitEvent(const unsigned ievt, FitResult & fit,
		const std::vector<unsigned> & lToRemove, const bool cutOutliers) {
	...
	if (!getPositionFromFile(ievt, layerId, posx, posy, posz, posxtruth,
			posytruth, Ereco, lToRemove, cutOutliers)) {
		nInvalidFits_++;
		return 1;
	}
	...
	for (unsigned rt(0); rt < 2; ++rt) {
		if (debug_) {
			std::cout << "... Processing ";
			if (rt == 0)
				std::cout << " fit to reco position.";
			else
				std::cout << " fit to truth position.";
			std::cout << std::endl;
		}
		double chiSq(0.0);
		double position[2];
		...
		for (unsigned xy(0); xy < 2; xy++) {    //loop on x or y
			if (debug_) {
				std::cout << "... Processing ";
				if (xy == 0)
					std::cout << " fit to x position.";
				else
					std::cout << " fit to y position.";
				std::cout << std::endl;
			}
			for (unsigned i(0); i < nL; i++) {
				x(i) = rt == 0 ?
						((xy == 0) ? posx[i] : posy[i]) :
						((xy == 0) ? posxtruth[i] : posytruth[i]);
			...
			if (w(0, 0) == w(0, 0))
				fitMatrix[2 * xy][2 * xy] = fabs(w(0, 0));
			if (w(0, 1) == w(0, 1))
				fitMatrix[2 * xy][2 * xy + 1] = w(0, 1);
			if (w(1, 0) == w(1, 0))
				fitMatrix[2 * xy + 1][2 * xy] = w(1, 0);
			if (w(1, 1) == w(1, 1))
				fitMatrix[2 * xy + 1][2 * xy + 1] = fabs(w(1, 1));
		...
		if (chiSq / ndf > chi2ndfmax_) {
			...
			std::cout << "ecal frontface position = " << positionFF[0][0] << " "
					<< positionFF[0][1] << " truth " << truthPos(0).X() << " "
					<< truthPos(0).Y() << std::endl;
			std::cout << "ecal frontface tanAngle = " << TanAngle[0][0] << " "
					<< TanAngle[0][1] << " truth dir " << truthDir_.tanangle_x
					<< " " << truthDir_.tanangle_y << std::endl;
			return 2;
		}
		...
		if (rt == 0) {
			...
			fout_ << ievt << " " << position[0] << " " << sqrt(fitMatrix[0][0])
					<< " " << TanAngle[0][0] << " " << sqrt(fitMatrix[1][1])
					<< " " << position[1] << " " << sqrt(fitMatrix[2][2]) << " "
					<< TanAngle[0][1] << " " << sqrt(fitMatrix[3][3]);
			//<< std::endl;
			...
			fout_ << " " << position[0] << " " << TanAngle[1][0] << " "
					<< position[1] << " " << TanAngle[1][1] << std::endl;
		}      //truth
...
bool PositionFit::getPositionFromFile(const unsigned ievt,
		std::vector<unsigned> & layerId, std::vector<double> & posx,
		std::vector<double> & posy, std::vector<double> & posz,
		std::vector<double> & posxtruth, std::vector<double> & posytruth,
		std::vector<double> & E, const std::vector<unsigned> & lToRemove,
		bool cutOutliers, bool print) {
	...
	while (!fin.eof()) {
		...
		if (l < nLayers_) {
			...
			bool pass = fabs(xr - xt) < residualMax_
					&& fabs(yr - yt) < residualMax_;
			bool keep = true;
