...
void Clusterizer::buildClusters(std::vector<HGCSSRecoHit> *rechitvec,
	...
	if (debug_)
		std::cout << " -- Building clusters out of " << rechits.size()
				<< " hits: " << std::endl;
	...
	if (debug_)
		std::cout << " -- Size of seed vec: " << seeds.size() << std::endl;

	if (debug_ > 1) {
		for (unsigned i = 0; i < seeds.size(); ++i) {
			std::cout << " Seed " << i << " index " << seeds[i] << " energy "
					<< rechits[seeds[i]].energy() << std::endl;
		}
	}
	...
	for (const unsigned i : seeds) {
		...
		if (debug_ > 1)
			std::cout << " - Seed idx " << i << " energy " << hit.energy()
					<< " layer " << hit.layer() << std::endl;
		//if(hit.neighbours8().size() > 0 ) {
		HGCSSCluster layer_cluster;
		build2DCluster(rechits, rechitMask, seedable, i, usable_rechits,
				layer_cluster);

		if (debug_ > 1)
			std::cout << " --- Cluster has : "
					<< layer_cluster.recHitFractions().size()
					<< " rechits associated. Energy of first ele: "
					<< layer_cluster.recHitFractions().begin()->first->energy()
					<< " weight = "
					<< layer_cluster.recHitFractions().begin()->second
					<< std::endl;
	...
	if (debug_)
		std::cout << " -- Number of clusters per layer: "
				<< clusters_per_layer.size() << std::endl;
	...
	if (debug_)
		std::cout << " -- Number of clusters after linking in z: "
				<< z_linked_clusters.size() << std::endl;
...
void Clusterizer::build2DCluster(const std::vector<HGCSSRecoHit> & rechitvec,
		const std::vector<bool>& rechitMask, const std::vector<bool>& seedable,
		const unsigned current_index, std::vector<bool>& usable,
	...
	if (debug_ > 1)
		std::cout << " -- Current index = " << current_index << " hit energy = "
				<< current_cell.energy() << std::endl;

	cluster.addRecHitFraction(
			std::pair<HGCSSRecoHit*, double>(
					const_cast<HGCSSRecoHit*>(&current_cell), 1.0));

	if (debug_ > 2)
		std::cout << " Cluster now has : " << cluster.recHitFractions().size()
				<< " rechits associated. Energy of first ele: "
				<< cluster.recHitFractions().begin()->first->energy()
				<< std::endl;
	...
	if (debug_ > 1)
		std::cout << " -- Number of closest neighbours found: " << found.size()
				<< std::endl;

	for (const KDNode& nbourpoint : found) {
		// only cluster if not a seed, not used, and energy less than present
		if (debug_ > 2)
			std::cout << " Neighbour index: " << nbourpoint.data << " usable = "
					<< usable[nbourpoint.data] << " seedable "
					<< seedable[nbourpoint.data] << " energy "
					<< rechitvec[nbourpoint.data].energy() << " rechitmask "
					<< rechitMask[nbourpoint.data] << std::endl;
		...
		if (usable[nbourpoint.data] && !seedable[nbourpoint.data]
			...
			build2DCluster(rechitvec, rechitMask, seedable, nbourpoint.data,
					usable, cluster);
		}
...
void Clusterizer::linkClustersInLayer(const HGCSSClusterVec & input_clusters,
		HGCSSClusterVec & output) {

	std::vector<bool> dummy(input_clusters.size(), true);
	KDTreeCube kd_boundingregion = fill_and_bound_kd_tree(input_clusters, dummy,
			_cluster_nodes);
	...
	for (unsigned i = 0; i < input_clusters.size(); ++i) {
		...
		//CAMM: why 2* ?
		auto z = minmax(pos.Z() + 2 * moliere_radius,
				pos.Z() - 2 * moliere_radius);
		KDTreeCube kd_searchcube((float) x.first, (float) x.second,
				(float) y.first, (float) y.second, (float) z.first,
				(float) z.second);
		...
		for (const auto& found_node : _found) {
			...
			if (diff_pos.rho() < moliere_radius
					&& std::abs(diff_pos.Z()) > 0.03) {
				if (pos.mag2() > found_pos.mag2()) {
					back_links.insert(
							std::pair<unsigned, unsigned>(i, found_node.data));
				} else {
					back_links.insert(
							std::pair<unsigned, unsigned>(found_node.data, i));
				}
			}
		}
		_found.clear();
	...
	for (unsigned i = 0; i < input_clusters.size(); ++i) {
		...
		for (auto connected = clusrange.first; connected != clusrange.second;
				++connected) {
			const auto& pos_connected =
					input_clusters[connected->second].position();
