...
int main() {
	...
	for (size_t i = 0; i < 4; i++) {
		TString pf("");
		pf += i;
		TH1F *h = new TH1F("stochgr" + pf,
				";Detector version;(#sigma/E)_{stochastic}", nversions, 0,
				nversions);
		h->SetMarkerStyle(20 + i);
		h->SetDirectory(0);
		stochGr.push_back(h);
		h = new TH1F("constgr" + pf, ";Detector version;(#sigma/E)_{cte}",
				nversions, 0, nversions);
		h->SetMarkerStyle(20 + i);
		h->SetDirectory(0);
		constGr.push_back(h);
	}
	...
	for (size_t iv = 0; iv < nversions; iv++) {
		...
		if (i == v_HGCALEE_Si80)
			binLabel = "Si 80 #mum";
		if (i == v_HGCALEE_Si120)
			binLabel = "Si 120 #mum";
		if (i == v_HGCALEE_Si200)
			binLabel = "Si 200 #mum";
		if (i == v_HGCALEE_Si500)
			binLabel = "Si 500 #mum";
		if (i == v_HGCALEE_gap1)
			binLabel = "Air gap 1mm";
		if (i == v_HGCALEE_CALICE)
			binLabel = "#splitline{CALICE-like}{sampling}";
		if (i == v_HGCALEE_fineSampling)
			binLabel = "Fine sampling";
		if (i == v_HGCALEE_inverted)
			binLabel = "Inverted";
		if (i == v_HGCALEE_concept)
			binLabel = "Concept";
		if (i == v_HGCALEE_W)
			binLabel = "W absorber";
		if (i == v_HGCALEE_gap4)
			binLabel = "Air gap 4mm";
		...
		for (size_t ialgo = 0; ialgo < 3; ialgo++) {
			...
			stochGr[ialgo]->SetBinContent(iv + 1,
					props.stochTerms_[ialgo].first);
			stochGr[ialgo]->SetBinError(iv + 1,
					props.stochTerms_[ialgo].second);
			constGr[ialgo]->SetTitle(props.resCurve_[ialgo]->GetTitle());
			constGr[ialgo]->GetXaxis()->SetBinLabel(iv + 1, binLabel);
			constGr[ialgo]->SetBinContent(iv + 1,
					props.constTerms_[ialgo].first);
			constGr[ialgo]->SetBinError(iv + 1,
					props.constTerms_[ialgo].second);
		}
	}
