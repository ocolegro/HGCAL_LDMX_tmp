...
int main(int argc, char** argv) { //main
	...
	if (argc < 8) {
		std::cout << " Usage: " << argv[0] << " <mipCut-200um>" << std::endl
				<< " <eosPath>" << std::endl << " <version>" << std::endl
				<< " <pt>" << std::endl << " <eta>" << std::endl << " <nRuns>"
				<< std::endl << " <nEvts to process (0=all)>" << std::endl
				<< std::endl;
		return 1;
	}
	...
	inFilePath << eosPath << "HGcal__version" << version << "_model2_BOFF_et"
			<< pt << "_eta" << eta;
	if (eta == "1.75" || eta == "2.25" || eta == "2.75")
		inFilePath << "0";
	else
		inFilePath << "00";
	if (nRuns == 0)
		...
		inFilePath << "_run0.root";

	std::ostringstream inputrec;
	inputrec << eosPath << "DigiIC3__version" << version << "_model2_BOFF_et"
			<< pt << "_eta" << eta;
	if (eta == "1.75" || eta == "2.25" || eta == "2.75")
		inputrec << "0";
	else
	...
	TTree *outtree = new TTree("outtree",
			"Output tree with cells above 26 mips");
	unsigned nover = 0;
	unsigned ntot = 0;
	outtree->Branch("nover", &nover);
	outtree->Branch("ntot", &ntot);
	TH1F *nAbove = new TH1F("nAbove", ";n_{cells}(E>26 MIPs);Probability", 20,
			0, 20);
	TH1F *nTot = new TH1F("nTot", ";n_{cells};events", 200, 0, 1000);
	TH2D *hxy = new TH2D("hxy", ";x (mm);y (mm);cells E>26 MIPs", 339, -1695,
			1695, 339, -1695, 1695);
	...
	if (!inputSim) {
		std::cout << " -- Error, input file " << inFilePath.str()
	...
	HGCSSGeometryConversion geomConv(inFilePath.str(), info->model(),
			info->cellSize());
	...
	std::cout << " -- Processing " << nEvts << " events." << std::endl
			<< " -- mipCut is set to " << mipCut << " mips for 200um Si."
			<< std::endl;
	...
	lcut << "HGCSSRecoHitVec.energy_>" << mipCut
			<< " && HGCSSRecoHitVec.layer_<" << nLayers - 12;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {  //loop on hits
			...
			const HGCSSSubDetector & subdet = myDetector.subDetectorByLayer(
					layer);
			if (subdet.type == DetectorEnum::BHCAL1
					|| subdet.type == DetectorEnum::BHCAL2)
				continue;
			double lRadius = sqrt(pow(lHit.get_x(), 2) + pow(lHit.get_y(), 2));
			double cutVal = mipCut * 2.
					/ geomConv.getNumberOfSiLayers(subdet.type, lRadius);
			if (energy > cutVal) {
				nover++;
			}
			ntot++;
