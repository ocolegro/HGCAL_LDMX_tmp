...
int main(int argc, char** argv) { //main  
	...
	std::cout << " -- Input parameters: " << std::endl
			...
			<< " -- Number cells in signal region for fit: " << nSR
			<< " *2.5*2.5 mm^2 cells" << std::endl << " -- Processing ";
	if (pNevts == 0)
	...
	TH1F *p_ErecoTotal = new TH1F("p_ErecoTotal", ";Ereco (MIPs)", 5000, 0,
			50000);
	...
	TH1F *p_residuals_x = new TH1F("p_residuals_x", ";xreco-xtruth (mm)", 1000,
			-50, 50);
	TH1F *p_residuals_y = new TH1F("p_residuals_y", ";yreco-ytruth (mm)", 1000,
			-50, 50);
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		lName.str("");
		lName << "p_xy_" << iL;
		p_xy[iL] = new TH2F(lName.str().c_str(), ";x(mm);y(mm)", nX, minX, maxX,
				nY, minY, maxY);
		lName.str("");
		lName << "p_recoxy_" << iL;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		TH2F *etavsphi = new TH2F("etavsphi", ";#phi;#eta;hits", 150, -3.1416,
				3.1416, 160, 1.4, 3.0);
		...
		for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) {    //loop on hits
			...
			geomConv.fill(type, subdetLayer, energy, lRealTime, posx, posy,
					posz);
			...
			double absweight = (*ssvec)[layer].volX0trans()
					/ (*ssvec)[1].volX0trans();
		...
		for (unsigned iP(0); iP < (*genvec).size(); ++iP) { //loop on gen particles    
			...
			if ((*genvec)[iP].trackID() == 1) {
				...
				double p = sqrt(
						pow((*genvec)[iP].px(), 2) + pow((*genvec)[iP].py(), 2)
								+ pow((*genvec)[iP].pz(), 2));
				...
				ROOT::Math::XYZVector unit((*genvec)[iP].px() / p,
						(*genvec)[iP].py() / p, (*genvec)[iP].pz() / p);

				//std::cout << " Gen particle eta,phi = " << unit.eta() << " " << unit.phi() << std::endl;

				for (unsigned iL(0); iL < nLayers; ++iL) {
					if (avgZ[iL] < z0)
						avgZ[iL] = geomConv.getAverageZ(iL);
					if (avgZ[iL] > z0) {
		...
		for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) { //loop on rechits
			...
			if (layer >= nLayers) {
				std::cout << " WARNING! RecoHits with layer " << layer
						<< " outside of detector's definition range ! Please fix the digitiser or the detector definition used here. Ignoring..."
						<< std::endl;
				continue;
			}
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {      //loop on layers
			...
			if (debug)
				std::cout << iL << " nHits=" << nHits[iL] << " Max=("
						<< xmax[iL] << "," << ymax[iL] << ")\t Reco=("
						<< recoPos[iL].X() << "," << recoPos[iL].Y()
						<< ")\t Truth=(" << truthPos[iL].X() << ","
						<< truthPos[iL].Y() << ")" << std::endl;
			fout << iL << " " << recoPos[iL].X() << " " << recoPos[iL].Y()
					<< " " << avgZ[iL] << " " << truthPos[iL].X() << " "
					<< truthPos[iL].Y() << std::endl;
		}
		for (unsigned iL(0); iL < nLayers; ++iL) {      //loop on layers
			if (nHits[iL] == 0)
			...
			if (fabs(residual_xi) > residualMax
					|| fabs(residual_yi) > residualMax)
				continue;
			...
			for (unsigned jL(0); jL < nLayers; ++jL) {      //loop on layers
				...
				if (fabs(residual_xj) > residualMax
						|| fabs(residual_yj) > residualMax)
					continue;
	...
	std::cout << " -- Total Ereco in MIPS: " << p_ErecoTotal->GetEntries()
	...
	outputFile->cd();
	TH2F *p_errorMatrix = new TH2F("p_errorMatrix", ";i;j;M_{ij}", nLayers, 0,
			nLayers, nLayers, 0, nLayers);
	TH2F *p_corrMatrix = new TH2F("p_corrMatrix", ";i;j;M_{ij}", nLayers, 0,
			nLayers, nLayers, 0, nLayers);
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {      //loop on layers
		for (unsigned jL(0); jL < nLayers; ++jL) {      //loop on layers
			if (matrix[iL][iL] != 0 && matrix[jL][jL] != 0)
				p_corrMatrix->Fill(iL, jL,
						matrix[iL][jL] / sqrt(matrix[iL][iL] * matrix[jL][jL]));
		}
	}
	...
	TH1F *p_impactX[2];
	p_impactX[0] = new TH1F("p_impactX", ";x front face impact (mm);n_{events}",
			200, -500, 500);
	p_impactX[1] = new TH1F("p_impactX_truth",
			";x front face impact (mm);n_{events}", 200, -500, 500);
	TH1F *p_impactY[2];
	p_impactY[0] = new TH1F("p_impactY", ";y front face impact (mm);n_{events}",
			240, 300, 1500);
	p_impactY[1] = new TH1F("p_impactY_truth",
			";y front face impact (mm);n_{events}", 240, 300, 1500);
	TH1F *p_angleX[2];
	p_angleX[0] = new TH1F("p_angleX", ";x direction angle (rad);n_{events}",
			150, -3.1416, 3.1416);
	p_angleX[1] = new TH1F("p_angleX_truth",
			";x direction angle (rad);n_{events}", 150, -3.1416, 3.1416);
	TH1F *p_angleY[2];
	p_angleY[0] = new TH1F("p_angleY", ";y direction angle (rad);n_{events}",
			150, -3.1416, 3.1416);
	p_angleY[1] = new TH1F("p_angleY_truth",
			";y direction angle (rad);n_{events}", 150, -3.1416, 3.1416);

	TH1F *p_positionReso[2];
	TH1F *p_angularReso[2];
	p_positionReso[0] = new TH1F("p_positionResoX",
			";#sigma_{x,y} (mm);n_{events}", 100, 0, 20);
	p_positionReso[1] = new TH1F("p_positionResoY",
			";#sigma_{x,y} (mm);n_{events}", 100, 0, 20);
	p_angularReso[0] = new TH1F("p_angularResoX",
			";#sigma_{#theta} (rad);n_{events}", 100, 0, 1);
	p_angularReso[1] = new TH1F("p_angularResoY",
			";#sigma_{#theta} (rad);n_{events}", 100, 0, 1);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {      //loop on entries
		...
		for (unsigned rt(0); rt < 2; ++rt) {
			...
			if (rt == 0) {
				...
				fout << ievt << " " << position[0] << " "
						<< sqrt(fabs(fitMatrix[0][0])) << " " << TanAngle[0]
						<< " " << sqrt(fabs(fitMatrix[1][1])) << " "
						<< position[1] << " " << sqrt(fabs(fitMatrix[2][2]))
						<< " " << TanAngle[1] << " "
						<< sqrt(fabs(fitMatrix[3][3])) << std::endl;
			}
