...
int main(int argc, char** argv) { //main  
	...
	std::cout << " -- Input parameters: " << std::endl
			...
			<< " -- Number cells in signal region for fit: " << nSR << " cells"
			<< std::endl
	...
	PositionFit lChi2Fit(nSR, residualMax, nLayers, nSiLayers, applyPuMixFix,
			debug);
	lChi2Fit.initialise(outputFile, "PositionFit", outFolder, geomConv,
			puDensity);

	//try getting z position from input file, if doesn't exit,
	//perform first loop over simhits to find z positions of layers
	if ((redoStep < 2 && !lChi2Fit.getZpositions(versionNumber))
			|| redoStep > 1)
	...
	if (!dofit && redoStep == 0)
		std::cout << " -- Info: fit positions taken from file on disk."
				<< std::endl;
	else
		std::cout << " -- Info: redoing least square fit." << std::endl;
		...
		if (dofit) {
			...
			for (unsigned r(0); r < nRemove + 1; ++r) {
				...
				if (lChi2Fit.performLeastSquareFit(ievt, fit, lToRemove) == 0) {
					...
					std::cout << " -- remove " << r << " Fit failed."
							<< std::endl;
				if (r < nRemove)
					lToRemove.push_back(list[r]);
			}

		} else
			SignalEnergy.fillEnergies(ievt, (*ssvec), (*simhitvec),
					(*rechitvec), nPuVtx);
