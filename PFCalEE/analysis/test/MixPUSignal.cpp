...
int main(int argc, char** argv) { //main  
	...
	if (argc < 9) {
		std::cout << " Usage: " << argv[0] << " <nEvts to process (0=all)>"
				<< " <path to MinBias file>" << " <MinBias file name>"
				<< " <path to signal file>" << " <signal file name>"
				<< " <name of input sim file>" << " <full path to output file>"
				<< " <Number of PU to add (140)>" << std::endl;
		return 1;
	}
	...
	if (!signalFile) {
		std::cout << " -- Error, input file " << signalInput.str()
				<< " cannot be opened. Exiting..." << std::endl;
		return 1;
	} else
		std::cout << " -- input file " << signalFile->GetName()
				<< " successfully opened." << std::endl;
	...
	const unsigned nEvts = (
			(pNevts > signalTree->GetEntries() || pNevts == 0) ?
					static_cast<unsigned>(signalTree->GetEntries()) : pNevts);
	...
	TFile *outputFile = TFile::Open((outPath + "/PuMix.root").c_str(),
			"RECREATE");

	if (!outputFile) {
		std::cout << " -- Error, output file " << outPath
				<< "/PuMix.root cannot be opened. Please create output directory. Exiting..."
				<< std::endl;
		return 1;
	} else {
		...
		for (unsigned iV(0); iV < nPuVtx; ++iV) {  //loop on interactions
			...
			for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) { //loop on hits
				...
				if (inFid) {
					...
					double posy = lHit.get_y();
					double posz = lHit.get_z();
					geomConv.fill(type, subdetLayer, energy, 0, posx, posy,
							posz);
				}
