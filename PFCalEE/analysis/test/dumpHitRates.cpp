...
int main(int argc, char** argv) { //main  
	...
	if (argc < 5) {
		std::cout << " Usage: " << argv[0] << " <nEntries to process (0=all)>"
				<< " <number of runs>"
				...
				<< " <path to output file>" << " <optional: debug (default=0)>"
				<< std::endl;
		return 1;
	}
	...
	std::cout << " -- Input parameters: " << std::endl << " -- Input file: "
			<< filePath << std::endl << " -- Output file: " << outPath
			<< std::endl << " -- Processing ";
	if (pNevts == 0)
	...
	TFile * inputFile = 0;
	if (nRuns == 0) {
		if (!testInputFile(filePath, inputFile))
			return 1;
		...
		for (unsigned i(0); i < nRuns; ++i) {
			...
			if (!testInputFile(lstrrec.str(), inputFile))
				continue;
	...
	for (unsigned it(0); it < nT; ++it) {
		...
		if (!hitsOut[it].is_open()) {
			std::cout
					<< " -- Cannot open output file for writting ! Please create directory: "
					<< lstrrec.str() << std::endl;
			return 1;
		}
	}
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
		...
		if (debug) {
			std::cout << "...Number of rechits  " << (*rechitvec).size() << "."
					<< std::endl;
		}
		...
		for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) { //loop on hits
			...
			unsigned sector =
					phi >= 0 ?
							6
									+ static_cast<unsigned>(phi
											/ (2 * TMath::Pi()) * nSectors) :
							5
									- static_cast<unsigned>(fabs(phi)
											/ (2 * TMath::Pi()) * nSectors);

			if (sector == 12)
				sector = 0;
			if (sector >= nSectors) {
				std::cout << " Wrong sector value! phi=" << phi << " sector="
						<< sector << std::endl;
				return 1;
			}
