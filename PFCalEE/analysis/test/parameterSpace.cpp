...
void calculateLength(const std::vector<SamplingLayer> & fullstruct,
		double & length, double & X0tot, double & L0tot) {
	length = 0;
	X0tot = 0;
	L0tot = 0;
...
void fillFullStruct(std::vector<SamplingLayer> & fullstruct,
		const std::vector<double> & wThick, const std::vector<double> & pbThick,
	...
	lThick1.push_back(0);
	lEle1.push_back("W");
	lThick1.push_back(0.5);
	lEle1.push_back("Cu");
	lThick1.push_back(airThick);
	lEle1.push_back("Air");
	lThick1.push_back(pcbThick);
	lEle1.push_back("PCB");
	lThick1.push_back(0.3);
	lEle1.push_back("Si");
	...
	lThickL.push_back(1);
	lEleL.push_back("Pb");
	lThickL.push_back(wThick[0]);
	lEleL.push_back("W");
	lThickL.push_back(0.5);
	lEleL.push_back("Cu");
	lThickL.push_back(airThick);
	lEleL.push_back("Air");
	lThickL.push_back(pcbThick);
	lEleL.push_back("PCB");
	lThickL.push_back(0.3);
	lEleL.push_back("Si");
	...
	lThickR.push_back(3);
	lEleR.push_back("Cu");
	lThickR.push_back(pbThick[0]);
	lEleR.push_back("Pb");
	lThickR.push_back(3);
	lEleR.push_back("Cu");
	lThickR.push_back(0.3);
	lEleR.push_back("Si");
	lThickR.push_back(pcbThick);
	lEleR.push_back("PCB");
	lThickR.push_back(airThick);
	lEleR.push_back("Air");
...
int main(int argc, char** argv) {  //main  
	...
	if (argc < 2) {
		std::cout << " Usage: " << argv[0] << " <nLayers>" << std::endl;
		return 1;
	}
	...
	fillFullStruct(fullstruct, wThick, pbThick, nLayers);
	...
	std::cout << " -- Starting point:" << std::endl
			<< " ------  Total length = " << length << "mm , nX0 = " << X0tot
			<< ", nL0 = " << L0tot << std::endl;

	if (length > scan.maxThick || L0tot > scan.maxLambda) {
		std::cout << " ERROR! Minimum model is already out from max length..."
				<< std::endl;
		return 1;
	}
	...
	std::cout << " ------  Total length = " << tmptot << "mm , nX0 = "
			<< tmpxtot << ", nL0 = " << tmpltot << std::endl;

	if (tmpxtot < scan.minX0) {
		std::cout << " ERROR! Maximum model is already out from min X0..."
				<< std::endl;
		return 1;
	}
	...
	std::cout << " -- Middle scanned: " << std::endl
			<< " ------  Total length = " << midlength << "mm , nX0 = " << midx0
			<< ", nL0 = " << midl0 << std::endl;

	if (midlength > scan.maxThick || midl0 > scan.maxLambda
			|| midx0 < scan.minX0) {
		std::cout
				<< " WARNING! Middle model is already out... Please check it is OK..."
				<< std::endl;
		//return 1;
	}
	...
	for (unsigned iC(0); iC < nC; ++iC) {
		...
		label << "myc" << iC;
		scan.myc[iC] = new TCanvas(label.str().c_str(), label.str().c_str(), 1);
		scan.first[iC] = true;
		scan.counter[iC] = 0;
	}
	...
	std::cout << " Found " << scan.validModels << " valid models out of "
			<< scan.totModels << std::endl;

	std::cout << " -- number of uniq X0tot: " << scan.modelMap.size()
			<< std::endl;
	std::map<double, unsigned>::iterator iter;
	unsigned count = 0;
	for (iter = scan.modelMap.begin(); iter != scan.modelMap.end();
			++iter, ++count) {
		std::cout << count << " " << std::setprecision(10) << iter->first << " "
				<< iter->second << std::endl;
	}

	//  summary->Draw("colz");

	std::cout << " -- Found " << scan.counter[0] << " models for ordered"
			<< std::endl;
	std::cout << " -- Found " << scan.counter[1] << " models for down-up"
			<< std::endl;
	std::cout << " -- Found " << scan.counter[2] << " models for up-down"
			<< std::endl;
	std::cout << " -- Found " << scan.counter[3] << " models for others"
			<< std::endl;
