...
int main(int argc, char** argv) { //main  
	...
	if (argc < 3) {
		std::cout << " Usage: " << argv[0] << " <nEvts to process (0=all)>"
				...
				<< " <file name (PFcal.root, or DigiPFcal.root)>" << std::endl;
		return 1;
	}
	...
	if (!lTree) {
		std::cout
				<< " -- Error, tree HGCSSTree cannot be opened. Trying RecoTree instead."
				<< std::endl;
		isG4Tree = false;
		lTree = (TTree*) inputFile->Get("RecoTree");
		if (!lTree) {
			std::cout
					<< " -- Error, tree RecoTree cannot be opened either. Exiting..."
					<< std::endl;
			return 1;
		}
	}
	...
	TFile *outputFile = TFile::Open("140PU/PFcal_140PU_EEHE_full.root",
			"RECREATE");

	if (!outputFile) {
		std::cout << " -- Error, cannot open output file. Exiting..."
				<< std::endl;
		return 1;
	}
	...
	for (unsigned evtOut(0); evtOut < nEvtsOut; ++evtOut) { //loop on output events
		...
		for (unsigned iV(0); iV < lVtx; ++iV) {  //loop on interactions
			...
			std::cout << " -- PU Random number #" << iV << " for event "
					<< evtOut << " is " << ipuevt[iV] << std::endl;
		}
		...
		for (unsigned iV(0); iV < lVtx; ++iV) {  //loop on interactions
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {  //loop on layers
				...
				for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) { //loop on hits
					...
					bool inFid = posx > minX && posx < maxX && posy > minY
							&& posy < maxY && posz > minZ && posz < maxZ;

					if (inFid)
						tmpvec[iL].push_back(lHit);
