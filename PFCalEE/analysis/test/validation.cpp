...
bool testInputFile(std::string input, TFile* & file) {
	...
	if (!file) {
		std::cout << " -- Error, input file " << input.c_str()
				<< " cannot be opened. Skipping..." << std::endl;
		return false;
	} else
		std::cout << " -- input file " << file->GetName()
...
int main(int argc, char** argv) { //main  
	...
	preconfig.add_options()("cfg,c",
			po::value < std::string > (&cfg)->required());
	po::variables_map vm;
	po::store(
			po::command_line_parser(argc, argv).options(preconfig).allow_unregistered().run(),
			vm);
	po::notify(vm);
	po::options_description config("Configuration");
	config.add_options()
	//Input output and config options //->required()
	("concept", po::value<bool>(&concept)->default_value(true))("pNevts,n",
			po::value<unsigned>(&pNevts)->default_value(0))("filePath,i",
			po::value < std::string > (&filePath)->required())("digifilePath",
			po::value < std::string > (&digifilePath)->default_value(""))(
			"nRuns", po::value<unsigned>(&nRuns)->default_value(0))(
			"simFileName,s",
			po::value < std::string > (&simFileName)->required())(
			"recoFileName,r",
			po::value < std::string > (&recoFileName)->required())("outPath,o",
			po::value < std::string > (&outPath)->required())("nSiLayers",
			po::value<unsigned>(&nSiLayers)->default_value(2))("debug,d",
			po::value<unsigned>(&debug)->default_value(0));

	// ("output_name,o",            po::value<std::string>(&outputname)->default_value("tmp.root"))

	po::store(
			po::command_line_parser(argc, argv).options(config).allow_unregistered().run(),
			vm);
	...
	if (pNevts == 0)
		...
		std::cout << pNevts << " events." << std::endl;
	...
	if (recoFileName.find("Digi") != recoFileName.npos)
		...
		lRecTree = new TChain("PUTree");

	if (nRuns == 0) {
		if (!testInputFile(inputsim.str(), simFile))
			return 1;
		lSimTree->AddFile(inputsim.str().c_str());
		if (!testInputFile(inputrec.str(), recFile))
			return 1;
		...
		for (unsigned i(0); i < nRuns; ++i) {
			...
			if (testInputFile(lstr.str(), simFile)) {
				if (simFile)
					...
					std::cout
							<< " -- Error in getting information from simfile!"
							<< std::endl;
					return 1;
				}
			} else
			...
			if (!testInputFile(lstr.str(), recFile))
				continue;
	...
	if (!lSimTree) {
		std::cout << " -- Error, tree HGCSSTree cannot be opened. Exiting..."
				<< std::endl;
		return 1;
	}

	if (!lRecTree) {
		std::cout << " -- Error, tree RecoTree cannot be opened. Exiting..."
				<< std::endl;
		return 1;
	}
	...
	std::cout << " -- Version number is : " << versionNumber << ", model = "
			<< model << ", cellSize = " << cellSize << std::endl;
	...
	if (!outputFile) {
		std::cout << " -- Error, output file " << outPath
				<< " cannot be opened. Please create output directory. Exiting..."
				<< std::endl;
		return 1;
	} else {
		std::cout << " -- output file " << outputFile->GetName()
				<< " successfully opened." << std::endl;
	}

	std::cout << " -- 2-D histograms: " << std::endl << " -- X: " << nX << " "
			<< minX << " " << maxX << std::endl << " -- Y: " << nY << " "
			<< minY << " " << maxY << std::endl << " -- Z: " << nZ << " "
			<< minZ << " " << maxZ << std::endl;
	outputFile->cd();
	TH2F *p_EsimvsLayer = new TH2F("p_EsimvsLayer", ";layer ; Esim (MIPs)",
			nLayers, 0, nLayers, 1000, 0, 5000);
	TH2F *p_ErecovsLayer = new TH2F("p_ErecovsLayer", ";layer ; Ereco (MIPs)",
			nLayers, 0, nLayers, 1000, 0, 5000);
	TH1F *p_timeSim = new TH1F("p_timeSim", ";G4 time (ns)", 1000, 0, 1000);
	TH2F *p_HCALvsECAL = new TH2F("p_HCALvsECAL", ";ECAL (GeV);HCAL (GeV)", 500,
			0, 500, 500, 0, 500);

	TH2F *p_BHCALvsFHCAL = new TH2F("p_BHCALvsFHCAL",
			";FHCAL (GeV);BHCAL (GeV)", 1000, 0, 2000, 1000, 0, 2000);

	TH2F *p_G4_BHCALvsFHCAL = new TH2F("p_G4_BHCALvsFHCAL",
			";FHCAL (GeV);BHCAL (GeV)", 5000, 0, 100000, 5000, 0, 100000);

	TH1F *p_nGenPart = new TH1F("p_nGenPart", ";n(genParticles)", 200, 0, 200);
	TH1F *p_genPartId = new TH1F("p_genPartId", ";pdgid", 12000, -6000, 6000);

	TH1F *p_firstInteraction = new TH1F("p_firstInteraction",
			";layer with 1st nucl. int.", nLayers, 0, nLayers);

	TH1F *p_maxEhit_2d5 = new TH1F("p_maxEhit_2d5",
			";maxE (MIPS) in 2.5 #times 2.5 mm^{2} cell; n_{events}", 5000, 0,
			5000);
	TH1F *p_maxEhit_5 = new TH1F("p_maxEhit_5",
			";maxE (MIPS) in 5 #times 5 mm^{2} cell; n_{events}", 5000, 0,
			15000);
	TH1F *p_maxEhit_10 = new TH1F("p_maxEhit_10",
			";maxE (MIPS) in 10 #times 10 mm^{2} cell; n_{events}", 5000, 0,
			15000);
	TH1F *p_maxEhit_15 = new TH1F("p_maxEhit_15",
			";maxE (MIPS) in 15 #times 15 mm^{2} cell; n_{events}", 5000, 0,
			15000);

	TH1F *p_maxEhit_ee = new TH1F("p_maxEhit_ee",
			";maxE (MIPS) in ECAL; n_{events}", 5000, 0, 15000);
	TH1F *p_maxEhit_fh = new TH1F("p_maxEhit_fh",
			";maxE (MIPS) in FHCAL; n_{events}", 5000, 0, 15000);
	TH1F *p_maxEhit_bh = new TH1F("p_maxEhit_bh",
			";maxE (MIPS) in BHCAL; n_{events}", 5000, 0, 15000);

	TH1F *p_layerMaxEHit = new TH1F("p_layerMaxEHit",
			";layer with max E hit; n_{events}", nLayers, 0, nLayers);

	TH2F *p_layerMaxEHitvsEphotonmax = new TH2F("p_layerMaxEHitvsEphotonmax",
			";max E^{#gamma} (GeV);layer with max E hit", 1000, 0, 3000,
			nLayers, 0, nLayers);
	...
	TH1F *p_nAboveMax_5 = new TH1F("p_nAboveMax_5",
			";n(E>maxE) 5 #times 5 mm^{2} cell; n_{events}", 50, 0, 50);
	TH1F *p_nAboveMax_10 = new TH1F("p_nAboveMax_10",
			";n(E>maxE) 10 #times 10 mm^{2} cell; n_{events}", 50, 0, 50);
	TH1F *p_nAboveMax_15 = new TH1F("p_nAboveMax_15",
			";n(E>maxE) 15 #times 15 mm^{2} cell; n_{events}", 50, 0, 50);

	TH1F *p_nAbove1_5 = new TH1F("p_nAbove1_5",
			";n(E>1 MIP) 5 #times 5 mm^{2} cell; n_{events}", 1000, 0, 1000);
	TH1F *p_nAbove1_10 = new TH1F("p_nAbove1_10",
			";n(E>1 MIP) 10 #times 10 mm^{2} cell; n_{events}", 1000, 0, 1000);
	TH1F *p_nAbove1_15 = new TH1F("p_nAbove1_15",
			";n(E>1 MIP) 15 #times 15 mm^{2} cell; n_{events}", 1000, 0, 1000);
	p_nAbove1_5->StatOverflows();
	p_nAbove1_10->StatOverflows();
	p_nAbove1_15->StatOverflows();

	TH1F *p_fracEmissed_5 = new TH1F("p_fracEmissed_5",
			";#frac{E_{missed}}{E_{tot}} 5 #times 5 mm^{2} cell;n_{events}",
			500, 0, 1);
	TH1F *p_fracEmissed_10 = new TH1F("p_fracEmissed_10",
			";#frac{E_{missed}}{E_{tot}} 10 #times 10 mm^{2} cell;n_{events}",
			500, 0, 1);
	TH1F *p_fracEmissed_15 = new TH1F("p_fracEmissed_15",
			";#frac{E_{missed}}{E_{tot}} 15 #times 15 mm^{2} cell;n_{events}",
			500, 0, 1);
	...
	std::cout << " -- Values taken for <MaxEhit>: 5x5=" << maxEavg5
			<< ", 10x10=" << maxEavg10 << ", 15x15=" << maxEavg15 << std::endl;

	TH1F *p_nSimHits = new TH1F("p_nSimHits", "n(SimHits)", 1000, 0, 500000);
	p_nSimHits->StatOverflows();

	TH1F *p_nRecHits = new TH1F("p_nRecHits", "n(RecHits)", 1000, 0, 5000);
	p_nRecHits->StatOverflows();

	TH1F *p_EsimTotal = new TH1F("p_EsimTotal", ";Esim (MIPs)", 30000, 0,
			300000);
	TH1F *p_ErecoTotal = new TH1F("p_ErecoTotal", ";Ereco (GeV)", 2000, 0,
			1000);
	...
	for (unsigned iD(0); iD < nSections; ++iD) {
		...
		if (myDetector.detType(iD) == DetectorEnum::BHCAL1
				|| myDetector.detType(iD) == DetectorEnum::BHCAL2)
			p_Esim[iD] = new TH1F(lName.str().c_str(), ";Esim (MIPs)", 2000, 0,
					20000);
		else
			p_Esim[iD] = new TH1F(lName.str().c_str(), ";Esim (MIPs)", 20000, 0,
					200000);
		...
		if (myDetector.detType(iD) == DetectorEnum::BHCAL1
				|| myDetector.detType(iD) == DetectorEnum::BHCAL2)
			p_Ereco[iD] = new TH1F(lName.str().c_str(), ";Ereco (MIPs)", 200, 0,
					2000);
		else
			p_Ereco[iD] = new TH1F(lName.str().c_str(), ";Ereco (MIPs)", 1000,
					0, 10000);
	...
	const unsigned nEvts = (
			(pNevts > lSimTree->GetEntries() || pNevts == 0) ?
					static_cast<unsigned>(lSimTree->GetEntries()) : pNevts);

	std::cout << "- Processing = " << nEvts << " events out of "
			<< lSimTree->GetEntries() << std::endl;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iP(0); iP < (*genvec).size(); ++iP) { //loop on gen particles
			...
			if ((*genvec)[iP].pdgid() == 22 && (*genvec)[iP].E() > maxEphoton)
				maxEphoton = (*genvec)[iP].E();
		...
		if (!skipSimStuff) {
			...
			for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) { //loop on hits
				...
				if (layer != prevLayer) {
					const HGCSSSubDetector & subdet =
							myDetector.subDetectorByLayer(layer);
					type = subdet.type;
					subdetLayer = layer - subdet.layerIdMin;
					prevLayer = layer;
					if (debug > 1)
						std::cout << " - layer " << layer << " " << subdet.name
								<< " " << subdetLayer << std::endl;
				}

				unsigned sec = myDetector.getSection(layer);

				if (firstInteraction == 0
						&& (lHit.nNeutrons() > 0 || lHit.nProtons() > 0
								|| lHit.nHadrons() > 0)
				...
				geomConv2d5.fill(type, subdetLayer, energy, lRealTime, posx,
						posy, posz);
				if (layer == 11 || layer == 18)
					geomConv5.fill(type, subdetLayer, energy, lRealTime, posx,
							posy, posz);
				geomConv10.fill(type, subdetLayer, energy, lRealTime, posx,
						posy, posz);
				geomConv15.fill(type, subdetLayer, energy, lRealTime, posx,
						posy, posz);
				...
				if (debug > 1)
					std::cout << "-hit" << iH << "-" << layer << " " << energy
							<< " " << EtotSim[layer];

				//double absweight = myDetector.subDetectorByLayer(layer).absWeight;
				double absweight = (*ssvec)[layer].voldEdx()
						/ (*ssvec)[1].voldEdx();
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {	//loop on layers
				double absweight = (*ssvec)[iL].voldEdx()
						/ (*ssvec)[1].voldEdx();
				...
				if (Emax > maxE5)
					maxE5 = Emax;
				for (int bin(1);
						bin < (hist->GetNbinsX() * hist->GetNbinsY() + 1);
						++bin) {
		...
		for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {//loop on rechits
			...
			if (debug > 1) {
				std::cout << " --  RecoHit " << iH << "/" << (*rechitvec).size()
						<< " --" << std::endl << " --  position x,y "
						<< lHit.get_x() << "," << lHit.get_y() << std::endl;
				lHit.Print(std::cout);
			}
			...
			if (layer
					>= myDetector.subDetectorByEnum(DetectorEnum::FECAL).layerIdMin
					&& layer
							< myDetector.subDetectorByEnum(DetectorEnum::BECAL).layerIdMax) {
				if (energy > maxEee)
					maxEee = energy;
			}
			if (layer
					>= myDetector.subDetectorByEnum(DetectorEnum::FHCAL).layerIdMin
					&& layer
							< myDetector.subDetectorByEnum(DetectorEnum::FHCAL).layerIdMax) {
				if (energy > maxEfh)
					maxEfh = energy;
			}
			if (layer
					>= myDetector.subDetectorByEnum(DetectorEnum::BHCAL1).layerIdMin
					&& layer
							< myDetector.subDetectorByEnum(DetectorEnum::BHCAL1).layerIdMax) {
				if (energy > maxEbh)
					maxEbh = energy;
			}
			if (energy > maxEall) {
		...
		double Eecal = 0;
		if (myDetector.section(DetectorEnum::FECAL) < nSections)
			Eecal += myDigitiser.MIPtoGeV(
					myDetector.subDetectorByEnum(DetectorEnum::FECAL),
					Ereco[myDetector.section(DetectorEnum::FECAL)]);
		if (myDetector.section(DetectorEnum::MECAL) < nSections)
			Eecal += myDigitiser.MIPtoGeV(
					myDetector.subDetectorByEnum(DetectorEnum::MECAL),
					Ereco[myDetector.section(DetectorEnum::MECAL)]);
		if (myDetector.section(DetectorEnum::BECAL) < nSections)
			Eecal += myDigitiser.MIPtoGeV(
					myDetector.subDetectorByEnum(DetectorEnum::BECAL),
					Ereco[myDetector.section(DetectorEnum::BECAL)]);
		/*double Efhcal = 0;
		...
		double Efhcal = 0;
		if (myDetector.section(DetectorEnum::FHCAL) < nSections)
			Efhcal += (Ereco[myDetector.section(DetectorEnum::FHCAL)]
					- FHcalEMOffset) / FHcalEMCalib;
		double Ebhcal = 0;
		if (myDetector.section(DetectorEnum::BHCAL1) < nSections)
			Ebhcal += (Ereco[myDetector.section(DetectorEnum::BHCAL1)]
					- BHcalEMOffset) / BHcalEMCalib;
		if (myDetector.section(DetectorEnum::BHCAL2) < nSections)
			Ebhcal += (Ereco[myDetector.section(DetectorEnum::BHCAL2)]
					- BHcalEMOffset) / BHcalEMCalib;

		double Etotcal = Eecal
				+ (Efhcal + (1. / BHcalSlope * Ebhcal) - HcalPionOffset)
						/ HcalPionCalib;

		bool doFill = true;
		if (selectEarlyDecays && firstInteraction > 5)
			doFill = false;
		...
		double etotmips = 0;
		if (versionNumber == 21)
			etotmips = Esim[0] + Esim[1] / G4BHcalSlope;
		else {
			for (unsigned iD(0); iD < nSections; ++iD) {
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {    //loop on layers
			if (debug)
				std::cout << " -- Layer " << iL << " total sim E = "
						<< EtotSim[iL] << " total rec E = " << EtotRec[iL]
						<< " absweight = " << (*ssvec)[iL].voldEdx() << "/"
						<< (*ssvec)[0].voldEdx() << " = "
						<< (*ssvec)[iL].voldEdx() / (*ssvec)[0].voldEdx()
						<< std::endl << std::endl;
		...
		firstEvent = false;
		   ...
		   //write
	for (unsigned iD(0); iD < nSections; ++iD) {
		std::cout << " -- Summary of sim energies " << myDetector.detName(iD)
				<< std::endl << p_Esim[iD]->GetEntries() << " mean "
				<< p_Esim[iD]->GetMean() << " rms " << p_Esim[iD]->GetRMS()
				<< " underflows " << p_Esim[iD]->GetBinContent(0)
				<< " overflows "
				<< p_Esim[iD]->GetBinContent(p_Esim[iD]->GetNbinsX() + 1)
				<< std::endl;
		std::cout << " -- Summary of reco energies " << myDetector.detName(iD)
				<< std::endl << p_Ereco[iD]->GetEntries() << " mean "
				<< p_Ereco[iD]->GetMean() << " rms " << p_Ereco[iD]->GetRMS()
				<< " underflows " << p_Ereco[iD]->GetBinContent(0)
				<< " overflows "
				<< p_Ereco[iD]->GetBinContent(p_Ereco[iD]->GetNbinsX() + 1)
				<< std::endl;
	}
	std::cout << " -- Total Esim in MIPS: " << p_EsimTotal->GetEntries()
			<< " mean " << p_EsimTotal->GetMean() << " rms "
			<< p_EsimTotal->GetRMS() << " rms/mean "
			<< p_EsimTotal->GetRMS() / p_EsimTotal->GetMean() << " underflows "
			<< p_EsimTotal->GetBinContent(0) << " overflows "
			<< p_EsimTotal->GetBinContent(p_EsimTotal->GetNbinsX() + 1)
			<< std::endl;

	std::cout << " -- Total Ereco in GeV: " << p_ErecoTotal->GetEntries()
			<< " mean " << p_ErecoTotal->GetMean() << " rms "
			<< p_ErecoTotal->GetRMS() << " rms/mean "
			<< p_ErecoTotal->GetRMS() / p_ErecoTotal->GetMean()
			<< " underflows " << p_ErecoTotal->GetBinContent(0) << " overflows "
			<< p_ErecoTotal->GetBinContent(p_ErecoTotal->GetNbinsX() + 1)
			<< std::endl;
