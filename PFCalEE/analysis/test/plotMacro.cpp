...
int main(int argc, char** argv) { //main
	...
	for (unsigned iE(0); iE < nGenEn; ++iE) {
		...
		for (unsigned iS(0); iS < 2; ++iS) {
			...
			TFile *inputFile =
					TFile::Open(
							"/afs/cern.ch/work/a/amagnan/public/HGCalEEGeant4/git_V00-02-05/version_8/model_1/e-/BOFF/e_"
									+ genEnStr + "/PFcal_" + lSuffix[iS]
									+ ".root");
			if (!inputFile) {
				std::cout
						<< " -- Error, input file cannot be opened. Exiting..."
						<< std::endl;
				return 1;
			}
			...
			TH1F *simhitEnergy = new TH1F("simhitEnergy", ";E (MIPs)", 750, 0,
					1500);
			...
			simhitEnergy->Draw();
			lat.DrawLatex(0, yMax * 2,
					"SIM e-, " + genEnStr + " GeV, 2.5#times2.5 mm^{2} cells");

			myc->Update();
			myc->Print(
					"PLOTS/SimHitEnergy_" + genEnStr + "GeV" + lSuffix[iS]
							+ ".png");
			myc->Print(
					"PLOTS/SimHitEnergy_" + genEnStr + "GeV" + lSuffix[iS]
							+ ".pdf");
