...
int main(int argc, char** argv) { //main  
	...
	if (argc < 4) {
		std::cout << " Usage: " << argv[0]
				<< " <nEntries to process (0=all) [1 entry = 1 layer]>"
	...
	if (!inputFile) {
		std::cout << " -- Error, input file " << filePath
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
		...
		if (debug) {
			std::cout << "... Processing layer " << volNb << " with "
					<< (*simhitvec).size() << " simhits " << std::endl;
		}
		...
		for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) { //loop on hits
			...
			if (layer == volNb + 1) {
				if (event == 0)
					std::cout
							<< " -- Warning, applying patch to layer number..."
							<< std::endl;
				layer = volNb;
			}
			...
			if (energy > 0) {
				hitsOut << event << " " << layer << " " << posx << " " << posy
						<< " " << posz << " " << energy << std::endl;
			}
