...
int main(int argc, char** argv) { //main  
	...
	("concept", po::value<bool>(&concept)->default_value(true))("pNevts,n",
			...
			"outFilePath,o",
			po::value < std::string > (&outFilePath)->required())("nSiLayers",
			po::value<unsigned>(&nSiLayers)->default_value(2))("debug,d",
			po::value<unsigned>(&debug)->default_value(0))("start",
			po::value<unsigned>(&start)->default_value(1))("end",
			po::value<unsigned>(&end)->default_value(50))("etamean",
			po::value<double>(&etamean)->default_value(2.85))("deta",
			po::value<double>(&deta)->default_value(0.15));
	...
	if (inFilePath.find(".root") != inFilePath.npos) {
		...
		for (unsigned idx(start); idx < end + 1; ++idx) {
			puInput.str("");
			puInput << inFilePath << "HGcal_version12_model2_BOFF_MinBias_"
					<< idx << ".root";
	...
	label << outFilePath << "/MipStudy_eta" << (etamean - deta) << "_"
			<< (etamean + deta) << "_MinBias" << start << "_" << end << ".root";

	TFile *outputFile = TFile::Open(label.str().c_str(), "RECREATE");

	if (!outputFile) {
		std::cout << " -- Error, output file " << label.str()
	...
	const unsigned nEvts = (
			(pNevts > puTree->GetEntries() || pNevts == 0) ?
					static_cast<unsigned>(puTree->GetEntries()) : pNevts);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iH(0); iH < (*hitvec).size(); ++iH) {  //loop on hits
			...
			if (energy > 0 && passeta
					&& lHit.silayer()
							< geomConv.getNumberOfSiLayers(type, lR)) {
				if (debug > 1)
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {	//loop on layers
			...
			if (isScint) {
				//2.5% per 30-mm edge
				myDigitiser.setIPCrossTalk(
						0.025 * histE->GetXaxis()->GetBinWidth(1) / 30.);
			} else {
			...
			for (int iX(1); iX < histE->GetNbinsX() + 1; ++iX) {
				for (int iY(1); iY < histE->GetNbinsY() + 1; ++iY) {
					...
					double simEcor =
							isTBsetup ?
									xtalkE :
									myDigitiser.mipCor(xtalkE, x, y, meanZpos);
					digiE = simEcor;
