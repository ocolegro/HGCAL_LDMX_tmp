...
bool matchingLayers(const MyHit & before2, const MyHit & before,
		const MyHit & central, const MyHit & after, const MyHit & after2) {
	...
	if (fabs(before.x - central.x) < precision
			...
			&& fabs(after2.y - central.y) < precision)
		return true;
...
int main(int argc, char** argv) {  //main  
	...
	config.add_options()("pNevts,n",
			po::value<unsigned>(&pNevts)->default_value(0))("inFilePath,i",
			po::value < std::string > (&inFilePath)->required())(
			"outFilePath,o",
			po::value < std::string > (&outFilePath)->required())("Ethresh,e",
			po::value<double>(&Ethresh)->default_value(0.9))("EthreshMax,m",
			po::value<double>(&EthreshMax)->default_value(5.0))("EmaxCut",
			po::value<double>(&EmaxCut)->default_value(10000.))("oneOnly",
			po::value<bool>(&oneOnly)->default_value(true))("debug,d",
			po::value<unsigned>(&debug)->default_value(0))("layerRange,l",
			po::value<unsigned>(&layerRange)->default_value(1))("start",
			po::value<unsigned>(&start)->default_value(0))("etamean",
			po::value<double>(&etamean)->default_value(2.85))
	...
	for (unsigned i(0); i < nTrees; ++i) {
		if (nTrees > 1) {
			input.str("");
			input << inputbase.str() << start + i << ".root";
		}
	...
	if (!outputFile) {
		std::cout << " -- Error, output file " << outPath.str()
	...
	for (unsigned ie(0); ie < nEta; ++ie) {
		for (unsigned in(0); in < nNoise; ++in) {
			...
			label.str("");
			label << "EmaxNeighbour_eta" << eta[ie] * 10 << "_noise"
					<< noise[in] * 100;
			p_EmaxNeighbour[ie][in] = new TH1F(label.str().c_str(),
					";E_{max}^{neighbour} (Mips);hits", 100, 0, 2);
			for (unsigned il(0); il < nLayers; ++il) {
				label.str("");
				label << "hitSpectrum_eta" << eta[ie] * 10 << "_noise"
						<< noise[in] * 100 << "_layer" << il;
				hitSpectrum[ie][in][il] = new TH1F(label.str().c_str(),
						";E_{0.5#times0.5 cm^{2}} (mips);N_{cells}", 100, 0, 5);
				label.str("");
				label << "hitSpectrumSig_eta" << eta[ie] * 10 << "_noise"
						<< noise[in] * 100 << "_layer" << il;
				hitSpectrumSig[ie][in][il] = new TH1F(label.str().c_str(),
						";E_{0.5#times0.5 cm^{2}} (mips);N_{cells}", 100, 0, 5);
				label.str("");
				label << "Nhits_eta" << eta[ie] * 10 << "_noise"
						<< noise[in] * 100 << "_layer" << il;
				p_nHits[ie][in][il] = new TH1F(label.str().c_str(),
						";N_{0.5#times0.5 cm^{2}};N_{events}", 500, 0, 5000);
			}
		}
	}
	...
	if (pNevts == 0)
		pNevts = tree->GetEntries();
	if (pNevts > tree->GetEntries())
		pNevts = tree->GetEntries();

	std::cout << " -- Tree contains: " << tree->GetEntries() << " events."
			<< std::endl;
	...
	for (unsigned ievt(0); ievt < pNevts; ++ievt) {  //loop on entries
		...
		for (unsigned ie(0); ie < nEta; ++ie) {
			if (debug)
				std::cout << " - Eta = " << eta[ie] << std::endl;
			for (unsigned in(0); in < nNoise; ++in) {
				if (debug)
					std::cout << " --- Noise = " << noise[in] << std::endl;
				for (unsigned iL(0); iL < nLayers; ++iL) {
					nHits[ie][in][iL] = 0;
				}

				fillKDTree(_hit_kdtree, _hit_nodes, lHitVec[ie][in]);

				if (debug)
					std::cout << " ----- Number of hits in vector : "
							<< lHitVec[ie][in].size() << std::endl;
				...
				for (unsigned iH(0); iH < lHitVec[ie][in].size(); ++iH) {
					...
					findNeighbours(cell_size, _hit_kdtree, current, neighbours,
							layerRange + 1);

					if (debug > 1)
						std::cout << " -- Number of closest neighbours found: "
					...
					for (const KDNode& nbourpoint : neighbours) {
						...
						if (nbour.layer != iL
								&& (nbour.e <= Ethresh || nbour.e >= EthreshMax)) {
							//neighbours.erase(neighbours.begin()+iH);
							continue;
						}
						if (nbour.layer == iL
								&& (nbour.x == current.x || nbour.y == current.y)) {
							if (nbour.e > EmaxSC)
								EmaxSC = nbour.e;
							//if (nbour.e>=EmaxCut) break;
						} else if ((iL > 0 && nbour.layer == iL - 1)
								|| (iL == 0 && nbour.layer == iL + 2))
							idxBefore.push_back(index);
						else if ((iL < nLayers - 1 && nbour.layer == iL + 1)
								|| (iL == nLayers - 1 && nbour.layer == iL - 2))
							idxAfter.push_back(index);
								...
								&& ((iL > 1 && iL < nLayers - 1
										...
										|| (iL == 0 && nbour.layer == iL + 3)))
							idx2Before.push_back(index);
						else if (layerRange == 2
								&& ((iL > 1 && iL < nLayers - 2
										&& nbour.layer == iL + 2)
										...
										|| (iL == nLayers - 1
												&& nbour.layer == iL - 4)))
							idx2After.push_back(index);
					}

					const MyHit & ibef =
							idxBefore.size() > 0 ?
									lHitVec[ie][in][idxBefore[0]] : current;
					const MyHit & iaft =
							idxAfter.size() > 0 ?
									lHitVec[ie][in][idxAfter[0]] : current;
					const MyHit & i2bef =
							idx2Before.size() > 0 ?
									lHitVec[ie][in][idx2Before[0]] : current;
					const MyHit & i2aft =
							idx2After.size() > 0 ?
									lHitVec[ie][in][idx2After[0]] : current;
					if (!oneOnly)
						passTrack = idxBefore.size() > 0 && idxAfter.size() > 0
								&& ((layerRange == 2 && idx2Before.size() > 0
										&& idx2After.size() > 0)
										|| layerRange == 1);
					else
						passTrack = idxBefore.size() == 1
								&& idxAfter.size() == 1
								&& ((layerRange == 2 && idx2Before.size() == 1
										&& idx2After.size() == 1)
										|| layerRange == 1)
								&& matchingLayers(i2bef, ibef, current, iaft,
										i2aft);
					if (!passTrack)
						continue;
					...
					if (current.hasSignal)
						hitSpectrumSig[ie][in][iL]->Fill(current.e);
					nHits[ie][in][iL]++;
