...
double showerEnergy(const double Cglobal, const double Etotcal,
...
unsigned encodeChannelId(const unsigned iL, const unsigned rebin,
	...
	return (rebin & 0x000F) | ((binX & 0x00FF) << 4) | ((binY & 0x00FF) << 12)
			| ((iL & 0x0FFF) << 20);

}

void fillHitHistos(TH1F *p_simhitEnergy, TH1F *p_simnoisehitEnergy,
		TH1F *p_xtalknoisehitEnergy, TH1F *p_rechitEnergy,
		TH2F *p_outvsinEnergy, const std::vector<double> & simE,
		const double & MeVToMip, const double & absweight, const unsigned layer,
		HGCSSDigitisation & myDigitiser, TH1F *p_noise, const unsigned rebin,
		std::vector<double> & energies, std::vector<double> & hitEnergies) {
...
void fillBHHistos(TH2D *histE, const unsigned iL, bool firstEvent,
		TRandom3 & lRndm, std::map<unsigned, bool> & channelAlive,
		TH1F *p_simhitEnergy, TH1F *p_simnoisehitEnergy,
		TH1F *p_xtalknoisehitEnergy, TH1F *p_rechitEnergy,
		TH2F *p_outvsinEnergy, const double & MeVtoMip,
		const double & absweight, HGCSSDigitisation & myDigitiser,
		TH1F *p_noise, std::vector<double> & Etotcal,
		std::vector<double> & hitEnergies, double minx, double maxx,
	...
	for (int iX(1); iX < histE->GetNbinsX() + 1; ++iX) {
		for (int iY(1); iY < histE->GetNbinsY() + 1; ++iY) {
			...
			if (iX > 1)
				simEvec.push_back(histE->GetBinContent(iX - 1, iY));
			if (iX < histE->GetNbinsX())
				simEvec.push_back(histE->GetBinContent(iX + 1, iY));
			if (iY > 1)
				simEvec.push_back(histE->GetBinContent(iX, iY - 1));
			if (iY < histE->GetNbinsY())
				simEvec.push_back(histE->GetBinContent(iX, iY + 1));
			...
			if (posx > minx && posx < maxx && posy > miny && posy < maxy) {
				...
				if (firstEvent) {
					...
					if (keep < 0.02) {
						...
						std::cout << " Channel " << iL << " " << posx << " "
								<< posy << " set to dead." << std::endl;
					} else
						channelAlive[channelId] = true;
				}
				if (!channelAlive[channelId])
					continue;

				fillHitHistos(p_simhitEnergy, p_simnoisehitEnergy,
						p_xtalknoisehitEnergy, p_rechitEnergy, p_outvsinEnergy,
						simEvec, MeVtoMip, absweight, iL, myDigitiser, p_noise,
						4, Etotcal, hitEnergies);
			}
...
int main(int argc, char** argv) { //main  
	...
	if (argc < 7) {
		std::cout << " Usage: " << argv[0] << " <nEvts to process (0=all)>"
				<< " <path to input files>" << " <name of input sim file>"
				<< " <name of input reco file>" << " <full path to output file>"
	...
	if (!simFile) {
		std::cout << " -- Error, input file " << input.str()
				<< " cannot be opened. Exiting..." << std::endl;
		return 1;
	} else
		std::cout << " -- input file " << simFile->GetName()
	...
	if (!recFile) {
		...
		std::cout << " -- input file " << recFile->GetName()
				<< " successfully opened." << std::endl;
	...
	TH2D *bottom = new TH2D("bottom", ";x;y", 7, -450, 390, 7, -450, 390);
	TH2D *right = new TH2D("right", ";x;y", 7, -390, 450, 7, -450, 390);
	TH2D *top = new TH2D("top", ";x;y", 7, -390, 450, 7, -390, 450);
	TH2D *left = new TH2D("left", ";x;y", 7, -450, 390, 7, -390, 450);
	...
	TH2F *p_BHCALvsFHCAL = new TH2F("p_BHCALvsFHCAL",
			";FHCAL (GeV);BHCAL (GeV)", 500, 0, 500, 500, 0, 500);
	...
	TH1F *p_EsimTotal = new TH1F("p_EsimTotal", ";Esim (MIPs)", 15000, 0,
			300000);
	TH1F *p_ErecoTotal = new TH1F("p_ErecoTotal", ";Ereco (GeV)", 20000, 0,
			5000);
	...
	TH1F *p_nHitsFHCAL = new TH1F("p_nHitsFHCAL", "n_{Hits} (FHCAL)", 1000, 0,
			1000);
	p_nHitsFHCAL->StatOverflows();
	TH2F *p_HCALvsCglobal = new TH2F("p_HCALvsCglobal",
			";Cglobal (e_{lim}=5 MIP);E_{HCAL} (GeV)", 200, 0., 2.0, 100, 0,
			100);

	TH1F *p_simhitEnergy = new TH1F("p_simhitEnergy", ";E_{hit} (MIPs)", 200, 0,
			10);
	TH1F *p_simnoisehitEnergy = new TH1F("p_simnoisehitEnergy",
			";E_{hit} (MIPs)", 200, 0, 10);
	TH1F *p_xtalknoisehitEnergy = new TH1F("p_xtalknoisehitEnergy",
			";E_{hit} (MIPs)", 200, 0, 10);
	TH1F *p_rechitEnergy = new TH1F("p_rechitEnergy", ";E_{hit} (MIPs)", 500, 0,
			20);
	p_rechitEnergy->StatOverflows();
	TH2F *p_outvsinEnergy = new TH2F("p_outvsinEnergy",
			";E_{hit}^{sim} (MIPs);E_{hit}^{rec} (MIPs)", 500, 0, 500, 500, 0,
			500);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) {    //loop on hits
			...
			if (layer != prevLayer) {
				const HGCSSSubDetector & subdet = myDetector.subDetectorByLayer(
						layer);
				type = subdet.type;
			...
			if (energy > 0 && isCaliceHcal && plotHitSpectra) {
				geomConv.fill(type, subdetLayer, lHit.energy(), lRealTime, posx,
						posy, posz);
			}
			if (debug > 1) {
				std::cout << " --  SimHit " << iH << "/" << (*simhitvec).size()
		...
		for (unsigned iE(0); iE < energies.size(); ++iE)
			energies[iE] = 0;
		...
		if (isCaliceHcal && plotHitSpectra) {
			...
			for (unsigned iL(0); iL < nLayers; ++iL) {	//loop on layers
				...
				if (debug > 1)
					std::cout << iL << " " << (*ssvec)[iL].volX0trans() << " / "
							<< (*ssvec)[0].volX0trans() << " = " << absweight
							<< std::endl;
				//3*3 cells
				if (!doCoarse) {
					...
					for (int iX(1); iX < histE->GetNbinsX() + 1; ++iX) {
						for (int iY(1); iY < histE->GetNbinsY() + 1; ++iY) {
							...
							if (iX > 1)
								simEvec.push_back(
										histE->GetBinContent(iX - 1, iY));
							if (iX < histE->GetNbinsX())
								simEvec.push_back(
										histE->GetBinContent(iX + 1, iY));
							if (iY > 1)
								simEvec.push_back(
										histE->GetBinContent(iX, iY - 1));
							if (iY < histE->GetNbinsY())
								simEvec.push_back(
										histE->GetBinContent(iX, iY + 1));
							...
							if (fabs(posx) < 150 && fabs(posy) < 150) {
								...
								unsigned channelId = encodeChannelId(iL, 1, iX,
										iY);
								//discard 2% randomly
								if (firstEvent) {
									double keep = lRndm.Rndm();
								...
								fillHitHistos(p_simhitEnergy,
										p_simnoisehitEnergy,
										p_xtalknoisehitEnergy, p_rechitEnergy,
										p_outvsinEnergy, simEvec, MeVtoMip,
										absweight, iL, myDigitiser, p_noise, 1,
										energies, hitEnergies);
							}
						}
					}
				}
					...
					for (int iY(1); iY < histE->GetNbinsY() + 1; ++iY) {
						...
						if ((doCoarse && fabs(posx) < 330 && fabs(posy) < 330)
							...
							fillHitHistos(p_simhitEnergy, p_simnoisehitEnergy,
									p_xtalknoisehitEnergy, p_rechitEnergy,
									p_outvsinEnergy, simEvec, MeVtoMip,
									absweight, iL, myDigitiser, p_noise, 2,
									energies, hitEnergies);
						}
					}
				}
				...
				fillBHHistos(bottom, iL, firstEvent, lRndm, channelAlive,
						p_simhitEnergy, p_simnoisehitEnergy,
						p_xtalknoisehitEnergy, p_rechitEnergy, p_outvsinEnergy,
						MeVtoMip, absweight, myDigitiser, p_noise, energies,
						hitEnergies, -330, 270, -450, -330);
				//for right column
				fillBHHistos(right, iL, firstEvent, lRndm, channelAlive,
						p_simhitEnergy, p_simnoisehitEnergy,
						p_xtalknoisehitEnergy, p_rechitEnergy, p_outvsinEnergy,
						MeVtoMip, absweight, myDigitiser, p_noise, energies,
						hitEnergies, 330, 450, -330, 270);
				//for top row
				fillBHHistos(top, iL, firstEvent, lRndm, channelAlive,
						p_simhitEnergy, p_simnoisehitEnergy,
						p_xtalknoisehitEnergy, p_rechitEnergy, p_outvsinEnergy,
						MeVtoMip, absweight, myDigitiser, p_noise, energies,
						hitEnergies, -270, 330, 330, 450);
				//for left column
				fillBHHistos(left, iL, firstEvent, lRndm, channelAlive,
						p_simhitEnergy, p_simnoisehitEnergy,
						p_xtalknoisehitEnergy, p_rechitEnergy, p_outvsinEnergy,
						MeVtoMip, absweight, myDigitiser, p_noise, energies,
						hitEnergies, -450, -330, -270, 330);
		...
		if (myDetector.section(DetectorEnum::FHCAL) < nSections)
			Efhcal += myDigitiser.MIPtoGeV(
					myDetector.subDetectorByEnum(DetectorEnum::FHCAL),
					Ereco[myDetector.section(DetectorEnum::FHCAL)]);
		double Ebhcal = 0;
		if (myDetector.section(DetectorEnum::BHCAL1) < nSections)
			Ebhcal += myDigitiser.MIPtoGeV(
					myDetector.subDetectorByEnum(DetectorEnum::BHCAL1),
					Ereco[myDetector.section(DetectorEnum::BHCAL1)]);
		if (myDetector.section(DetectorEnum::BHCAL2) < nSections)
			Ebhcal += myDigitiser.MIPtoGeV(
					myDetector.subDetectorByEnum(DetectorEnum::BHCAL2),
					Ereco[myDetector.section(DetectorEnum::BHCAL2)]);
		...
		if (doFill) {
			...
			for (unsigned iLim(0); iLim < nLimits; ++iLim) {
				...
				for (unsigned iH(0); iH < nHits; ++iH) {
					...
					if (hitEnergies[iH] <= pElim[iLim])
						nHitsCountNum[iLim]++;
					if (iLim == 0 && hitEnergies[iH] <= EmipMean)
						nHitsCountDen++;
				}
				double Cglobal = 0;
				if (nHitsCountDen > 0)
					Cglobal = nHitsCountNum[iLim] * 1.0 / nHitsCountDen;
		...
		if (firstEvent) {
			std::cout << " Check of channelAlive size: " << channelAlive.size()
					<< "/" << static_cast<unsigned>(216 * 30 + 141 * 24)
					<< std::endl;
		}
		firstEvent = false;
	}	  //loop on entries
