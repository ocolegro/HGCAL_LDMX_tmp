...
int main(int argc, char** argv) { //main
	...
	TFile *inputFile =
			TFile::Open(
					"root://eoscms//eos/cms/store/user/amagnan/HGCalHEGeant4/run_0/mu-/HGcal_"
							+ lSuffix + ".root");
	if (!inputFile) {
	...
	if (!lTree) {
		std::cout << " -- Error, tree cannot be opened. Exiting..."
				<< std::endl;
		return 1;
	}
	...
	TH2F *p_nHits = new TH2F("nHits", "; layer; Number of hits; Events",
			nLayers, 0, nLayers, 20, 0, 20);
	TH1F *p_hitEnergy_si = new TH1F("hitEnergy_si", ";E (MeV);SimHits", 250, 0,
			1);
	TH1F *p_hitEnergySel_si = new TH1F("hitEnergySel_si", ";E (MeV);SimHits",
			250, 0, 1);
	TH1F *p_hitEnergy_scint = new TH1F("hitEnergy_scint", ";E (MeV);SimHits",
			500, 0, 100);
	TH1F *p_hitEnergySel_scint = new TH1F("hitEnergySel_scint",
			";E (MeV);SimHits", 1000, 0, 10);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
		...
		if (ievt % (nLayers * 1000) == 0)
			std::cout << "entry " << ievt << " volNb = " << volNb << std::endl;
		...
		for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) { //loop on hits
			...
			if (energy > 0) {
				if (volNb < nHcalSiLayers)
					p_hitEnergy_si->Fill(energy);
				else
					p_hitEnergy_scint->Fill(energy);
				nHits++;
			}
		} //loop on hits
			...
			for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) { //loop on hits
				...
				if (energy > 0) {
					if (volNb < nHcalSiLayers)
						...
						p_hitEnergySel_scint->Fill(energy);
				}
			}
		}
