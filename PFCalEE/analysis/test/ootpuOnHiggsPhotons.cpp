...
double getCalibratedE(const std::vector<double> & Evec,
		const std::vector<unsigned> & region, const double eta) {
...
void getMaximumCellFromGeom(const double & phimax, const double & etamax,
	...
	for (unsigned iL(0); iL < xmax.size(); ++iL) {
		...
		if (xmax[iL] > 0)
			xmax[iL] = static_cast<int>((xmax[iL] + 4.999999) / 10.) * 10;
		else
			xmax[iL] = static_cast<int>((xmax[iL] - 4.999999) / 10.) * 10;
		if (ymax[iL] > 0)
			ymax[iL] = static_cast<int>((ymax[iL] + 4.999999) / 10.) * 10;
		else
			ymax[iL] = static_cast<int>((ymax[iL] - 4.999999) / 10.) * 10;
...
void fillKDTree(KDTree & _hit_kdtree, std::vector<KDNode> & _hit_nodes,
	...
	KDTreeCube kd_boundingregion = fill_and_bound_kd_tree(rechits,
			usable_rechits, _hit_nodes);
	_hit_kdtree.build(_hit_nodes, kd_boundingregion);
	_hit_nodes.clear();
}
;

void findNeighbours(const double & cell_size_mm, KDTree & _hit_kdtree,
		const MyHit & current_cell, std::vector<KDNode> & found,
	...
	auto z_rh = minmax(zPos(layer - layerRange) / 10.,
			zPos(layer + layerRange) / 10.);  //in cm!!!

	KDTreeCube hit_searchcube((float) x_rh.first, (float) x_rh.second,
			(float) y_rh.first, (float) y_rh.second, (float) z_rh.first,
			(float) z_rh.second);
...
int main(int argc, char** argv) {  //main  
	...
	("concept", po::value<bool>(&concept)->default_value(true))("pNevts,n",
			po::value<unsigned>(&pNevts)->default_value(0))("inFilePath,i",
			po::value < std::string > (&inFilePath)->required())(
			"outFilePath,o",
			po::value < std::string > (&outFilePath)->required())("debug,d",
			po::value<unsigned>(&debug)->default_value(0))("nPu,p",
			po::value<unsigned>(&nPu)->default_value(140))("run,r",
			po::value<unsigned>(&run)->default_value(0));
	...
	std::ostringstream input;
	input
			<< "/afs/cern.ch/work/a/amagnan/PFCalEEAna/gitV00-02-14/version12/Hgg/pu0/run"
			<< run << ".root";
	if (doHgg)
		inputSig = TFile::Open(input.str().c_str());
	else
		inputSig =
				TFile::Open(
						"PLOTS/gitV00-02-12/version12/gamma/200um/eta19_et60_pu0_new_logw3.root");
	if (!inputSig) {
	...
	double Etruth1 = 0;
	if (doHgg)
		sigTree1->SetBranchAddress("truthE", &Etruth1);
	else
		Etruth1 = 60. * cosh(1.9);
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		...
		for (unsigned iy(0); iy < 3; ++iy) {
			for (unsigned ix(0); ix < 3; ++ix) {
				...
				label.str("");
				label << "E_" << iL << "_" << idx;
				sigTree1->SetBranchAddress(label.str().c_str(),
						&Exyg1[iL][idx]);
			}
		}
	}
	...
	TTree *sigTree2 = 0;
	if (doHgg)
		sigTree2 = (TTree*) gDirectory->Get("EcellsSR2");

	double Etruth2 = 0;
	if (doHgg)
		sigTree2->SetBranchAddress("truthE", &Etruth2);
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			...
			for (unsigned iy(0); iy < 3; ++iy) {
				for (unsigned ix(0); ix < 3; ++ix) {
					...
					label.str("");
					label << "E_" << iL << "_" << idx;
					sigTree2->SetBranchAddress(label.str().c_str(),
							&Exyg2[iL][idx]);
				}
			}
		}
	...
	if (!outputFile) {
		std::cout << " -- Error, output file " << output.str().c_str()
	...
	TH1F *E_noOOTPU = new TH1F("E_noOOT", ";E_{#gamma}-E_{truth} (GeV);showers",
			1000, -100, 100);
	TH1F *E_mips = new TH1F("E_mips", ";E_{#gamma} (mips);showers", 10000, 0,
			100000);
	TH1F *E_OOTPU = new TH1F("E_OOT", ";E_{#gamma}-E_{truth} (GeV);showers",
			1000, -100, 100);
	TH1F *E_OOTpastPU = new TH1F("E_OOTpast",
			";E_{#gamma}-E_{truth} (GeV);showers", 1000, -100, 100);
	TH2F *EcalibvsTruth = new TH2F("EcalibvsTruth",
			";E_{truth} (GeV);E_{#gamma} (GeV);showers", 5000, 0, 5000, 5000, 0,
			5000);
	TH2F *ErawvsTruth = new TH2F("ErawvsTruth",
			";E_{truth} (GeV);E_{#gamma} (Mips);showers", 1000, 0, 1000, 10000,
			0, 50000);
	TH2F *ErawvsTruth1 = new TH2F("ErawvsTruth1",
			";E_{truth} (GeV);E_{#gamma} (Mips);showers", 1000, 0, 1000, 10000,
			0, 50000);
	TH2F *ErawvsTruth2 = new TH2F("ErawvsTruth2",
			";E_{truth} (GeV);E_{#gamma} (Mips);showers", 1000, 0, 1000, 10000,
			0, 50000);
	TH2F *ErawvsTruth3 = new TH2F("ErawvsTruth3",
			";E_{truth} (GeV);E_{#gamma} (Mips);showers", 1000, 0, 1000, 10000,
			0, 50000);
	TH2F *EratiovsEta = new TH2F("EratiovsEta",
			";#eta;E_{#gamma}/E_{truth};showers", 100, 1.5, 3.0, 400, 0, 1.5);

	TH1F *ErecooverEtrue = new TH1F("ErecooverEtrue",
			";E_{#gamma}/E_{truth};showers", 400, 0, 1.5);
	TH1F *ErecooverEtrueOOT = new TH1F("ErecooverEtrueOOT",
			";E_{#gamma}/E_{truth};showers", 400, 0, 1.5);
	TH1F *ErecooverEtrueOOTpast = new TH1F("ErecooverEtrueOOTpast",
			";E_{#gamma}/E_{truth};showers", 400, 0, 1.5);

	TH1F *EpuoverErecoFutur = new TH1F("EpuoverErecoFutur",
			";E_{PU}/E_{#gamma};showers", 1000, 0, 0.1);
	TH1F *EpuoverErecoPast = new TH1F("EpuoverErecoPast",
			";E_{PU}/E_{#gamma};showers", 1000, 0, 0.1);
	TH1F *EpuoverErecoAll = new TH1F("EpuoverErecoAll",
			";E_{PU}/E_{#gamma};showers", 1000, 0, 0.1);

	TH1F *EpuFutur = new TH1F("EpuFutur", ";E_{PU} (mips);showers", 1000, 0,
			500);
	...
	TH2F *nAbovevsEta = new TH2F("nAbovevsEta",
			";#eta;n_{cells}(E>100 fC);showers", 14, 1.5, 2.9, 200, 0, 200);

	double bxthresh[nbx] = { 250, 250, 1000, 1930, 2910, 3870, 4870, 5880, 6930,
			7990, 9080 };
	...
	for (unsigned ibx(0); ibx < nbx; ++ibx) {
		...
		label.str("");
		label << "Epu_bx_" << ibx;
		Epu_perbx[ibx] = new TH1F(label.str().c_str(), ";E_{PU} (mips);showers",
				500, 0, 500);
		label.str("");
		label << "nAbove_bx_" << ibx;
		nAbove_perbx[ibx] = new TH1F(label.str().c_str(),
				";n(E>thresh);showers", 271, 0, 271);
	}

	const unsigned nEvts = (
			(pNevts > sigTree1->GetEntries() || pNevts == 0) ?
					static_cast<unsigned>(sigTree1->GetEntries()) : pNevts);

	std::vector<HGCSSRecoHit> * rechitvec = 0;
	unsigned nPuVtx = 0;
	lTree->SetBranchAddress("HGCSSRecoHitVec", &rechitvec);
	if (lTree->GetBranch("nPuVtx"))
		lTree->SetBranchAddress("nPuVtx", &nPuVtx);

	std::vector<double> posz;
	posz.resize(nLayers, 0);
	if (!getZpositions(versionNumber, posz))
		return 1;

	//loop on events
	const unsigned nPuEvts = lTree->GetEntries();
	std::cout << "- Number of pu events available: " << lTree->GetEntries()
			<< std::endl;
	std::cout << "- Number of signal events available: "
			<< sigTree1->GetEntries();
	if (doHgg)
		std::cout << " " << sigTree2->GetEntries();
	std::cout << std::endl;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		if (debug)
			std::cout << " Truth photon: x=" << truthPosXg1[0] << " y="
					<< truthPosYg1[0] << " E=" << Etruth1 << " eta=" << eta1
					<< " phi=" << phi1 << std::endl;
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {
			...
			for (unsigned iy(0); iy < 3; ++iy) {
				for (unsigned ix(0); ix < 3; ++ix) {
					...
					if (Exyg1[iL][idx] > (100. / mipE * f1))
						nover100_1++;
					if (Exyg2[iL][idx] > (100. / mipE * f2))
						nover100_2++;
		...
		for (unsigned ibx(1); ibx < nbx; ++ibx) {      //loop on bx
			if (fid1)
				nAbove_perbx[ibx]->Fill(nCellsAbove1[ibx]);
			if (fid2)
				nAbove_perbx[ibx]->Fill(nCellsAbove2[ibx]);
		}
		...
		for (unsigned ibx(0); ibx < nbx; ++ibx) {      //loop on bx
			...
			if (ibx > 0 && nCellsAbove1[ibx] == 0 && nCellsAbove2[ibx] == 0)
				continue;
			...
			while (1) {
				...
				if (lidxSet.find(ipu) == lidxSet.end()) {
					...
					std::cout << " -- Found duplicate ! Taking another shot."
							<< std::endl;
				}
			}
			lTree->GetEntry(ipu);
			...
			for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) { //loop on hits
				...
				if (fabs(posx - xmax1[layer]) < 15.1
					...
					double r1 = sqrt(
							pow(xmax1[layer], 2) + pow(ymax1[layer], 2));
					double f1 = getFactor(r1);
					if ((ibx == 0 && energy > (bxthresh[0] * f1))
							|| (ibx > 0
									&& xyid1[ibx][layer].find(idx)
											!= xyid1[ibx][layer].end())) {
				...
				if (doHgg && fabs(posx - xmax2[layer]) < 15.1
					...
					unsigned idx = 3 * (iy + 1) + (ix + 1);
					double r2 = sqrt(
							pow(xmax2[layer], 2) + pow(ymax2[layer], 2));
					double f2 = getFactor(r2);

					if ((ibx == 0 && energy > (bxthresh[0] * f2))
							|| (ibx > 0
									&& xyid2[ibx][layer].find(idx)
											!= xyid2[ibx][layer].end())) {
			...
			if (ibx == 0) {
				if (fid1) {
					E_OOTpastPU->Fill(
							getCalibratedE(EperLayer1OOT, regiong1, eta1)
									- Etruth1);
					ErecooverEtrueOOTpast->Fill(
							getCalibratedE(EperLayer1OOT, regiong1, eta1)
									/ Etruth1);
				...
				if (doHgg && fid2) {
					E_OOTpastPU->Fill(
							getCalibratedE(EperLayer2OOT, regiong2, eta2)
									- Etruth2);
					ErecooverEtrueOOTpast->Fill(
							getCalibratedE(EperLayer2OOT, regiong2, eta2)
									/ Etruth2);
		...
		if (fid1) {
			...
			EpuAll->Fill(EtotPu1);
			E_noOOTPU->Fill(
					getCalibratedE(EperLayer1, regiong1, eta1) - Etruth1);
			E_mips->Fill(getEtotal(EperLayer1, eta1));
			EcalibvsTruth->Fill(Etruth1,
					getCalibratedE(EperLayer1, regiong1, eta1));
			ErawvsTruth->Fill(Etruth1, getEtotal(EperLayer1, eta1));
			if (region1[0] && !region1[1] && !region1[2])
				ErawvsTruth1->Fill(Etruth1, getEtotal(EperLayer1, eta1));
			if (!region1[0] && region1[1] && !region1[2])
				ErawvsTruth2->Fill(Etruth1, getEtotal(EperLayer1, eta1));
			if (!region1[0] && !region1[1] && region1[2])
				ErawvsTruth3->Fill(Etruth1, getEtotal(EperLayer1, eta1));
			E_OOTPU->Fill(
					getCalibratedE(EperLayer1OOT, regiong1, eta1) - Etruth1);
			ErecooverEtrue->Fill(
					getCalibratedE(EperLayer1, regiong1, eta1) / Etruth1);
			EratiovsEta->Fill(eta1,
					getCalibratedE(EperLayer1, regiong1, eta1) / Etruth1);
			ErecooverEtrueOOT->Fill(
					getCalibratedE(EperLayer1OOT, regiong1, eta1) / Etruth1);
		}
		if (doHgg && fid2) {
			...
			EpuAll->Fill(EtotPu2);
			E_noOOTPU->Fill(
					getCalibratedE(EperLayer2, regiong2, eta2) - Etruth2);
			EcalibvsTruth->Fill(Etruth2,
					getCalibratedE(EperLayer2, regiong2, eta2));
			ErawvsTruth->Fill(Etruth2, getEtotal(EperLayer2, eta2));
			if (region2[0] && !region2[1] && !region2[2])
				ErawvsTruth1->Fill(Etruth2, getEtotal(EperLayer2, eta2));
			if (!region2[0] && region2[1] && !region2[2])
				ErawvsTruth2->Fill(Etruth2, getEtotal(EperLayer2, eta2));
			if (!region2[0] && !region2[1] && region2[2])
				ErawvsTruth3->Fill(Etruth2, getEtotal(EperLayer2, eta2));
			EratiovsEta->Fill(eta2,
					getCalibratedE(EperLayer2, regiong2, eta2) / Etruth2);
			E_OOTPU->Fill(
					getCalibratedE(EperLayer2OOT, regiong2, eta2) - Etruth2);
			ErecooverEtrue->Fill(
					getCalibratedE(EperLayer2, regiong2, eta2) / Etruth2);
			ErecooverEtrueOOT->Fill(
					getCalibratedE(EperLayer2OOT, regiong2, eta2) / Etruth2);
