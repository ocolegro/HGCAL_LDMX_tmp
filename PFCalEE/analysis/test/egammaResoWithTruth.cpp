...
int main(int argc, char** argv) { //main  
	...
	if (nRuns == 0) {
		...
		for (unsigned i(0); i < nRuns; ++i) {
			...
			if (!testInputFile(lstrrec.str(), recFile))
				continue;
	...
	if (calorSizeXY < 1 || calorSizeXY > 6000)
		calorSizeXY = 495;
	...
	if (doHexa)
		...
		geomConv.initialiseSquareMap(calorSizeXY, cellSize);
	...
	SignalRegion SignalEnergy(outFolder, nLayers, nEvts, geomConv, puDensity,
			applyPuMixFix, versionNumber, doHexa);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {    //loop on entries
		...
		SignalEnergy.fillEnergies(ievt, (*event), (*genvec), (*ssvec),
				(*simhitvec), (*rechitvec), nPuVtx);
