...
int main(int argc, char** argv) { //main  
	...
	const double pElim[nLimits] = { 2.5, 5, 7.5, 10, 15, 20, 25, 30, 35, 40, 45,
			50, 60, 70, 80 };
	...
	unsigned genEn = 0;
	std::cout << inFilePath << " " << begin << " " << end << " "
			<< inFilePath.substr(begin, end - begin) << std::endl;
	...
	TH1F *p_hitSpectrum_lowTail = new TH1F("p_hitSpectrum_lowTail",
			";E^{hit}_{HCAL} (MIPs)", 1000, 0, 500);
	p_hitSpectrum_lowTail->StatOverflows();
	p_hitSpectrum_lowTail->Sumw2();
	TH1F *p_hitSpectrum_highTail = new TH1F("p_hitSpectrum_highTail",
			";E^{hit}_{HCAL} (MIPs)", 1000, 0, 500);
	p_hitSpectrum_highTail->StatOverflows();
	p_hitSpectrum_highTail->Sumw2();
	TH1F *p_meanHitSpectrum_lowTail = new TH1F("p_meanHitSpectrum_lowTail",
			";<E^{hit}_{HCAL}> (MIPs)", 1000, 0, 200);
	p_meanHitSpectrum_lowTail->StatOverflows();
	p_meanHitSpectrum_lowTail->Sumw2();
	TH1F *p_meanHitSpectrum_highTail = new TH1F("p_meanHitSpectrum_highTail",
			";<E^{hit}_{HCAL}> (MIPs)", 1000, 0, 200);
	...
	for (unsigned iLim(0); iLim < nLimits; ++iLim) {
		...
		p_Cglobal[iLim] = new TH1F(lName.str().c_str(), ";C_{global}", 1000, 0,
				2);

		lName.str("");
		lName << "p_Eshower_" << iLim;
		p_Eshower[iLim] = new TH1F(lName.str().c_str(), ";E_{shower} (GeV)",
				2000, 0, 1000);

		lName.str("");
		lName << "p_EvsCglobal_" << iLim;
		p_EvsCglobal[iLim] = new TH2F(lName.str().c_str(),
				";C_{global};Etot (GeV)", 200, 0, 2, 700, 0, 700);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		bool doFill = true;
		if (selectEarlyDecay && firstInteraction > 5)
			doFill = false;
		...
		if (doFill) {
			...
			bool lowTail = Etotcal
					< (genEn - getResolution(genEn, versionNumber));
			bool highTail = Etotcal
					> (genEn + getResolution(genEn, versionNumber));
			//fill Cglobal histos
			if (lowTail)
				p_meanHitSpectrum_lowTail->Fill(EmipMean);
			if (highTail)
				p_meanHitSpectrum_highTail->Fill(EmipMean);
			...
			for (unsigned iLim(0); iLim < nLimits; ++iLim) {
				...
				if (iLim == idxRef)
					p_EshowerCor->Fill(showerEnergy(Cglobal, Etotcal, true));
			}
		}      //if fill
	...
	std::cout << " -- Summary of total Energy: " << std::endl
			<< " ---- entries " << p_ErecoTotal->GetEntries() << " mean "
			<< p_ErecoTotal->GetMean() << " rms " << p_ErecoTotal->GetRMS()
			<< " underflows " << p_ErecoTotal->GetBinContent(0) << " overflows "
			<< p_ErecoTotal->GetBinContent(p_ErecoTotal->GetNbinsX() + 1)
			<< std::endl << " -- Summary of total Energy corrected: "
			<< std::endl << " ---- entries " << p_EshowerCor->GetEntries()
			<< " mean " << p_EshowerCor->GetMean() << " rms "
			<< p_EshowerCor->GetRMS() << " underflows "
			<< p_EshowerCor->GetBinContent(0) << " overflows "
			<< p_EshowerCor->GetBinContent(p_EshowerCor->GetNbinsX() + 1)
			<< std::endl << " -- Summary of hit spectra low tail: " << std::endl
			<< " ---- entries " << p_hitSpectrum_lowTail->GetEntries()
			<< " mean " << p_hitSpectrum_lowTail->GetMean() << " rms "
			<< p_hitSpectrum_lowTail->GetRMS() << " underflows "
			<< p_hitSpectrum_lowTail->GetBinContent(0) << " overflows "
			<< p_hitSpectrum_lowTail->GetBinContent(
					p_hitSpectrum_lowTail->GetNbinsX() + 1) << std::endl
			<< " -- Summary of hit spectra high tail: " << std::endl
			<< " ---- entries " << p_hitSpectrum_highTail->GetEntries()
			<< " mean " << p_hitSpectrum_highTail->GetMean() << " rms "
			<< p_hitSpectrum_highTail->GetRMS() << " underflows "
			<< p_hitSpectrum_highTail->GetBinContent(0) << " overflows "
			<< p_hitSpectrum_highTail->GetBinContent(
					p_hitSpectrum_highTail->GetNbinsX() + 1) << std::endl;
