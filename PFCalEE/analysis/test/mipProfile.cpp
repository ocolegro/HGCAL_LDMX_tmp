...
int main(int argc, char** argv) { //main  
	...
	("concept", po::value<bool>(&concept)->default_value(true))("pNevts,n",
			...
			po::value<unsigned>(&nPu)->default_value(140))("etamean,e",
	 ...
	 */
	TTree *outtree = new TTree("MipStudy",
			"HGC standalone simulation mip study variables");
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {    //loop on layers
			...
			if (debug > 1)
				std::cout << " Nhits in layer " << iL << " : "
						<< histE->GetNbinsX() * histE->GetNbinsY() << std::endl;
				...
				for (int iY(1); iY < histE->GetNbinsY() + 1; ++iY) {
					...
					for (unsigned ie(0); ie < nEta; ++ie) {
						...
						for (unsigned in(0); in < nNoise; ++in) {
							...
							if (digiE > 0) {
								lMap[ie][in][iL].insert(
										std::pair<std::pair<int, int>, double>(
												std::pair<int, int>(iX, iY),
												digiE));
