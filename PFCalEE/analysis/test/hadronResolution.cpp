...
void getValueFromString(std::vector<std::string>& outputString,
		std::string& sourceString) {
	boost::split(outputString, sourceString, boost::is_any_of(","));
}
;

void getValueFromString(std::vector<int>& outputNumber,
		std::string& sourceString) {
	std::vector<std::string> outputString;
	boost::split(outputString, sourceString, boost::is_any_of(","));
	for (unsigned i(0); i < outputString.size(); i++)
		outputNumber.push_back(atoi(outputString[i].c_str()));
}
;

void getValueFromString(std::vector<unsigned>& outputNumber,
		std::string& sourceString) {
	std::vector<std::string> outputString;
	boost::split(outputString, sourceString, boost::is_any_of(","));
	for (unsigned i(0); i < outputString.size(); i++)
...
void getTotalEnergy(const std::vector<unsigned> & dropLay, TTree* & tree,
		double & sumEE, double & sumFH, double & sumBH,
		std::ostringstream& inputsim, std::ostringstream& inputrec,
		const unsigned nRunsEM = 0) {
	...
	if (nRunsEM == 0) {
		...
		for (unsigned i(0); i < nRunsEM; ++i) {
			...
			if (!testInputFile(tmpsim.str(), simFile))
				continue;
			lSimTree->AddFile(tmpsim.str().c_str());
			tmprec << inputrec.str() << "_run" << i << ".root";
			if (!testInputFile(tmprec.str(), recFile))
				continue;
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
		...
		for (unsigned iP(0); iP < (*genvec).size(); ++iP) { //loop on gen particles    
			...
			if ((*genvec)[iP].trackID() == 1) {
				found = true;
				//set direction
				truthPos = ROOT::Math::XYZPoint((*genvec)[iP].x(),
						(*genvec)[iP].y(), (*genvec)[iP].z());
		...
		if (!found) {
			std::cout
					<< " - Info: no particle G4trackID=1 found, already converted or outside of acceptance ..."
					<< std::endl;
			continue;
		}
		...
		for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) {//loop over rechits
			...
			if (fabs(posx - truthposx) <= 25 && fabs(posy - truthposy) <= 25) {
		...
		for (unsigned iS(0); iS < nSec; iS++) {
			...
			if (type == DetectorEnum::FECAL || type == DetectorEnum::MECAL
				...
				sumFH += recSum[iS];
			} else if (type == DetectorEnum::BHCAL1
					|| type == DetectorEnum::BHCAL2)
				sumBH += recSum[iS];
		}
...
void setCalibFactor(const std::vector<unsigned> & dropLay,
		const std::vector<int> & genEn, const unsigned nRunsEM,
		double & calibSlopeEE, double & calibOffsetEE, double & calibSlopeFH,
		double & calibOffsetFH, double & calibSlopeBH, double & calibOffsetBH,
		...
		TFile* outputFileEM, TCanvas *c1, TCanvas *c2, TCanvas *c3,
		TGraphErrors * slopesummaryEE, TGraphErrors * slopesummaryFH,
		TGraphErrors * slopesummaryBH) {

	const unsigned nP = fileName1.size();
	if (fileName1.size() != genEn.size() || fileName1.size() != fileName2.size()
			|| fileName1.size() != fileName3.size()) {
		std::cout
				<< "Got different number of EE/FH/BH files when set calibration factor"
				<< std::endl;
		return;
	}
	outputFileEM->cd();
	...
	unsigned iP(0);
	for (std::vector<std::pair<std::string, std::string>>::const_iterator iF =
			fileName1.begin(); iF != fileName1.end(); ++iF) {
		...
		getTotalEnergy(dropLay, treeEE[iP], sumEE, sumFH, sumBH, inputsim,
				inputrec, nRunsEM);
		...
		TF1 *fitResult = new TF1("fitResult", "[0]*TMath::Gaus(x,[1],[2],0)",
				p_Energy1->GetXaxis()->GetXmin(),
				p_Energy1->GetXaxis()->GetXmax());
		...
		std::cout << "Total energy  gen=" << E[iP] << " rec= " << Energy1[iP]
				<< "(MIPS)" << std::endl;
		p_Energy1->Delete();
		iP++;
	}
	...
	iP = 0;
	for (std::vector<std::pair<std::string, std::string>>::const_iterator iF =
			fileName2.begin(); iF != fileName2.end(); ++iF) {
		...
		getTotalEnergy(dropLay, treeFH[iP], sumEE, sumFH, sumBH, inputsim,
				inputrec, nRunsEM);
		...
		p_Energy2->Draw("PE");
		TF1 *fitResult = new TF1("fitResult", "[0]*TMath::Gaus(x,[1],[2],0)",
				p_Energy2->GetXaxis()->GetXmin(),
				p_Energy2->GetXaxis()->GetXmax());
		...
		std::cout << "Total energy  gen=" << E[iP] << " rec  = " << Energy2[iP]
				<< "(MIPS)" << std::endl;
		p_Energy2->Delete();
		iP++;
	}
	...
	iP = 0;
	for (std::vector<std::pair<std::string, std::string>>::const_iterator iF =
			fileName3.begin(); iF != fileName3.end(); ++iF) {
		...
		getTotalEnergy(dropLay, treeBH[iP], sumEE, sumFH, sumBH, inputsim,
				inputrec, nRunsEM);
		outputFileEM->cd();
		treeBH[iP]->Write();
		...
		p_Energy3->Draw("PE");
		TF1 *fitResult = new TF1("fitResult", "[0]*TMath::Gaus(x,[1],[2],0)",
				p_Energy3->GetXaxis()->GetXmin(),
				p_Energy3->GetXaxis()->GetXmax());
		...
		std::cout << "Total energy  gen=" << E[iP] << " rec  = " << Energy3[iP]
				<< "(MIPS)" << std::endl;
		p_Energy3->Delete();
		iP++;
	}
...
int main(int argc, char** argv) {    //main  
	...
	("concept", po::value<bool>(&concept)->default_value(true))("pNevts,n",
			po::value<unsigned>(&pNevts)->default_value(0))("doFHEMcalib",
			po::value<bool>(&doFHEMcalib)->default_value(false))("doBHEMcalib",
			po::value<bool>(&doBHEMcalib)->default_value(false))("doFHBHcalib",
			po::value<bool>(&doFHBHcalib)->default_value(false))("eeSimFiles",
			po::value < std::string > (&eeSimFiles)->default_value(""))(
			"fhSimFiles",
			po::value < std::string > (&fhSimFiles)->default_value(""))(
			"bhSimFiles",
			po::value < std::string > (&bhSimFiles)->default_value(""))(
			"eeRecoFiles",
			po::value < std::string > (&eeRecoFiles)->default_value(""))(
			"fhRecoFiles",
			po::value < std::string > (&fhRecoFiles)->default_value(""))(
			"bhRecoFiles",
			po::value < std::string > (&bhRecoFiles)->default_value(""))(
			"filePath,i", po::value < std::string > (&filePath)->required())(
			"nRuns", po::value<unsigned>(&nRuns)->default_value(0))("nRunsEM",
			po::value<unsigned>(&nRunsEM)->default_value(0))("genEnergy",
			po::value < std::string > (&genEnergy)->required())("genEnergyEM",
			po::value < std::string > (&genEnergyEM)->required())(
			"simFileName,s",
			po::value < std::string > (&simFileName)->required())(
			"recoFileName,r",
			po::value < std::string > (&recoFileName)->required())("outPath,o",
			po::value < std::string > (&outPath)->required())("outFileEM,o",
			po::value < std::string > (&outFileEM)->required())("outFilePi,o",
			po::value < std::string > (&outFilePi)->required())("nSiLayers",
			po::value<unsigned>(&nSiLayers)->default_value(2))("debug,d",
			po::value<unsigned>(&debug)->default_value(0))("dropLayers",
			po::value < std::string > (&dropLayers)->required());
...
//
	...
	std::cout << " -- Dropping " << nDrop << " layers: " << dropLayers
			<< std::endl;

	if (doFHEMcalib && doBHEMcalib) {

		if (eeSimFiles == "" || eeRecoFiles == "" || fhSimFiles == ""
				|| fhRecoFiles == "" || bhSimFiles == "" || bhRecoFiles == "") {
			std::cout
					<< "EE/FH/BH files cannot be empty for FHvsEE and BHvsEE calibration"
					<< std::endl;
		} else {
			std::vector<std::string> eeSimFileVec, eeRecoFileVec, fhSimFileVec,
					fhRecoFileVec, bhSimFileVec, bhRecoFileVec;
			...
			std::vector<std::pair<std::string, std::string>> eeFileName,
					fhFileName, bhFileName;
			for (unsigned iF(0); iF < eeSimFileVec.size(); iF++) {
				eeFileName.push_back(
						make_pair(eeSimFileVec[iF], eeRecoFileVec[iF]));
				fhFileName.push_back(
						make_pair(fhSimFileVec[iF], fhRecoFileVec[iF]));
				bhFileName.push_back(
						make_pair(bhSimFileVec[iF], bhRecoFileVec[iF]));
			}
			...
			if (outFileEM.find(".root") == outFileEM.npos)
				outFile << outPath << "/" << outFileEM << "_" << genEn[0]
						<< ".root";
			else
				outFile << outPath << "/" << outFileEM;
			TFile *outputFileEM = TFile::Open(outFile.str().c_str(),
					"RECREATE");

			if (!outputFileEM) {
				std::cout << " -- Error, output file " << outFile.str()
						<< " cannot be opened. Please create output directory. Exiting..."
						<< std::endl;
				return 1;
			} else {
				std::cout << " -- output file " << outputFileEM->GetName()
			...
			setCalibFactor(dropLay, genEn, nRunsEM, ECALslope, ECALoffset,
					FHtoEslope, FHtoEoffset, BHtoEslope, BHtoEoffset,
					eeFileName, fhFileName, bhFileName, outputFileEM, c1, c2,
					c3, slopesummaryEE, slopesummaryFH, slopesummaryBH);
	...
	std::size_t run(0);
	if (nRuns > 0)
		run = simFileName.find("_run");
	std::string simHeader = simFileName.substr(0, begin);
	std::string simAppend("");
	if (nRuns > 0)
		simAppend = simFileName.substr(middle, run - middle);
	else
		simAppend = simFileName.substr(middle, end - middle);
	...
	if (nRuns > 0)
		run_rec = recoFileName.find("_run");
	std::string recHeader = recoFileName.substr(0, begin_rec);
	std::string recAppend;
	if (nRuns > 0)
		recAppend = recoFileName.substr(middle_rec, run_rec - middle_rec);
	else
		recAppend = recoFileName.substr(middle_rec, end_rec - middle_rec);
	...
	for (unsigned iGen(0); iGen < nGenEn; iGen++) {
		...
		std::ostringstream inputsim, inputrec;
		if (nRuns == 0) {
			inputsim << filePath << "/" << simHeader << genEn[iGen]
					<< simAppend;
			inputrec << filePath << "/" << recHeader << genEn[iGen]
					<< recAppend;
			;
			if (!testInputFile(inputsim.str(), simFile))
				return 1;
			...
			for (unsigned i(0); i < nRuns; ++i) {
				inputsim.str("");
				inputsim << filePath << "/" << simHeader << genEn[iGen]
						<< simAppend << "_run" << i << ".root";
				inputrec.str("");
				inputrec << filePath << "/" << recHeader << genEn[iGen]
						<< recAppend << "_run" << i << ".root";
				if (!testInputFile(inputsim.str(), simFile))
					return 1;
				lSimTree->AddFile(inputsim.str().c_str());
				if (!testInputFile(inputrec.str(), recFile))
		...
		HadEnergy myhadReso(dropLay, myDetector, lSimTree, lRecTree, outputFile,
				pNevts);
