...
int main(int argc, char** argv) { //main  
	...
	if (argc < 6) {
		std::cout << " Usage: " << argv[0] << " <nEvts to process (0=all)>"
				<< " <path to input files>" << " <name of input reco file>"
	...
	if (!recFile) {
		std::cout << " -- Error, input file cannot be opened. Exiting..."
				<< std::endl;
		return 1;
	}
	...
	TH1F *p_nRecHits = new TH1F("p_nRecHits", "n(RecHits)", 1000, 0, 500000);
	p_nRecHits->StatOverflows();

	TProfile *p_EvsLayer = new TProfile("p_EvsLayer", "Average E vs Layer", 30,
			1, 30);
	...
	for (unsigned iL(0); iL < nLayers; iL++) {
		...
		p_AveE[iL] = new TProfile(p_AveE_Name.str().c_str(),
				"Average Energy vs eta", 40, 1.5, 3.5);
	}
	...
	for (unsigned iL(0); iL < nLayers; iL++) {
		...
		p_Occupancy[iL] = new TH2F(p_AveE_Name.str().c_str(),
				"RecoHits Occupancy", 340, -170, 170, 340, -170, 170);
	}
	...
	const unsigned nEvts = (
			(pNevts > lRecTree->GetEntries() || pNevts == 0) ?
					static_cast<unsigned>(lRecTree->GetEntries()) : pNevts);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iH(0); iH < (*rechitvec).size(); ++iH) { //loop on rechits
			...
			if (ievt == 0) {
				if (Z_layer[layer] == 0)
					Z_layer[layer] = lHit.get_z() / 10.0;
			}

			for (unsigned iEta(0); iEta < 40; iEta++) {
				if (eta > 1.5 + iEta * 0.05 && eta < 1.5 + (iEta + 1) * 0.05)
					EvsLayer[layer][iEta] += energy;
			}
			...
			for (unsigned iL(0); iL < nLayers; iL++) {
				for (int iX(-170); iX < 170; iX++) {
					for (int iY(-170); iY < 170; iY++) {
						...
						if (radius > 170 || radius < 15)
							continue;
						double R = sqrt(
								radius * radius + Z_layer[iL] * Z_layer[iL]);
						double theta = acos(Z_layer[iL] / R);
						double eta = -log(tan(theta / 2.));
						for (unsigned iEta(0); iEta < 40; iEta++) {
							if (eta > 1.5 + iEta * 0.05
									&& eta < 1.5 + (iEta + 1) * 0.05)
								CellNumber[iL][iEta] += 1;
						}
					}
				}
			}
		...
		for (unsigned iEta(0); iEta < 40; iEta++) {
			for (unsigned iL(0); iL < nLayers; iL++) {
				p_AveE[iL]->Fill(1.5 + iEta * 0.05,
						EvsLayer[iL][iEta] / CellNumber[iL][iEta]);
			}
		}
