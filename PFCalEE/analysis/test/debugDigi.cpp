...
int main(int argc, char** argv) { //main  
	...
	TH1F *p_simhitEnergy = new TH1F("p_simhitEnergy", ";E_{hit} (MIPs)", 200, 0,
			10);
	TH1F *p_xtalkhitEnergy = new TH1F("p_xtalkhitEnergy", ";E_{hit} (MIPs)",
			200, 0, 10);

	TH2F *p_pixvspe = new TH2F("p_pixvspe", ";npe;npixels", 500, 0, 5000, 1160,
			0, 1160);
	TH1F *p_npixels = new TH1F("p_npixels", ";pixels", 1160, 0, 1160);
	TH1F *p_npixelssmeared3 = new TH1F("p_npixelssmeared3",
			";pixels smeared #sigma=3", 1160, 0, 1160);
	TH1F *p_npixelssmeared6 = new TH1F("p_npixelssmeared6",
			";pixels smeared #sigma=6", 1160, 0, 1160);
	TH2F *p_outvsnpix3 = new TH2F("p_outvsnpix3",
			";pixels smeared #sigma=3;E_{hit} (MIPs)", 1160, 0, 1160, 1000, 0,
			500);
	TH2F *p_outvsnpix6 = new TH2F("p_outvsnpix6",
			";pixels smeared #sigma=6;E_{hit} (MIPs)", 1160, 0, 1160, 1000, 0,
			500);

	TH1F *p_rand3hitEnergy = new TH1F("p_rand3hitEnergy", ";E_{hit} (MIPs)",
			200, 0, 10);
	TH1F *p_rand6hitEnergy = new TH1F("p_rand6hitEnergy", ";E_{hit} (MIPs)",
			200, 0, 10);
	TH1F *p_rechitEnergy = new TH1F("p_rechitEnergy", ";E_{hit} (MIPs)", 200, 0,
			10);
	...
	TH1F *p_simhitEnergy_tail = new TH1F("p_simhitEnergy_tail",
			";E_{hit} (MIPs)", 200, 0, 1000);
	TH1F *p_xtalkhitEnergy_tail = new TH1F("p_xtalkhitEnergy_tail",
			";E_{hit} (MIPs)", 200, 0, 1000);
	TH1F *p_rand3hitEnergy_tail = new TH1F("p_rand3hitEnergy_tail",
			";E_{hit} (MIPs)", 200, 0, 1000);
	TH1F *p_rand6hitEnergy_tail = new TH1F("p_rand6hitEnergy_tail",
			";E_{hit} (MIPs)", 200, 0, 1000);
	TH1F *p_rechitEnergy_tail = new TH1F("p_rechitEnergy_tail",
			";E_{hit} (MIPs)", 200, 0, 1000);
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		for (unsigned iH(0); iH < (*simhitvec).size(); ++iH) {  //loop on hits
			...
			if (layer >= nLayers) {
				std::cout << " WARNING! SimHits with layer " << layer
		...
		for (unsigned iL(0); iL < nLayers; ++iL) {  //loop on layers
			...
			double absweight = ((*ssvec)[iL].volX0trans())
					/ ((*ssvec)[0].volX0trans());
			//3*3 cells
			...
			for (int iX(1); iX < histE->GetNbinsX() + 1; ++iX) {
				for (int iY(1); iY < histE->GetNbinsY() + 1; ++iY) {
					...
					digiE = myDigitiser.digiE(xtalkE, p_pixvspe, p_npixels,
							p_npixelssmeared3, p_outvsnpix3);
					...
					TH2F *ht1 = 0;
					TH1F *ht2 = 0;
					digiE = myDigitiser.digiE(xtalkE, ht1, ht2,
							p_npixelssmeared6, p_outvsnpix6);
