...
int main() {    //EvtDisplay(){//main  
	...
	ltitleBase << "VBF jet"
	//<< " Event #" << event[ievt]
			<< ", E_{1#times1 cm^{2}} > " << mipThresh << " Mips";
	...
	for (unsigned iC(0); iC < nCanvas; ++iC) {
		lName.str("");
		lName << "myc" << iC;
		myc[iC] = new TCanvas(lName.str().c_str(), lName.str().c_str(), 800,
				800);
	}
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on events
		...
		if (!inputFile) {
			std::cout << " -- Error, input file " << lName.str()
					<< " cannot be opened. Going to next..." << std::endl;
			continue;
			//return 1;
		}
		...
		if (!p_xyz) {
			std::cout
					<< " -- ERROR, pointer for XYZ histogram is null. Exiting..."
					<< std::endl;
			return 1;
		}
		p_xyz->Sumw2();

		std::cout << " --min and max of hist = " << p_xyz->GetMinimum() << " "
				<< p_xyz->GetMaximum() << std::endl;
		...
		for (unsigned iS(0); iS < nSlices; ++iS) {
			...
			p_slice[iS] = new TH3F(lName.str().c_str(), ";z(mm);x(mm);y(mm)",
					p_xyz->GetNbinsX(), p_xyz->GetXaxis()->GetBinLowEdge(1),
					p_xyz->GetXaxis()->GetBinLowEdge(p_xyz->GetNbinsX() + 1),
					p_xyz->GetNbinsY(), p_xyz->GetYaxis()->GetBinLowEdge(1),
					p_xyz->GetYaxis()->GetBinLowEdge(p_xyz->GetNbinsY() + 1),
					p_xyz->GetNbinsZ(), p_xyz->GetZaxis()->GetBinLowEdge(1),
					p_xyz->GetZaxis()->GetBinLowEdge(p_xyz->GetNbinsZ() + 1));
