...
int main(int argc, char ** argv) {
	...
	TH2F *hvtx_tvsz = new TH2F("hvtx_tvsz", ";z (mm); t (ps);vertices", 1000,
			-200, 200, 1200, -600, 600);
	TH1F *hvtx_t_z[nZ];
	for (unsigned i(0); i < nZ; ++i) {
		...
		hvtx_t_z[i] = new TH1F(label.str().c_str(), ";t (ps);vertices", 120,
				-600, 600);
	}
	...
	TH1F *hProtonLog = new TH1F("hProtonLog", ";log(p) (log(GeV));particles",
			100, -2, 2);
	TH1F *hNeutronLog = new TH1F("hNeutronLog", ";log(p) (log(GeV));particles",
			100, -2, 2);
	TH1F *hPipmLog = new TH1F("hPipmLog", ";log(p) (log(GeV));particles", 100,
			-2, 2);
	...
	for (unsigned i(0); i < nFiles; ++i) {
		...
		lname
				<< "/afs/cern.ch/work/a/amagnan/public/HepMCFiles/ggHgg_1428658356.dat";
		...
		while (evt) {
			...
			if (ievt % 10000 == 0)
				std::cout << "Processing Event Number " << ievt << std::endl;
