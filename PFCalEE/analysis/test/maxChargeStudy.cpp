...
int main(int argc, char** argv) {  //main  
	...
	("concept", po::value<bool>(&concept)->default_value(true))("pNevts,n",
			...
			po::value<unsigned>(&nPu)->default_value(140));
	...
	TFile *inputSig =
			TFile::Open(
					"/afs/cern.ch/work/a/amagnan/PFCalEEAna/PLOTS/gitV00-02-14/version12/Hgg/200um/E3by3pu0.root");
	...
	TH2F *hxy = new TH2F("hxy", ";x (mm);y (mm); hits", 113, -1695, 1695, 113,
			-1695, 1695);

	TH2F *hphieta = new TH2F("hphieta", ";#phi;#eta; hits", 100, -3.1416,
			3.1416, 50, 1.6, 2.4);
	TH2F *hxyAbove = new TH2F("hxyAbove", ";x (mm);y (mm); hits (E>250 fC)",
			113, -1695, 1695, 113, -1695, 1695);

	TH2F *hphietaAbove = new TH2F("hphietaAbove", ";#phi;#eta; hits (E>250 fC)",
			100, -3.1416, 3.1416, 50, 1.6, 2.4);

	TH1F *nAbove = new TH1F("nAbove", ";n(E>250 fC)", 50, 0, 50);
	TH1F *nTot = new TH1F("nTot", ";n in SR", 300, 0, 300);
	TH2F *nAbovevsLayer = new TH2F("nAbovevsLayer",
			";layer;n_{1#times 1 cm^{2}}(E>250 fC)/n(1.9 < #eta < 2.1)",
			nLayers, 0, nLayers, 10000, 0, 0.005);
	TProfile *prof_nAbovevsLayer = new TProfile("prof_nAbovevsLayer",
			";layer;n_{1#times 1 cm^{2}}(E>250 fC)/n(1.9 < #eta < 2.1)",
			nLayers, 0, nLayers, 0, 1);

	TH1F *EmaxNeighbourSC3D = new TH1F("EmaxNeighbourSC3D",
			";E_{max}^{SC3D} (Mips);hits in 1.9 < #eta < 2.1", 400, 0, 100);

	TH2D *EneighvsEcell = new TH2D("EneighvsEcell", ";E_{cell};E_{max}^{SC3D}",
			100, 100, 1000, 400, 0, 100);

	TProfile *hLayerProfile_above50mips = new TProfile(
			"hLayerProfile_above50mips", ";layer;E (mips) in 1.9 < #eta < 2.1",
			nLayers, 0, nLayers, 0, 10000);

	const unsigned nEvts = (
			(pNevts > lTree->GetEntries() || pNevts == 0) ?
					static_cast<unsigned>(lTree->GetEntries()) : pNevts);
	...
	for (unsigned iL(0); iL < nLayers; ++iL) {
		lArea[iL] = areaInCm2(radius(eta[0] - deta, posz[iL]))
				- areaInCm2(radius(eta[0] + deta, posz[iL]));
		std::cout << " Layer " << iL << " area = " << lArea[iL] << " cm^2 r1="
				<< radius(eta[0] - deta, posz[iL]) << " r2="
				<< radius(eta[0] + deta, posz[iL]) << std::endl;
	}
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) {  //loop on entries
		...
		fillKDTree(_hit_kdtree, _hit_nodes, lHitVec);

		if (debug)
			std::cout << " ----- Number of hits in vector : " << lHitVec.size()
					<< std::endl;
		...
		for (unsigned iH(0); iH < lHitVec.size(); ++iH) {
			...
			for (const KDNode& nbourpoint : neighbours) {
				...
				if ((nbour.layer == iL
						...
						|| (abs(nbour.layer - iL) < 2 && nbour.x == current.x
								&& nbour.y == current.y)) {
					if (nbour.e > EmaxSC3D)
						EmaxSC3D = nbour.e;
			...
			if (current.e > Ethresh) {
				...
				if (EmaxSC3D > 50)
					fillShowerProfile = true;
			}
		}

		for (unsigned iL(0); iL < nLayers; ++iL) {
			if (fillShowerProfile)
				hLayerProfile_above50mips->Fill(iL, EperLayer[iL]);
